 ...
 */
#if (defined __GNUC__ &&                                                       \
     (defined __i686__ || defined __i386__ || defined__x86_64__))
...
static inline bool PUSH(int myid) {
  long *p =
      (long *)(1 + counter.fetch_add(1)); //  atomic_long_inc_return(&counter));
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < numC; ++i) {
    ...
    do
      ;
    while (!queue_enqueue(msq, (void *)FF_EOS));
#else
    do
      ;
    while (!msq->push((long *)FF_EOS));
#endif
  }
  ...
  for (int i = 0; i < ntasks; ++i)
    if (results[i] != i + 1) {
      std::cerr << "WRONG result in position " << i << " is " << results[i]
                << " should be " << i + 1 << "\n";
      wrong = true;
    }
