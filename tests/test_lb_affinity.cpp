...
public:
  emitter(ff_loadbalancer *const lb, const svector<ff_node *> &workers)
    ...
    for (int i = 0; i < NWORKERS; i++) completed[i] = false;
    done = 0;
  }
  ...
  void *svc(void *t) {
    ...
    if (worker_task == NULL) {
      for (int j = 0; j < INBUF_Q_SIZE; j++) {
        for (int i = 0; i < NWORKERS; i++) {
          ...
          //
          int targetcore =
              threadMapper::instance()->getCoreId(lb->getTid(workers[i]));
          ...
          if (res)
            printf("sent to worker %ld on core %ld mnode %ld\n",
                worker_task->sourceW, worker_task->core, worker_task->mnode);
          else
            printf("ERROR: send failed - should never happen - task is lost - "
                   "queue are too short\n");
        }
      }
    } else {
    ...
    /* 
           ...
           if (!completed[i])
             return GO_ON ;
		*/
    for (int i = 0; i < NWORKERS; i++) done &= completed[i];
    if (done) {
      free(worker_task);
      delete[] completed;
      return NULL;
