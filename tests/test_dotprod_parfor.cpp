...
#pragma omp parallel for default(shared)                                \
    ...
    for (long i = 0; i < arraySize; ++i) sum += A[i] * B[i];
  ...
  // init data
  tbb::parallel_for(
      tbb::blocked_range<long>(0, arraySize, CHUNKSIZE),
  ...
  for (int z = 0; z < NTIMES; ++z) {
    ...
    sum += tbb::parallel_reduce(
        tbb::blocked_range<long>(0, arraySize, CHUNKSIZE), double(0),
        [=](const tbb::blocked_range<long> &r, double in) {
          return std::inner_product(A + r.begin(), A + r.end(), B + r.begin(),
              in, std::plus<double>(), std::multiplies<double>());
        },
        std::plus<double>(), ap);
  }
  ffTime(STOP_TIME);
  ...
  {
    parallel_for(0, arraySize, 1, CHUNKSIZE, [&](const long j) {
      A[j] = j * 3.14;
      B[j] = 2.1 * j;
    });
    ParallelForPipeReduce<double *> pfr(
        nworkers, (nworkers < ff_numCores())); // spinwait is set to true if ...

    auto Map = [&](const long start, const long stop, const int thid,
                   ff_buffernode &node) {
      if (start == stop) return;
      double localsum = 0.0;
      for (long i = start; i < stop; ++i) localsum += A[i] * B[i];
      node.put(new double(localsum));
    };
    auto Reduce = [&](const double *v) { sum += *v; };
  ...
  // (default) FastFlow version
  {
    ...
    pfr.parallel_for(0, arraySize, 1, CHUNKSIZE, [&](const long j) {
      A[j] = j * 3.14;
      B[j] = 2.1 * j;
    });
    ...
    for (int z = 0; z < NTIMES; ++z) {
      ...
      pfr.parallel_reduce(
          sum, 0.0, 0, arraySize, 1, CHUNKSIZE,
  ...
  for (int z = 0; z < NTIMES; ++z) {
    ...
    FF_PARFORREDUCE_START(
        dp, sum, 0.0, i, 0, arraySize, 1, CHUNKSIZE, nworkers) {
      sum += A[i] * B[i];
