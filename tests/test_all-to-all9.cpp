...
int main() {
  ...
  { // first test, workers are multi-output
    ...
    for (int i = 0; i < nfilter1; ++i) W1.push_back(new ff_comb(helper, F1));
    std::vector<ff_node *> W2;
    for (int i = 0; i < nfilter2; ++i) W2.push_back(new Filter2);
  ...
  { // second test, workers are standard node
    ...
    std::vector<ff_node *> W1;
    for (int i = 0; i < nfilter1; ++i) W1.push_back(new Filter1(nfilter2));
    std::vector<ff_node *> W2;
    for (int i = 0; i < nfilter2; ++i) W2.push_back(new Filter2);
