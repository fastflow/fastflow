...
public:
  E(svector<ff_node *> &w, long id, long ntasks, long niterations)
      : id(id), ntasks(ntasks), niterations(niterations) {
    set_output(w);
  }
  ...
  void svc_end() { printf("E%ld terminating\n", id); }
...
public:
  ...
  void svc_end() { printf("N%ld terminating, got %ld tasks\n", id, received); }
...
public:
  C(svector<ff_node *> &w, long id) : id(id) { set_input(w); }
  ...
  void svc_end() { printf("C%ld terminating, got %ld tasks\n", id, received); }
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 2; ++i) e[i]->run();
  c[0]->run();

  printf("All running\n");

  // Waiting for all to complete. All nodes should be run before waiting.
  for (int i = 0; i < 2; ++i) e[i]->wait();
  for (int i = 0; i < 6; ++i) n[i]->wait();
  c[0]->wait();
