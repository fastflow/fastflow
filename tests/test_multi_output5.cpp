...
public:
  Emitter1(long ntasks, ff_loadbalancer *lb, ff_buffernode *const cmdChannel)
      : ntasks(ntasks), lb(lb), cmdChannel(cmdChannel) {}
...
struct Worker1 : ff_node_t<long> {
  ...
  void svc_end() { printf("Worker1 id=%ld going to sleep\n", get_my_id()); }
};
...
public:
  ...
  long *svc(long *t) {
    ...
    if (data.size() > 0) {
      ...
      ready[wid] = false;
      --nready;
    } else if (eos_received == ready.size() &&
               (nready + sleeping) == ready.size()) {
  ...
  void eosnotify(ssize_t id) {
    if (id == -1) { // we have to receive all EOS from the previous stage
      eos_received++;
      printf(
          "EOS received eos_received = %u nready = %u\n", eos_received, nready);
      if (eos_received == ready.size() && (nready + sleeping) == ready.size() &&
          data.size() == 0) {
...
struct Worker2 : ff_monode_t<long> {
  ...
  void svc_end() { printf("Worker2 id=%ld going to sleep\n", get_my_id()); }
};
