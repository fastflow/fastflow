...
public:
  TasksGenerator(int nsims) : nsims(nsims) {}
...
struct out_task_t {
  out_task_t(input_task_t *st, double t, sample_t *m)
      : task_id(st->id), progress(t), monitor(m) {}
  int task_id;
  double progress;
  sample_t *monitor;
};
...
public:
  void *svc(void *task) {
    ...
    for (int i = 0; i < POINTSPEROFFLOAD; ++i) {
      ...
      double x = 0;
      for (int j = 0; j < RANDOMSTEPS; ++j) x += rand() % 10;
      x /= (10 * RANDOMSTEPS);
      t->progress += 0.1;
...
public:
  ...
  void *svc(void *task) {
    ...
    for (int i = 0; i < nworkers; i++) workers.push_back(new Acc1_Worker());
    ...
    while (offloaded < NOFFLOADS) {
      //offload
      for (int i = 0; i < nsims; ++i) farm.offload(input_tasks[i]);
      ++offloaded;
    ...
    for (int i = 0; i < nsims; ++i) delete input_tasks[i];
    delete &input_tasks;
    return GO_ON;
  }
...
typedef struct window_task_t {
  window_task_t(double t, sample_t **s) : sample_time(t), samples(s) {}
  double sample_time;
  sample_t **samples;
} window_task_t;
...
public:
  TrajectoriesAlignment(int nsims)
      : n_simulations(nsims), n_samples(NOFFLOADS * POINTSPEROFFLOAD) {
    //sampling queues
  ...
  void svc_end() {
    for (unsigned int i = 0; i < n_simulations; ++i) delete sq[i];
    delete tq;
    delete sizes;
  }
  ...
  void *svc(void *task) {
    ...
    if (!push_counter) {
      ...
      while (n_pop--) {
        ...
        sample_t **instant_samples = (sample_t **)malloc(
            n_simulations * sizeof(sample_t *)); //to be deallocated
        ...
        ff_send_out(
            new window_task_t(*(double *)sample_time_, instant_samples));
        free(sample_time_);
      }
    }
    return GO_ON;
...
public:
  ...
  double get_last_time() { return *window_times[end]; }
  ...
  valarray<T> &front_sample() { return *window[end]; }

  void add_front(T datum, int sim_id) { (*window[end])[sim_id] = datum; }
  ...
  unsigned int size() { return fill; }

  bool full() { return fill == window_size; }
  ...
  friend ostream &operator<<(ostream &os, Stat_Window &w) {
    for (unsigned int j = 0; j < w.size(); ++j) {
      for (unsigned int i = 0; i < w.n_simulations; ++i) os << "|\t" << w[j][i];
      os << endl;
    }
    return os;
  }
...
public:
  WindowsGenerator(int nsims) : n_simulations(nsims), n_monitors(1) {
    //stat windows
  ...
  void svc_end() {
    for (int i = 0; i < n_monitors; ++i) delete stat_windows[i];
  }
  ...
  void *svc(void *task) {
    ...
    //slide
    for (int i = 0; i < n_monitors; i++) stat_windows[i]->slide(sample_time);
...
typedef struct stat_task_t {
  ...
  int opcode;
  stat_task_t(Stat_Window<double> *w, int op) : window(w), opcode(op) {}
} stat_task_t;
...
typedef struct res_task_t {
  ...
  int opcode;
  double value;
  res_task_t(double t, int o, double v) : time(t), opcode(o), value(v) {}
} res_task_t;
...
public:
  Acc2Engine(int nworkers) : n_monitors(1), n_stat_workers(nworkers) {
    stat_windows = NULL;
