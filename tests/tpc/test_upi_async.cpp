...
struct ExampleTaskCopy : public baseTPCTask<ExampleTaskCopy> {
  ...
  ExampleTaskCopy(
      uint32_t *in, uint32_t sizein, uint32_t *out, uint32_t sizeout)
  ...
  void setTask(const ExampleTaskCopy *t) {
    ...
    setInPtr(&t->sizein, 1, BitFlags::COPYTO, BitFlags::DONTREUSE,
        BitFlags::RELEASE);
    setInPtr(t->in, t->sizein, BitFlags::COPYTO, BitFlags::DONTREUSE,
        BitFlags::RELEASE);
    setInPtr(&t->sizeout, 1, BitFlags::COPYTO, BitFlags::DONTREUSE,
        BitFlags::RELEASE);
    // neither copied back nor released
    setOutPtr(t->out, t->sizeout, BitFlags::DONTCOPYBACK, BitFlags::DONTREUSE,
...
struct ExampleTask : public baseTPCTask<ExampleTask> {
  ...
  ExampleTask(uint32_t *in, uint32_t sizein, uint32_t start, uint32_t stop,
      uint32_t *result)
  ...
  void setTask(const ExampleTask *t) {
    ...
    setInPtr(t->in, t->sizein, BitFlags::DONTCOPYTO, BitFlags::REUSE,
        BitFlags::DONTRELEASE);
...
static void check(uint32_t to, uint32_t from, uint32_t result) {
  if (result != ingauss(to, from + 1))
    std::cerr << "Wrong result: " << result
              << " (expected: " << ingauss(to, from + 1) << ")\n";
  else
...
int main() {
  ...
  for (size_t j = 0; j < size; ++j) delays[j] = j + 1;
  ...
  for (size_t i = 0; i < 4; ++i)
    tasks.push_back(
        ExampleTask(delays, size, i * 64, i * 64 + 64, &results[i]));
  for (size_t i = 0; i < 4; ++i)
