...
struct VectorAddDotTask
    : public baseTPCTask<VectorAddDotTask, VectorAddDotTask> {
  VectorAddDotTask() : a(nullptr), b(nullptr), c(nullptr), len(0), r(nullptr) {}

  VectorAddDotTask(
      int32_t *r, uint32_t len, elem_t const *a, elem_t const *b, elem_t *c)
...
static void check4(uint32_t r, std::vector<elem_t> &H) {
  bool const res4 =
      check(MAX_LEN, &H[0]); // return value overflows, so is ignored
