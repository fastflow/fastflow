...
public:
  Thread(int nbarriers, bool check = true)
      : nbarriers(nbarriers), check(check) {}
  ...
  void set_id(ssize_t id) { ff_node::set_id(id); }
  int run(bool = false) { return ff_node::run(); }
  int wait() { return ff_node::wait(); }

  double wffTime() { return ff_node::wffTime(); }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < nthreads; ++i) N[i]->wait();

  std::vector<double> threadTime(nthreads, 0.0);
  for (int i = 0; i < nthreads; ++i) threadTime[i] = N[i]->wffTime();

  std::vector<double>::iterator it = std::max_element(
      threadTime.begin(), threadTime.end(), std::less<double>());
