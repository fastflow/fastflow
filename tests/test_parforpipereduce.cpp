...
int main(int argc, char *argv[]) {
...
#if 1
  {
    ...
    ParallelForPipeReduce<std::vector<long> *> pfr(
        nworkers, true); // spinwait is set to true
  ...
  // (default) FastFlow version
  {
    ...
    ff::ffTime(ff::START_TIME);
    for (int z = 0; z < NTIMES; ++z) {
      pfr.parallel_for_thid(
          0, arraySize, 1, CHUNKSIZE, [&](const long i, const int thid) {
            // waste some time to simulate a more complex computation
