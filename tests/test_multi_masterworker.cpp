...
struct Worker : ff_node {
  ...
  void svc_end() { printf("FARM(%d) W(%ld) EXITING\n", id, get_my_id()); }
};

class Emitter : public ff_node {
public:
  Emitter(int id, ff_loadbalancer *const lb)
      : eosarrived(false), _id(id), numtasks(0), lb(lb) {}
...
public:
  ...
  void *svc(void *) {
    for (long i = 1; i <= streamlen; ++i) ff_send_out(new long(i));
    return NULL;
  }
