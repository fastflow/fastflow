...
#if defined(PAPI_PERF)
  if (PAPI_register_thread() != PAPI_OK) std::cerr << "PAPI_register_thread\n";
#endif

  if (cpu_P != -1)
    if (ff_mapThreadToCpu(cpu_P) != 0)
      std::cerr << "Cannot map producer thread to cpu " << cpu_P
                << ", going on...\n";
...
void *C(void *) {
  ...
  if (cpu_C != -1)
    if (ff_mapThreadToCpu(cpu_C) != 0)
      std::cerr << "Cannot map consumer thread to cpu " << cpu_C
                << ", going on...\n";
  ...
  const int eventlist[] = {PAPI_L1_DCM, PAPI_L2_DCM, 0};
    ...
    if (b->size()) {
      ...
      if (task.b == (void *)FF_EOS) {
        ...
        if (task.b != (void *)(0x1234 + k))
          std::cerr << " ERROR, wrong task received " << task.b << " expected "
                    << (void *)(0x1234 + k) << " \n";
#else
        for (int j = 0; j < 8; ++j) {
          long *t = (long *)task.b;
...
#if defined(UBUFFER_STATS)
  printf("\nlength:\n");
  for (unsigned int i = 0; i < length.size(); ++i) printf("%ld ", length[i]);
  printf("\nmiss:\n");
  for (unsigned int i = 0; i < miss.size(); ++i) printf("%ld ", miss[i]);
  printf("\nhit:\n");
  for (unsigned int i = 0; i < hit.size(); ++i) printf("%ld ", hit[i]);
#endif
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (argc != 3) {
      if (argc == 5) {
        ...
        std::cerr << "use: " << argv[0]
                  << " (base-)buffer-size ntasks [#P-core] [#C-core]\n";
        return -1;
      }
    }
    size = atoi(argv[1]);
  ...
  if ((retval = PAPI_thread_init((unsigned long (*)(void))(pthread_self))) !=
      PAPI_OK) {
