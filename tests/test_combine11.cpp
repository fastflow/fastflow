...
struct Generator : ff_monode_t<long> {
  long *svc(long *) {
    for (size_t i = 1; i <= ntasks; ++i) ff_send_out((long *)i);
    return EOS;
  }
};
...
struct Seq : ff_node_t<long> {
  long *svc(long *in) {
    V.push_back(in);
    if (V.size() == soglia) {
      for (size_t i = 0; i < V.size(); ++i) ff_send_out(V[i]);
      V.resize(0);
    }
    return GO_ON;
  }
...
int main(int argc, char *argv[]) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < nworkers; ++i) W1.push_back(new Worker);
  ...
  printf("TEST2 DONE\n");
  {
    ...
    for (size_t i = 0; i < nworkers; ++i) W0.push_back(new Seq);
