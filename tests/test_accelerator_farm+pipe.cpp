...
public:
  ...
  int svc_init() { return ff_setPriority(priority_level); }
  ...
  if (argc > 1) {
    if (argc != 4 && argc != 5) {
      std::cerr
          << "use:\n"
          << " " << argv[0]
          << " max-stream-length num-farm-workers iterations [priority]\n";
      std::cerr << " NOTE: <priority> values less then 0 require CAP_SYS_NICE "
                   "capability\n\n";
      return -1;
    }
