...
int main(int argc, char *argv[]) {
  ...
  usleep(500000);
  {
    ...
    auto pipe = combine_farms(
        farm1, (Collector1 *)nullptr, farm2, (Collector1 *)nullptr, true);
    if (pipe.run_and_wait_end() < 0) {
      error("running pipe\n");
      return -1;
    }
  ...
  usleep(500000);
  {
    ...
    auto pipe = combine_farms(
        farm1, (Collector1 *)nullptr, farm2, (Collector1 *)nullptr, false);
    if (pipe.run_and_wait_end() < 0) {
      error("running pipe\n");
      return -1;
    }
  ...
  usleep(500000);
  {
    ...
    for (int i = 0; i < nworkers1; ++i) W1.push_back(make_unique<Worker1>());
    std::vector<std::unique_ptr<ff_node>> W2;
    for (int i = 0; i < nworkers1; ++i) W2.push_back(make_unique<Worker2>());
    ff_Farm<> farm1(std::move(W1), E, C1);
    ff_Farm<> farm2(std::move(W2), E2, C2);
