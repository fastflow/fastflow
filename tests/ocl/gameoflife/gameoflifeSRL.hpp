...
FF_OCL_STENCIL_ELEMFUNC_2D(
    mapf, unsigned char, h, w, r, c, unsigned char alive_in = GET_IN(r, c);
...
FF_OCL_REDUCE_COMBINATOR(reducef, unsigned char, x, y, return x || y;);

#endif // BUILD_WITH_SOURCE

struct oclTaskGol : public ff::baseOCLTask_2D<oclTaskGol, unsigned char> {
  oclTaskGol() : M_in(NULL), M_out(NULL), niters(0), someone(0), nrows(0) {}
  oclTaskGol(unsigned char *M_in_, unsigned char *M_out_, unsigned int niters_,
      unsigned int nrows_)
