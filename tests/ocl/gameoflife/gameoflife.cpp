...
inline char pixel2char(unsigned char x) { return x ? '*' : ' '; }
...
unsigned int get_and_print_diff(
    unsigned char *a, unsigned char *b, unsigned int w) {
  unsigned int ndiff = 0;
  for (unsigned int r = 0; r < w; ++r)
    for (unsigned int c = 0; c < w; ++c)
      if (a[at(r, c, w)] != b[at(r, c, w)]) {
        printf("a(%u,%u) = %c, b(%u,%u) = %c\n", r, c,
            pixel2char(a[at(r, c, w)]), r, c, pixel2char(b[at(r, c, w)]));
        ++ndiff;
      }
  printf("+ %u diff found\n", ndiff);
  return ndiff;
...
//	bitmap_seed[at(5, 3, nrows)] = 1;
#ifdef VERIFY
  double sum = 0;
  for (unsigned int i = 0; i < length; ++i) sum += (double)bitmap_seed[i];
  sum /= length;
  printf("+ seed alive-%% = %f\n", sum * 100);
#endif
#ifdef PRINT
  ...
  ff::ff_stencilReduceLoopOCL_2D<oclTaskGol> oclStencilReduceOneShot(
      oclt, "cl_code/gameoflife.cl", "mapf", "reducef", 0, NULL, NACC, 1, 1);

  oclStencilReduceOneShot.saveBinaryFile();
  oclStencilReduceOneShot.reuseBinaryFile();
#else
  ff::ff_stencilReduceLoopOCL_2D<oclTaskGol> oclStencilReduceOneShot(
      oclt, mapf, reducef, 0, NULL, NACC, 1, 1);
#endif
  ...
  for (unsigned int iter = 0; iter < niters; ++iter) {
    ...
    pf.parallel_for(0, nrows, [&](const long r) {
      for (unsigned int c = 0; c < nrows; ++c) {
        ...
        naliven += has_top(r) && has_right(c, w)
                       ? bitmap_in_seq[at(r - 1, c + 1, w)]
                       : 0;
        naliven += has_right(c, w) ? bitmap_in_seq[at(r, c + 1, w)] : 0;
        naliven += has_bottom(r, w) && has_right(c, w)
                       ? bitmap_in_seq[at(r + 1, c + 1, w)]
                       : 0;
        naliven += has_bottom(r, w) ? bitmap_in_seq[at(r + 1, c, w)] : 0;
        naliven += has_bottom(r, w) && has_left(c)
                       ? bitmap_in_seq[at(r + 1, c - 1, w)]
                       : 0;
        naliven += has_left(c) ? bitmap_in_seq[at(r, c - 1, w)] : 0;
        bitmap_out_seq[at(r, c, w)] =
            naliven == 3 || (alive_in && naliven == 2);
      }
    });
  }
  gettimeofday(&tv2, NULL);
  double extime_seq = ((double)tv2.tv_sec - (double)tv1.tv_sec) * 1000 +
                      ((double)tv2.tv_usec - (double)tv1.tv_usec) / 1000;
#ifdef PRINT
