...
FF_OCL_STENCIL_ELEMFUNC(mapf, int, useless, i, in, (void)useless;
                        return in[i] + 1;);
...
unsigned int check(basictype *M, basictype r, int size) {
  ...
  for (unsigned int k = 0; k < niters; ++k) {
    ...
    in = M_out;
    M_out = tmp;
    for (int i = 0; i < size; ++i) M_out[i] = mapf_(in, i);
    //reduce
    red = 0;
...
int main(int argc, char *argv[]) {
  ...
  ff_stencilReduceLoopOCL_1D<oclTask> oclStencilReduceOneShot(
      oclt, mapf, reducef, 0, nullptr, nacc, WINWIDTH);
