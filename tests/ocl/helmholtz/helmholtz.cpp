 ...
 ******************************************************/
void initialize(
    int n, int m, float alpha, float *dx, float *dy, float *u, float *f) {
  int xx, yy;
...
public:
  HelmholtzTask(int n, int m, float dx, float dy, float alpha, float omega,
      float *u, float *f, int iters_)
      : w(n), h(m), u(u), f(f), Error(0.0), iters(iters_) {

    const float ax = 1.0 / (dx * dx); /* X-direction coef */
    const float ay = 1.0 / (dy * dy); /* Y_direction coef */
    const float b =
        -2.0 / (dx * dx) - 2.0 / (dy * dy) - alpha; /* Central coeff */

    std::cout << "ax = " << ax << ", ay = " << ay << ", b = " << b
  ...
  HelmholtzTask()
      : h(-1), w(-1), u(nullptr), uold(nullptr), Const(nullptr), f(nullptr),
        Error(0.0), iters(0) {}
  ...
  void setTask(HelmholtzTask *t) {
    ...
    ff::MemoryFlags mfout(ff::CopyFlags::COPY, ff::ReuseFlags::DONTREUSE,
        ff::ReleaseFlags::DONTRELEASE);
 ...
 */
FF_OCL_STENCIL_ELEMFUNC_2D_2ENV(
    stencilf, float, h, w, r, c, const_t, float,
...
int main(int argc, char **argv) {
  ...
  //one-shot srl
  ff::ff_stencilReduceLoopOCL_2D<HelmholtzTask> helmholtz(
      jt, stencilf, reducef, 0.0);
