...
FF_OCL_STENCIL_ELEMFUNC_ENV(mapf, float, size, i, A, float, B, (void)size;
                            return A[i] * B[i];);
...
struct Map : ff_Map<Task, Task, float> {
  ...
  Task *svc(Task *in) {
    ...
    float sum = 0.0;
    ff_Map<Task, Task, float>::parallel_reduce(
        sum, 0.0F, 0, arraySize,
...
int main(int argc, char *argv[]) {
  ...
  int gpudev =
      clEnvironment::instance()->getCPUDevice(); // this exists for sure
  if (cpugpu != 0) {
    int tmp;
  ...
  dev.push_back(clEnvironment::instance()->getDevice(
      gpudev)); //convert logical to OpenCL Id
