...
const long NUM_CPU_WORKERS =
    4; // parallelism degree for the CPU map (they computes in parallel small images)
const long SMALL_SIZE =
    250000;          // we consider an image small if #cols*rows <= SMALL_SIZE
long repeatTime = 1; // to repeat the same set of images a numer of times

/* --------------- OpenCL code ------------------- */
FF_OCL_STENCIL_ELEMFUNC_ENV(
    mapf, uchar, useless, k, usrc, env_t, env, (void)useless;
    long cols = env->cols; long rows = env->rows; long y = k / cols;
    if (y == 0 || y == (cols - 1)) return 0; long x = k % cols;
    if (x == 0 || x == (cols - 1)) return 0;

    long gx = xGradient(usrc, cols, x, y);
    long gy = yGradient(usrc, cols, x, y); long sum = abs(gx) + abs(gy);
...
T *Mat2uchar(cv::Mat &in) {
  ...
  for (int i = 0; i < in.rows; ++i)
    for (int j = 0; j < in.cols; ++j) out[i * (in.cols) + j] = in.at<T>(i, j);
  return out;
}
...
static inline long xGradient(uchar *image, long cols, long x, long y) {
  return image[XY2I(y - 1, x - 1, cols)] + 2 * image[XY2I(y, x - 1, cols)] +
         image[XY2I(y + 1, x - 1, cols)] - image[XY2I(y - 1, x + 1, cols)] -
         2 * image[XY2I(y, x + 1, cols)] - image[XY2I(y + 1, x + 1, cols)];
}

// returns the gradient in the y direction
static inline long yGradient(uchar *image, long cols, long x, long y) {
  return image[XY2I(y - 1, x - 1, cols)] + 2 * image[XY2I(y - 1, x, cols)] +
         image[XY2I(y - 1, x + 1, cols)] - image[XY2I(y + 1, x - 1, cols)] -
         2 * image[XY2I(y + 1, x, cols)] - image[XY2I(y + 1, x + 1, cols)];
}
...
struct Kernel : ff_nodeSelector<Task> {
  ...
  Task *svc(Task *in) {
    ...
    printf("(%ld,%ld) %ld (us) selected=%s\n", in->env.rows, in->env.cols,
        y - x, selectedDevice ? "GPU" : "CPU");
...
struct cpuMap : ff_Map<Task, Task> {
  ...
  cpuMap(const long mapworkers = std::max(ff_realNumCores(), NUM_CPU_WORKERS))
      : ff_Map<Task, Task>(mapworkers) {} //,true) {}

  Task *svc(Task *task) {
    uchar *src = task->src, *dst = task->dst;
    const long cols = task->env.cols;
    const long rows = task->env.rows;
    ff_Map<Task, Task>::parallel_for(
        1, rows - 1,
        [src, cols, &dst](const long y) {
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    std::cerr << "use: " << argv[0]
              << " [-r num_repeat] [-g cpu-or-gpu (-1|0|1)] <image-file> "
                 "[image-file]\n";
    return -1;
  }

  int cpuorgpu =
      -1; // by default selects the device on the base of the image size
  int start = 1;
  ...
  if (cpuorgpu < 0)
    printf("selecting the device dynamically\n");
  else
    printf("executing %s\n", cpuorgpu ? "OpenCL Map" : "CPU Map");

  auto reader = [&](Task *, ff_node *const stage) -> Task * {
    for (long k = 0; k < repeatTime;
         ++k) { // repeating a number of times the same set of images
      for (long i = 0; i < num_images; ++i) {
        ...
        *src = imread(filepath, CV_LOAD_IMAGE_GRAYSCALE);
        if (!src->data) {
          std::cerr << "ERROR reading image file " << filepath
                    << " going on....\n";
          continue;
        }
