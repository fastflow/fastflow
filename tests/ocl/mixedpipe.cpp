...
struct Task : public baseOCLTask<Task, float, float> {
  ...
  void setTask(Task *t) {
    ...
    case 1: {
      ...
      setOutPtr(
          const_cast<float *>(t->A.data()), t->A.size()); // A received back
    ...
    case 2: {
      ...
      setEnvPtr(const_cast<float *>(t->C->data()), t->C->size(),
          (t->k == 0) ? CopyFlags::COPY
                      : CopyFlags::DONTCOPY); // C copied only the 1st time
      setReduceVar(&(t->sum));                // sum
    } break;                                  // A,C kept (by default)
    case 3: {
      setInPtr(const_cast<float *>(t->R->data()),
          t->R->size(), // R copied only the 1st time
...
int main(int argc, char *argv[]) {
  ...
  ff_mapReduceOCL_1D<Task> oclmap2(
      path_k2, "map2f", "reducef", 0.0, nullptr, NACC);
  ff_mapOCL_1D<Task> oclmap3(path_k3, "map3f", nullptr, NACC);
  oclmap1.saveBinaryFile();
  oclmap1.reuseBinaryFile();
  oclmap2.saveBinaryFile();
  oclmap2.reuseBinaryFile();
  oclmap3.saveBinaryFile();
  oclmap3.reuseBinaryFile();
#else
