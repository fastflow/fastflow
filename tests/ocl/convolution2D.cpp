 ...
 */
FF_OCL_STENCIL_ELEMFUNC_2D(
    mapf, int, h, w, r, c, int res = GET_IN(r, c);
...
basictype reducef_(basictype x, basictype y) { return x + y; }
...
unsigned int check(basictype *M, basictype r, int size) {
  ...
  for (unsigned int k = 0; k < niters; ++k) {
    ...
    in = M_out;
    M_out = tmp;
    for (int i = 0; i < size; ++i) M_out[i] = mapf_(in, i, NULL, NULL);
    //reduce
    red = 0;
    for (int i = 0; i < size; ++i) red = reducef_(red, M_out[i]);
  }
  for (int i = 0; i < size; ++i)
    if (M[i] != M_out[i]) {
      ndiff++;
...
struct oclTask : public baseOCLTask_2D<oclTask, basictype> {
  oclTask() : M_in(NULL), M_out(NULL), result(0), size(0) {}
  ...
  bool iterCondition(const Tin &x, size_t iter) { return iter < niters; }
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (argc == 2) {
      ...
      if (argv[1] != std::string("-h"))
        ...
        std::cout << "Usage: arraysize accelerators niters (ex. " << size << " "
                  << nacc << " " << niters << ")" << std::endl;
        return 0;
      }
    }
    if (argc > 2) nacc = atol(argv[2]);
    if (argc > 3) niters = atol(argv[3]);
  }
  ...
  ff_stencilReduceLoopOCL_2D<oclTask> oclStencilReduceOneShot(
      oclt, mapf, reducef, 0, nullptr, nacc, 1, 1);
