...
FF_OCL_STENCIL_ELEMFUNC_ENV(
    map1f, float, useless, i, in, int, k_, (void)useless; const int k = *k_;
    return (float)((k + 1) + i););
FF_OCL_STENCIL_ELEMFUNC_ENV(
    map2f, float, useless, i, A, float, B, (void)useless; return A[i] * B[i];);
FF_OCL_STENCIL_ELEMFUNC_2ENV(
    map3f, float, useless, i, R, float, A, float, sum_, (void)useless;
    const float sum = *sum_; return R[i] + 1 / (A[i] + sum););
...
struct Task : public baseOCLTask<Task, float> {
  ...
  Task(const size_t size, size_t k)
      : A(size), sum(0.0), arraySize(size), k(k), kernelId(0), C(nullptr),
        R(nullptr) {}
  ...
  void setTask(Task *t) {
    ...
    case 1: {
      ...
      setOutPtr(const_cast<float *>(t->A.data()), t->A.size(),
          CopyFlags::DONTCOPY); // A not received back
    } break;                    // A kept
    case 2: {
      setInPtr(const_cast<float *>(t->A.data()), t->A.size(),
          CopyFlags::DONTCOPY,
          ReuseFlags::REUSE); // A not copied, reusing the previous buffer
      setEnvPtr(const_cast<float *>(t->C->data()), t->C->size(),
          CopyFlags::COPY, ReuseFlags::DONTREUSE,
          ReleaseFlags::RELEASE); // C copied and then released
      setReduceVar(&(t->sum));    // sum
    } break;                      // A kept (by default)
    case 3: {
      setInPtr(const_cast<float *>(t->R->data()), t->R->size(),
          (t->k == 0) ? CopyFlags::COPY
                      : CopyFlags::DONTCOPY); // R copied only the 1st time
      setEnvPtr(const_cast<float *>(t->A.data()), t->A.size(),
          CopyFlags::DONTCOPY,
          ReuseFlags::REUSE);  // A not copied, reusing the previous buffer (*)
      setEnvPtr(&(t->sum), 1); // sum
      setOutPtr(
          const_cast<float *>(t->R->data()), t->R->size()); // R get back result
...
struct Kernel : ff_nodeSelector<Task> {
  ...
  Kernel(const size_t kernelId, std::string command, const size_t NVECTORS,
      const size_t arraySize, std::vector<float> &C, std::vector<float> &R)
      : kernelId(kernelId), command(command), NVECTORS(NVECTORS),
        arraySize(arraySize), C(&C), R(&R) {}

  Task *svc(Task *in) {
    int selectedDevice =
        getDeviceId(); // select the device id from the command string
...
struct Map_kernel1 : ff_Map<Task, Task, float> {
  Task *svc(Task *task) {
    ...
    /* ------------ use code ------------- */
    for (size_t i = 0; i < arraySize; ++i) A[i] = (k + 1) + i;
    /* ----------------------------------- */
...
struct Map_kernel2 : ff_Map<Task, Task, float> {
  Task *svc(Task *task) {
    ...
    /* ------------ use code ------------- */
    sum = 0;
    for (size_t i = 0; i < arraySize; ++i) sum += A[i] * C[i];
    /* ----------------------------------- */
...
struct Map_kernel3 : ff_Map<Task, Task, float> {
  Task *svc(Task *task) {
    ...
    /* ------------ use code ------------- */
    for (size_t i = 0; i < arraySize; ++i) R[i] += 1 / (A[i] + sum);
    /* ----------------------------------- */
...
int main(int argc, char *argv[]) {
  ...
  if (command != "0:0:0" && command != "1:1:1") {
    std::cerr << "Wrong command, for this test the only valid commands are "
                 "0:0:0, 1:1:1\n";
    return -1;
  }
  ...
  ff_mapOCL_1D<Task> oclmap1(map1f, &allocator, NACC);
  SET_DEVICE_TYPE(oclmap1);
  ff_mapReduceOCL_1D<Task> oclmap2(map2f, reducef, 0.0, &allocator, NACC);
  SET_DEVICE_TYPE(oclmap2);
  ff_mapOCL_1D<Task> oclmap3(map3f, &allocator, NACC);
...
#if defined(CHECK)
  {
    ...
    for (size_t k = 0; k < NVECTORS; ++k) {
      ...
      for (size_t i = 0; i < arraySize; ++i) _R[i] += 1 / (A[i] + sum);
    }
