...
// in place
FF_OCL_STENCIL_ELEMFUNC_ENV(mapfenv, float, size, k, M, env_t, env, (void)size;
...
struct Task {
  Task(float *M, long coeff, long n_elems) : M(M), env(n_elems, coeff){};
  float *M;
...
#if defined(CHECK)
  for (size_t i = 0; i < size; ++i) printf("%g ", M[i]);
  printf("\n");
#endif
