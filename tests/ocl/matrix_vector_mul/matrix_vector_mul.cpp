 ...
 * 
 */
struct oclTask: public baseOCLTask<oclTask, float, float, float> {
    oclTask() {}
    ...
    void setTask(const oclTask *t) { 
        assert(t);
        setInPtr(t->V);  setSizeIn(t->cols);  // input 
        setOutPtr(t->R); setSizeOut(t->rows); // output
        const long N = t->rows * t->cols;      
        setEnvPtr1(t->M); setSizeEnv1(N);     // env1
        setCopyEnv1(t->copy);                 // Does the previous env1 have to be retained ?
    }
