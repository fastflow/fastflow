...
public:
  Node0(const unsigned size, const std::string &name,
      const std::string &address, zmqTransport *const transp)
      : cnt(0), size(size), name(name), address(address), transp(transp) {}
...
protected:
  static void callback(void *e, void *) { delete[]((char *)e); }

public:
  Node1(const unsigned size, const std::string &name,
      const std::string &address, zmqTransport *const transp)
...
public:
  ...
  Node2(const unsigned size, const std::string &name,
      const std::string &address, zmqTransport *const transp)
      : size(size), name(name), address(address), transp(transp) {}
...
public:
  ...
  Node3(const unsigned size, const std::string &name,
      const std::string &address, zmqTransport *const transp)
      : size(size), name(name), address(address), transp(transp) {}
...
int main(int argc, char *argv[]) {
  if (argc != 5) {
    std::cerr << "use: " << argv[0]
              << " 1|0 producer-host:port1 consumer-host:port2 size-in-bytes\n";
    return -1;
  }
