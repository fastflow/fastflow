...
struct ff_task_t {
  ff_task_t(double *t, unsigned numRows) : numRows(numRows), task(t) {}
  unsigned numRows;
  double *task;
};
...
uSWSR_Ptr_Buffer RECYCLE(
    1024); // NOTE: See also the farm_farm test for another solution to the same problem !

class Collector : public ff_dnode<COMM2> {
  typedef COMM2::TransportImpl transport_t;

public:
  Collector(unsigned nTasks, unsigned nHosts, const std::string &name,
  ...
  virtual void unmarshalling(
    ...
    for (int i = 0; i < vlen; ++i) {
      ...
      memcpy(myt + (i * t->numRows), t->task,
          t->numRows * matSize * sizeof(double));
    }
    task = myt;
  }
...
public:
  ...
  void prepare(svector<iovec> &v, void *ptr, const int sender) {
    ...
    ff_task_t *t =
        new ff_task_t((double *)ptr + (start * matSize), numRows + 1);
...
public:
  Worker(const std::string &name, const std::string &address,
...
public:
  ...
  int svc_init() {
    ...
    // longer in use by the transport layer
    ff_dnode<COMM2>::init(
        name, address, 1, transp, SENDER, transp->getProcId(), callback);
    return 0;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 7) {
    std::cerr << "use: " << argv[0]
              << " matsize stream-length 1|0 nhosts masterhost:port1 "
                 "masterhost:port2\n";
    std::cerr << "  1 for the master 0 for other hosts\n";
    std::cerr << "  nhosts: is the number of hosts for the master and the "
                 "hostID for other hosts\n";
    std::cerr
        << "  masterhost: is the hostname or IP address of the master node.\n";
