...
class U : public ff_dnode<COMM1> {};

class GW : public ff_dinout<COMM1, COMM2> {
protected:
...
public:
  ...
  GW(const std::string &name1, const std::string &address1,
      const std::string &name2, const std::string &address2,
      transport_t *const transp)
      : name1(name1), address1(address1), name2(name2), address2(address2),
        transp(transp) {}
  ...
  public:
    ...
    void svc_end() { printf("InOut1 ending\n"); }
  ...
  int main(int argc, char *argv[]) {
    if (argc != 6) {
      std::cerr << "use: " << argv[0]
                << " name1 name2 1|0 host0:port host1:port\n";
      return -1;
    }
