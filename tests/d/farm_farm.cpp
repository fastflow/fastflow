...
public:
  ...
  void svc_end() { printf("Collector: computed %d tasks\n", cnt); }
...
public:
  Worker(const int nw, const std::string &name, const std::string &address,
      transport_t *const transp)
  ...
  int svc_init() {
    ...
    assert(c);
    for (unsigned j = 0; j < (taskSize * taskSize); ++j) c[j] = j * 1.0;

#if !defined(SINGLE_FARM)
    ff_dnode<COMM2>::init(name, address, 1, transp, SENDER,
        transp->getProcId() * nw + get_my_id());
#endif
...
int main(int argc, char *argv[]) {
  ...
  for (unsigned i = 0; i < nw; ++i)
    w.push_back(new Worker(nw, "B", address2, NULL));
  ...
  if (atoi(P)) {
    ...
    std::vector<ff_node *> w;
    for (unsigned i = 0; i < nw; ++i)
      w.push_back(new Worker(nw, "B", address2, &transport));
    farm.add_workers(w); // add all workers to the farm
