...
public:
  StartStop(unsigned nTasks, const std::string &name,
  ...
  int svc_init() {
    ...
    ff_dnode<COMM3>::init(
        name, address, 1, transp, RECEIVER, transp->getProcId());
  ...
  void svc_end() { printf("StartStop: computed %d tasks\n", cnt); }
  ...
  virtual void unmarshalling(
      svector<msg_t *> *const v[], const int vlen, void *&task) {
...
protected:
  static void callback(void *e, void *arg) { delete[](double *) e; }

public:
  Emitter(unsigned nTasks, unsigned nHosts, const std::string &name,
      const std::string &address, transport_t *const transp)
      : nTasks(nTasks), nHosts(nHosts), name(name), address(address),
        transp(transp) {}
  ...
  int svc_init() {
    ...
    ff_dnode<COMM1>::init(
        name, address, nHosts, transp, SENDER, transp->getProcId(), callback);
...
public:
  Emitter2(const std::string &name, const std::string &address,
...
public:
  Collector2(const std::string &name, const std::string &address,
      transport_t *const transp)
      : name(name), address(address), transp(transp) {}
  ...
  int svc_init() {
    ...
    ff_dnode<COMM2>::init(
        name, address, 1, transp, SENDER, transp->getProcId());
    return 0;
  }
  ...
  void prepare(svector<iovec> &v, void *ptr, const int sender = -1) {
    struct iovec iov = {
        ((ff_task_t *)ptr)->getData(), taskSize * taskSize * sizeof(double)};
    v.push_back(iov);
    setCallbackArg(ptr);
  }
...
public:
  Collector(const std::string &name, unsigned nHosts,
      const std::string &address, transport_t *const transp)
  ...
  int svc_init() {
    ...
    ff_dnode<COMM2>::init(
        name, address, nHosts, transp, RECEIVER, transp->getProcId());
    return 0;
  }
...
public:
  Sender(const std::string &name, const std::string &address,
      transport_t *const transp)
      : name(name), address(address), transp(transp) {}
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 2 && (atoi(argv[1]) < 0)) {
    printf("                                                  Farm0            "
           ...
           "                                                                   "
           "                                            \n"
           "   COMM1, the server address is master:port1 (the address1 "
           "parameter)                                          \n"
           "   COMM2, the server address is master:port2 (the address2 "
           "parameter)                                          \n"
           "   COMM3, the server address is master:port3 (the address3 "
           "parameter)                                          \n");
    return 0;
  }
  ...
  if (argc < 9) {
    std::cerr << "\n";
    std::cerr << "use: " << argv[0]
              << " tasksize stream-length hostId nfarms nw host:port1 "
                 "host:port2 host:port3\n";
    std::cerr << "  hostId: the host identifier, for the farms the hostId(s) "
                 "are in the range [0..N[\n";
    std::cerr
        << "          -1 is the id of the first stage (First in the picture)\n";
    std::cerr
        << "          -2 is the id of the last stage (Last in the picture)\n";
    std::cerr << "  nfarms: number of farm hosts\n";
    std::cerr << "  nw: number of farm's worker\n\n";
    std::cerr << " To print the application picture use " << argv[0]
              << " -1\n\n\n";
    return -1;
  }
