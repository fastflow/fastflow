...
public:
  Node0(const std::string &name, const std::string &address,
  ...
  void *svc(void *task) {
    if (task == NULL) {
      for (unsigned long i = 1; i <= ntasks; ++i) ff_send_out((void *)i);
      return GO_ON;
    }
  ...
  void unmarshalling(svector<msg_t *> *const v[], const int vlen, void *&ptr) {
    assert(
        vlen == 1 && v[0]->size() == 1); // in this example we have just 1 msg
...
protected:
  static void callback(void *e, void *) { delete ((long *)e); }

public:
  Node1(const std::string &name, const std::string &address,
      zmq1_1::TransportImpl *const transp)
...
public:
  ...
  Node2(const std::string &name, const std::string &address,
      zmq1_1::TransportImpl *const transp)
      : name(name), address(address), transp(transp) {}
  ...
  void *svc(void *task) { return (new long(*(long *)task)); }
...
public:
  Node3(const std::string &name, const std::string &address,
      zmq1_1::TransportImpl *const transp)
      : name(name), address(address), transp(transp) {}
...
int main(int argc, char *argv[]) {
  if (argc != 6) {
    std::cerr << "use: " << argv[0]
              << " name1 name2 1|0 host1:port1 host2:port2\n";
    return -1;
  }
