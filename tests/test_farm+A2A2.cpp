...
int main() {
  // #Workers == #L-Workers A2A1, standard Workers
  {
    ...
    for (int i = 0; i < nworkers; ++i) W.push_back(new WorkerStandard);
  ...
  // #Workers == #L-Workers A2A1, multi-output Workers
  {
    ...
    Starter start;
    std::vector<ff_node *> W;
    for (int i = 0; i < nworkers; ++i) W.push_back(new WorkerMO);
    ff_farm farm(W, &start);
    farm.remove_collector();
    farm.cleanup_workers();
  ...
  // #Workers != #L-Workers A2A1, multi-output Workers and multi-input L-Workers of the A2A1
  {
    ...
    Starter start;
    std::vector<ff_node *> W;
    for (int i = 0; i < nworkers1; ++i) W.push_back(new WorkerMO);
    ff_farm farm(W, &start);
    farm.remove_collector();
    farm.cleanup_workers();
