...
#ifdef FF_INDEX_INTERLEAVED
#include <numa.h> // fastflow
#endif
/**
...
public:
	...
	void free() {
		if(p_ != NULL) {
			if(freeable_) {
#ifdef FF_INDEX_INTERLEAVED
			  numa_free(&(*p_), sizeof(T) * sz_); // fastflow
#else
			  delete[] p_;
#endif 
...
public:
		...
		pages_(cat)
	{
		for(size_t i = 0; i < ((bytes+pagesz-1)/pagesz); i++) {
			//std::cout << "page insrt " << new uint8_t[pagesz] << std::endl;

			pages_.push_back(new uint8_t[pagesz]);
			gMemTally.add(cat, pagesz);
			assert(pages_.back() != NULL);
		}
	//	std::cout << "***** pages_.size after alloc " << pages_.size() << std::endl;
		assert(repOk());
	}
	...
	uint8_t * alloc() {
		assert(repOk());
		//std::cout <<  " cur_ " << cur_ << " pages_.size " << pages_.size() << std::endl;
...
//#ifndef NDEBUG
	 ...
	 */
	bool repOk() const {
	//	std::cout << "repok pages_.size " << pages_.size() << std::endl;
...
public:
	...
	RedBlack(uint32_t pageSz, int cat = 0) :
		perPage_(pageSz/sizeof(TNode)), pages_(cat) {
		clear();
	...
	{
//		if(p.repOk())
//			std::cout << "pool repok" << std::endl;
//		else
//			std::cout << "pool not repok" << std::endl;
		// Look for key; if it's not there, get its parent
		//std::cout << "pages_size in TNode* add " << pages_.size() << std::endl;
		TNode* cur = root_;
...
//				std::cout << "exit here\n";
			...
			assert(cur != parent);
			// Initialize new node
//			std::cout << "init new node\n";
			cur->key = key;
			cur->left = cur->right = NULL;
			cur->red = true; // red until proven black
			keys_++;
//			std::cout << "init new node, tot keys " << keys_ << std::endl;
			if(added != NULL) *added = true;
//			std::cout << "added new node? " << *added << std::endl;
	...
	bool addNode(Pool& p, TNode*& node) {
		...
		if(pages_.size() == 0) {
			if(addPage(p) == NULL) {
				node = NULL;
//				std::cout << "node null, addPage(p) == NULL\n";
				return false;
			}
			assert_eq(1, pages_.size());
		}
...
//				std::cout << "node null, curPage_ == pages_.size()-1 && addPage(p) == NULL\n";
				return false;
			}
			cur_ = 0;
			curPage_++;
...
//		std::cout << "cerca di allocare il nodo\n";
