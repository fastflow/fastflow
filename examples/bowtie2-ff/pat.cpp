	...
	bool fixName)
{
	...
	//printf("call PatternSource::nextReadPair\n");
...
bool WrappedPatternSourcePerThread::nextReadPair(
	...
	bool fixName)
{
	//printf("WrappedPatternSourcePerThread::nextReadPair\n");
...
//			std::cout << "while true\n";
	...
	//std::cout << "install read origbuf " <<fb_.lastN() << "\n";
	r.readOrigBuf.install(fb_.lastN(), fb_.lastNLen());
	fb_.resetLastN();
	...
	// Set up a default name if one hasn't been set
	if(r.name.empty()) {
		//std::cout << "install read name" <<fb_.lastN() << "\n";
		char cbuf[20];
		itoa10<TReadId>(readCnt_, cbuf);
		r.name.install(cbuf);
	}
...
//	elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;      // sec to ms
//	elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;   // us to ms
//	printf("time elapsed in FastqPatternSource::read %f rdid %d readcnt %d\n", elapsedTime, rdid, readCnt_);
	return success;
}
