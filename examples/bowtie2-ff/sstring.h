...
public:
	...
	SStringExpandable<T,S>& operator=(const std::basic_string<T>& o) {
		//std::cout <<"SStringExpandable op=2\n";
		install(o.c_str(), o.length());
		return *this;
	}
...
//			std::cout << "time elapsed to install a read " << elapsedTime << std::endl;
	}
...
//		elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;      // sec to ms
//		elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;   // us to ms
//		std::cout << "time elapsed in expandNoCopy " << elapsedTime << " old size " << olds  <<  " size sz " << sz << std::endl;
	}
...
public:
	...
	explicit SStringFixed(const T* b, size_t sz) {
		//std::cout <<  "SStringFixed from an array and size\n";
		install(b, sz);
	}
	...
	explicit SStringFixed(const T* b) {
		//std::cout <<  "SStringFixed from a zero-term string\n";
		install(b, strlen(b));
	}
	 ...
	 */
	virtual void install(const T* b, size_t sz) {
		//std::cout <<  "Copy 'sz' bytes from buffer 'b' into this string.\n";
		assert_leq(sz, S);
		memcpy(cs_, b, sz * sizeof(T));
		len_ = sz;
	}
	 ...
	 */
	void install(const T* b) {
		//std::cout <<  "Copy all bytes from zero-terminated buffer 'b' into this string.\n";
		install(b, strlen(b)); }
...
public:
		...
		SStringFixed<char, S>()
	{
		if(chars) {
			...
			//std::cout <<  "Create an SStringFixed from a zero-terminated string.\n";
			install(b, strlen(b));
		}
	}
...
public:
		...
		SStringExpandable<char, S, M>()
	{
		if(chars) {
			...
			//std::cout <<  "SDnaStringExpandable1\n";
			install(str);
		}
	}
		...
		SStringExpandable<char, S, M>()
	{
		if(chars) {
			...
			//std::cout <<  "SDnaStringExpandable2\n";
			install(b, sz);
		}
	}
	...
	explicit SDnaStringExpandable(
		...
		SStringExpandable<char, S, M>()
	{
		//std::cout <<  "SDnaStringExpandable3\n";
	 ...
	 */
	virtual void install(const char* b, size_t sz) {
		//std::cout <<  " install(const char* b, size_t sz).\n";
		if(this->sz_ < sz) this->expandCopy((sz + S) * M);
		memcpy(this->cs_, b, sz);
#ifndef NDEBUG
		for(size_t i = 0; i < sz; i++) {
