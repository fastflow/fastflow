...
#ifdef FF_INDEX_INTERLEAVED
			  // fastflow - begin
			  printf("Using interleaved memory allocation (size %d)\n",eh->_ebwtTotLen);
			  void * p;
			  if (NULL==(p = numa_alloc_interleaved(sizeof(uint8_t)*eh->_ebwtTotLen)))
			    cerr << "Error: numa_alloc_interleaved - allocation failed\n";
			  // fastflow - end
			  _ebwt.init(new (p) uint8_t[eh->_ebwtTotLen], eh->_ebwtTotLen, true);
#else
