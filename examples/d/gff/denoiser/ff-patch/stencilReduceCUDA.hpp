...
/*
 *
 *  This is a patch for stencilReduceCUDA.hpp in ff.
 *  At rev509, ff does not work with CUDA 8.
 *
 *  Author:
 *    Maurizio Drocco
 */
...
protected:
    ...
    inline void *svc(void *task) {
        ...
        if(cudaMemcpyAsync(in_buffer, inPtr, Task.getBytesizeIn(),
                        cudaMemcpyHostToDevice, stream) != cudaSuccess)
            std::cerr << "Problem in, code = " << cudaGetLastError() <<" \n";
        ...
        if(isPureMap()) {
            ...
            if(isPureReduce()) {
                ...
                if(cudaStreamSynchronize(stream) != cudaSuccess)
                    std::cerr << "Problem sync, code = " << cudaGetLastError() <<" \n";
                ...
                do {
                    ...
                    //CUDA Map
                    mapCUDAKernel<TkernelMap, Tin, Tout, Tenv1, Tenv2, Tenv3, Tenv4, Tenv5, Tenv6><<<blockcnt, thxblock, 0>>>(*kernelMap, Task.getInDevicePtr(), Task.getOutDevicePtr(), Task.getEnv1DevicePtr(), Task.getEnv2DevicePtr(), Task.getEnv3DevicePtr(), Task.getEnv4DevicePtr(), Task.getEnv5DevicePtr(), Task.getEnv6DevicePtr(), size);
