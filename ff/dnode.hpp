  ...
  inline bool internal_push(void *ptr, CI &comm) {
    ...
    if (CI::MULTIPUT) {
      ...
      for (int i = 0; i < peers; ++i) {
        ...
        msg_t msg(v[v.size() - 1].iov_base, v[v.size() - 1].iov_len, freeMsg,
            callbackArg[v.size() - 1]);
        if (!comm.put(msg, i)) return false;
      }
    } else {
      svector<iovec> v;
...
#ifdef _WIN32
    svector<msg_t *> **v =
        (svector<msg_t *> **)malloc(sendingPeers * sizeof(svector<msg_t *> *));
#else // C99
...
public:
  /**
     ...
     */
  int init(const std::string &name, const std::string &address, const int peers,
      typename CommImpl::TransportImpl *const transp, const bool p,
      const int nodeId = -1, dnode_cbk_t cbk = 0) {
    if (!p && cbk) {
      error("dnode:init: WARNING: callback does not make sense for consumer "
            "end-point, ignoring it...\n");
      cbk = NULL;
    }
    skipdnode = false;
    P = p;
    ...
    com.setDescriptor(
        new typename CommImpl::descriptor(name, peers, transp, P));
  ...
  virtual void prepare(
      svector<msg_t *> *&v, size_t len, const int sender = -1) {
...
public:
  ...
  int initIn(const std::string &name, const std::string &address,
    ...
    ff_dnode<CommImplIn>::com.setDescriptor(new typename CommImplIn::descriptor(
        name, peers, transp, ff_dnode<CommImplIn>::RECEIVER));
  ...
  int initOut(const std::string &name, const std::string &address,
    ...
    comOut.setDescriptor(new typename CommImplOut::descriptor(
        name, peers, transp, ff_dnode<CommImplOut>::SENDER));
