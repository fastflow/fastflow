...
protected:
  ...
  struct ofarm_lb : ff_loadbalancer {
    ...
    inline bool schedule_task(
        void *task, unsigned long retry, unsigned long ticks) {
    ...
    bool ff_send_out_to(
        void *task, int id, unsigned long retry, unsigned long ticks) {
      if (victim == (size_t)id)
        return ff_loadbalancer::ff_send_out_to(task, id, retry, ticks);
      return false;
    }
  };
...
protected:
  ...
  inline int prepare() {
    ...
    if (lb->masterworker()) {
      bool is_parallel_bb = workers[0]->isAll2All() || workers[0]->isFarm() ||
                            workers[0]->isPipe();
      lb->parallel_workers = is_parallel_bb;
      for (size_t i = 1; i < workers.size(); ++i) {
        bool tmp = workers[i]->isAll2All() || workers[i]->isFarm() ||
                   workers[i]->isPipe();
        if (tmp != is_parallel_bb) {
          error("FARM, prepare, farm in master-worker configuration but with "
                "non homogeneous workers\n");
          return -1;
        }
      }
    }
    ...
    if (ordered) {
      ...
      if (workers[0]->isFarm() || workers[0]->isPipe() ||
          workers[0]->isMultiInput() || workers[0]->isMultiOutput() ||
          workers[0]->isAll2All() || workers[0]->isComp()) {
        error("FARM: ordered farm is currently supported only for standard "
              "node!\n");
        return -1;
      }
      ...
      if (ondemand) {
        ...
        assert(_lb);
        assert(_gt);
        ordering_Memory.resize(
            nworkers * (2 * ff_farm::ondemand_buffer() + DEF_IN_OUT_DIFF + 3) +
            ordering_memsize);
    ...
    for (size_t i = 0; i < nworkers; ++i) {
      ...
      if (a2a_first) {
        ...
        if (a2a_first->create_input_buffer(
                (int)(ondemand ? ondemand : (in_buffer_entries / nworkers + 1)),
                (ondemand ? true : fixedsize)) < 0)
          return -1;
        ...
        if (workers[i]->create_input_buffer(
                (int)(ondemand ? ondemand : (in_buffer_entries / nworkers + 1)),
                (ondemand ? true : fixedsize)) < 0)
          return -1;
      ...
      if (a2a_last) {
        ...
        if (collector && !collector_removed) {
          if (!lb->masterworker()) {
            ...
            a2a_last->create_output_buffer(
                (int)(out_buffer_entries / nworkers + DEF_IN_OUT_DIFF),
            ...
            for (size_t i = 0; i < W2.size(); ++i) {
              ...
              assert(w.size() > 0);
              for (size_t j = 0; j < w.size(); ++j) gt->register_worker(w[j]);
            }
          } else {
          ...
          if (outputNodes.size()) {
            ...
            // NOTE: the following call might fail because the buffers were already created for example by
            // the pipeline that contains this stage
            if (a2a_last->create_output_buffer(
                    (int)(out_buffer_entries / nworkers + DEF_IN_OUT_DIFF),
      ...
      auto create_feedback_buffers = [&]() {
        ...
        for (size_t j = 0; j < w.size(); ++j) {
          ff_buffernode *t =
              new ff_buffernode(out_buffer_entries, false, idx++);
          assert(t);
        ...
        if (workers[i]->get_out_buffer() == NULL) {
          if (workers[i]->isMultiOutput()) { // the worker is multi-output
            ...
            if (w.size() > 0) {
              ...
              for (size_t j = 0; j < w.size(); ++j) {
                ff_node *t =
                    new ff_buffernode(out_buffer_entries, fixedsize, idx++);
                assert(t);
                internalSupportNodes.push_back(t);
            ...
            if (workers[i]->create_output_buffer(
                    (int)(out_buffer_entries / nworkers + DEF_IN_OUT_DIFF),
                    (lb->masterworker() ? false : fixedsize)) < 0)
              return -1;
          ...
          if (workers[i]->isMultiOutput()) {
            ...
            if (outputNodes.size()) {
              ...
              if (outputNodesFeedback.size()) {
                assert(!lb->masterworker()); // no master-worker
                assert(outputNodesFeedback.size() ==
                       workers.size()); // same cardinality
                workers[i]->set_output_buffer(
                    outputNodesFeedback[i]->get_in_buffer());
              } else {
                if (lb->masterworker()) {
                  create_feedback_buffers();
                }
    ...
    if (lb->masterworker()) {
      ...
      for (size_t j = 0; j < nworkers; ++j) {
        ...
        if (last_multioutput) {
          ...
          if (w.size() == 0) workers[j]->get_out_nodes(w);
        } else
          workers[j]->get_out_nodes(w);
        assert(w.size() > 0);
  ...
  virtual inline bool init_output_blocking(
      pthread_mutex_t *&m, pthread_cond_t *&c, bool /*feedback*/ = true) {
    if (collector && !collector_removed) {
      if (collector == (ff_node *)gt) return gt->init_output_blocking(m, c);
...
public:
  ...
  /*
     ...
     */
  ff_farm(const std::vector<ff_node *> &W, ff_node *const Emitter = NULL,
      ff_node *const Collector = NULL, bool input_ch = false)
      : has_input_channel(input_ch), collector_removed(false), ordered(false),
        fixedsize(FF_FIXED_SIZE), myownlb(true), myowngt(true),
        worker_cleanup(false), emitter_cleanup(false), collector_cleanup(false),
        ondemand(0), in_buffer_entries(DEF_IN_BUFF_ENTRIES),
        out_buffer_entries(DEF_OUT_BUFF_ENTRIES),
        max_nworkers(DEF_MAX_NUM_WORKERS), ordering_memsize(0), emitter(NULL),
        collector(NULL), lb(new lb_t(max_nworkers)), gt(new gt_t(max_nworkers)),
        workers(W.size()) {
  ...
  /*
     ...
     * This is the farm constructor.
     */
  ff_farm(const std::vector<ff_node *> &W, ff_node &E, ff_node &C)
      : ff_farm(W, &E, &C, false) {}
  ...
  explicit ff_farm(bool input_ch = false,
      ...
      bool worker_cleanup =
          false, // NOTE: by default no cleanup at exit is done !
      size_t max_num_workers = DEF_MAX_NUM_WORKERS,
      bool fixedsize = FF_FIXED_SIZE)
      : has_input_channel(input_ch), collector_removed(false), ordered(false),
        fixedsize(fixedsize), myownlb(true), myowngt(true),
        worker_cleanup(worker_cleanup), emitter_cleanup(false),
        collector_cleanup(false), ondemand(0),
        in_buffer_entries(in_buffer_entries),
        out_buffer_entries(out_buffer_entries), max_nworkers(max_num_workers),
        ordering_memsize(0), emitter(NULL), collector(NULL),
  ...
  ff_farm(const ff_farm &f) : ff_node(f) {
    ...
    ondemand = f.ondemand;
    in_buffer_entries = f.in_buffer_entries;
  ...
  /* move constructor */
  ff_farm(ff_farm &&f)
      : ff_node(std::move(f)), workers(std::move(f.workers)),
        internalSupportNodes(std::move(f.internalSupportNodes)) {
  ...
  virtual ~ff_farm() {
    if (emitter_cleanup) {
      if (lb && myownlb && lb->get_filter())
        delete lb->get_filter();
      else if (emitter)
        delete emitter;
    }
    if (collector_cleanup) {
      if (gt && myowngt && gt->get_filter())
        delete gt->get_filter();
      else if (collector != (ff_node *)gt)
        delete collector;
    }
    if (lb && myownlb) {
      delete lb;
  ...
  int add_emitter(T *e) {
    ...
    if (e->isFarm() || e->isAll2All() || e->isPipe()) {
      error("FARM, add_emitter: wrong king of node, the Emitter filter can be "
            "either a standard node or a multi-output node\n");
      return -1;
    }
    if (emitter) {
  ...
  int add_workers(const std::vector<ff_node *> &w) {
    if ((workers.size() + w.size()) > max_nworkers) {
      error(
          "FARM, try to add too many workers, please increase max_nworkers\n");
      return -1;
    }
    if (w.size() == 0) {
  ...
  int add_collector(ff_node *c, bool cleanup = false) {
    if (c && (c->isFarm() || c->isAll2All() || c->isPipe())) {
      error("FARM, add_collector: wrong kind of node, the Collector filter can "
            "be either a standard node or a multi-input node or a multi-output "
            "node\n");
      return -1;
    }
  ...
  int wrap_around() {
    ...
    if (has_input_channel) {
      error("FARM: wrap_around: cannot create feedback if accelerator mode is "
            "set, and the collector is present!\n");
      return -1;
    }
  ...
  /**
     ...
     *
     */
  void cleanup_workers(bool onoff = true) { worker_cleanup = onoff; }
  void cleanup_emitter(bool onoff = true) { emitter_cleanup = onoff; }
  void cleanup_collector(bool onoff = true) { collector_cleanup = onoff; }
  ...
  virtual void no_barrier() { initial_barrier = false; }
  ...
  /**
     ...
     */
  inline bool offload(void *task, unsigned long retry = ((unsigned long)-1),
  ...
  inline void get_out_nodes(svector<ff_node *> &w) {
    ...
    for (size_t i = 0; i < workers.size(); ++i) workers[i]->get_out_nodes(wtmp);
    if (wtmp.size() == 0)
      w += workers;
    else
      w += wtmp;
  ...
  inline void get_in_nodes(svector<ff_node *> &w) { w.push_back(this); }
...
protected:
  ...
  /**
     ...
     */
  inline int set_input(const svector<ff_node *> &w) { return lb->set_input(w); }

  inline int set_input(ff_node *node) { return lb->set_input(node); }
  ...
  inline int set_output(const svector<ff_node *> &w) {
    if (collector && !collector_removed) {
      ...
      error(
          "FARM, cannot add output nodes, the collector is not multi-output\n");
      return -1;
    }
...
public:
  ...
  explicit ff_Farm(FUNC_t F, ssize_t nw, bool input_ch = false)
      : ff_farm(input_ch, DEF_IN_BUFF_ENTRIES, DEF_OUT_BUFF_ENTRIES, true, nw) {
  ...
  bool load_result(OUT_t *&task, unsigned long retry = ((unsigned long)-1),
      unsigned long ticks = ff_gatherer::TICKS2WAIT) {
...
public:
  ...
  explicit ff_OFarm(FUNC_t F, size_t nw, bool input_ch = false)
      : ff_farm(
            input_ch, DEF_IN_BUFF_ENTRIES, DEF_OUT_BUFF_ENTRIES, false, nw) {
    if (Workers.size() > 0) {
