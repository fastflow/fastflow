...
public:
  ...
  inline bool pop(void **data) {
    ...
    do {
      ...
      long diff = seq - (pr + 1);
      if (diff == 0) { // CAS
        if (pread.compare_exchange_weak(
                pr, (pr + 1), std::memory_order_relaxed))
          break;
...
private:
  union {
    std::atomic<unsigned long>
        pwrite; /// Pointer to the location where to write to
    char padding1[CACHE_LINE_SIZE];
  };
  union {
    std::atomic<unsigned long>
        pread; /// Pointer to the location where to read from
...
public:
  ...
  inline bool init(size_t size) {
    ...
    buf = (element_t *)getAlignedMemory(
        longxCacheLine * sizeof(long), size * sizeof(element_t));
    if (!buf) return false;
    for (size_t i = 0; i < size; ++i) {
      buf[i].data = NULL;
  ...
  inline bool push(void *const data) {
    ...
    do {
      ...
      if (pw == seq) {
        if (abstraction_cas((volatile atom_t *)&pwrite, (atom_t)(pw + 1),
                (atom_t)pw) == (atom_t)pw)
          break;
  ...
  inline bool pop(void **data) {
    ...
    do {
      ...
      long diff = seq - (pr + 1);
      if (diff == 0) {
        if (abstraction_cas((volatile atom_t *)&pread, (atom_t)(pr + 1),
                (atom_t)pr) == (atom_t)pr)
          break;
...
public:
  ...
  /**
     * \brief init
     */
  inline bool init(unsigned long nqueues = DEFAULT_NUM_QUEUES,
      size_t size = DEFAULT_uSPSC_SIZE) {
    if (nqueues < 2) nqueues = 2;
    if (!isPowerOf2(nqueues)) nqueues = nextPowerOf2(nqueues);
    mask = nqueues - 1;

    buf = (data_element_t *)getAlignedMemory(
        longxCacheLine * sizeof(long), nqueues * sizeof(data_element_t));
    seqP = (sequenceP_t *)getAlignedMemory(
        longxCacheLine * sizeof(long), nqueues * sizeof(sequenceP_t));
    seqC = (sequenceP_t *)getAlignedMemory(
        longxCacheLine * sizeof(long), nqueues * sizeof(sequenceC_t));
  ...
  inline bool push(void *const data) {
    ...
    do {
      ...
      if (pw == seq) {
        if (abstraction_cas((volatile atom_t *)&preadP, (atom_t)(pw + 1),
                (atom_t)pw) == (atom_t)pw)
          break;
  ...
  inline bool pop(void **data) {
    ...
    do {
      ...
      if (pr == (unsigned long)seq) {
        if (atomic_long_read(&seqP[idx]) <= (unsigned long)seq)
          return false; // queue
        if (abstraction_cas((volatile atom_t *)&preadC, (atom_t)(pr + 1),
                (atom_t)pr) == (atom_t)pr)
          break;
...
private:
  ...
  struct Pointer {
    ...
    inline bool operator!() { return (ptr[MSQUEUE_PTR] == 0); }
    ...
    inline Pointer &getNodeNext() { return ptr[MSQUEUE_PTR]->next; }
    ...
    inline void set(Node &node) { ptr[MSQUEUE_PTR] = &node; }
  ...
  struct Node {
    ...
    Node(void *data) : data(data) { next.ptr[MSQUEUE_PTR] = 0; }
...
private:
  inline void allocnode(Pointer &p, void *data) {
    ...
    if (delayedAllocator->posix_memalign(
            (void **)&pn.p2, ALIGN_DOUBLE_POINTER, sizeof(Node)) != 0) {
      abort();
    }
    new (pn.p2) Node(data);
    p.set(*pn.p1);
    ...
    do {
      ...
      if (tailptr == tail) {
        if (!next) { // tail was pointing to the last node
          done = (CAS((volatile atom_t *)(tailptr.getNodeNext()), (atom_t)node,
    ...
    do {
      ...
      if (head == headptr) {
        if (headptr.getNode() == tailptr.getNode()) {
          ...
          *data = next.getData();
          done = (CAS((volatile atom_t *)head, (atom_t)next, (atom_t)headptr) ==
                  (atom_t)headptr);
        }
      }
    } while (!done);
...
public:
  ...
  inline bool init(unsigned long nqueues = DEFAULT_NUM_QUEUES,
    ...
    buf = (uSWSR_Ptr_Buffer **)getAlignedMemory(
        CACHE_LINE_SIZE, nqueues * sizeof(uSWSR_Ptr_Buffer *));
    PLock = (CLHSpinLock *)getAlignedMemory(
        CACHE_LINE_SIZE, nqueues * sizeof(CLHSpinLock));
    CLock = (CLHSpinLock *)getAlignedMemory(
        CACHE_LINE_SIZE, nqueues * sizeof(CLHSpinLock));
...
public:
  ...
  inline bool pop(void **data) {
    ...
    do {
      ...
      if (CAS((volatile atom_t *)&dequeue, (atom_t)(q + 1), (atom_t)q) ==
          (atom_t)q)
        break;
...
public:
  multiMSqueue(
      size_t poolsize = scalableMPMCqueue<MSqueue>::DEFAULT_POOL_SIZE) {
