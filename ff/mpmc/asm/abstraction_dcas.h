...
INLINE unsigned char abstraction_dcas(
    volatile atom_t *destination, atom_t *exchange, atom_t *compare) {
  ...
  cas_result = _InterlockedCompareExchange128((volatile __int64 *)destination,
      (__int64)*(exchange + 1), (__int64)*exchange, (__int64 *)compare);
...
INLINE unsigned char abstraction_dcas(
  ...
  *(__int64 *)compare =
      _InterlockedCompareExchange64((volatile __int64 *)destination,
          *(__int64 *)exchange, *(__int64 *)compare);

  return (
      (unsigned char)(*(__int64 *)compare == *(__int64 *)&original_compare));
}
...
INLINE unsigned char abstraction_dcas(
  ...
  __asm__ __volatile__("lock;"          // make cmpxchg16b atomic
                       "cmpxchg16b %0;" // cmpxchg16b sets ZF on success
                       "setz       %3;" // if ZF set, set cas_result to 1

                       // output
                       : "+m"(*(volatile atom_t(*)[2])destination),
                       "+a"(*compare), "+d"(*(compare + 1)), "=q"(cas_result)
...
INLINE unsigned char abstraction_dcas(
  ...
  __asm__ __volatile__("lock;"         // make cmpxchg8b atomic
...
INLINE unsigned char abstraction_dcas(
  ...
  __asm__ __volatile__("xchg %%rsi, %%rbx;" // swap RBI and RBX
...
INLINE unsigned char abstraction_dcas(
  ...
  __asm__ __volatile__("xchg %%esi, %%ebx;" // swap EBI and EBX
...
/****************************************************************************/
#elif ((defined __i686__ || defined __arm__ || defined __aarch64__) &&         \
...
static INLINE unsigned char abstraction_dcas(
    volatile atom_t *destination, atom_t *exchange, atom_t *compare) {
  unsigned char cas_result = 0;

  unsigned long long int original_destination;
  ...
  original_destination = __sync_val_compare_and_swap(
      (volatile unsigned long long int *)destination,
      *(unsigned long long int *)compare, *(unsigned long long int *)exchange);
  __asm__ __volatile__("" : : : "memory");
...
INLINE atom_t abstraction_cas(
    volatile atom_t *destination, atom_t exchange, atom_t compare) {
  assert(destination != NULL);
  // TRD : exchange can be any value in its range
  // TRD : compare can be any value in its range

  return ((atom_t)_InterlockedCompareExchangePointer(
      (void *volatile *)destination, (void *)exchange, (void *)compare));
}
#define _ACAS_
#endif

/****************************************************************************/
#if (!defined __arm__ && !defined __aarch64__ && __GNUC__ >= 4 &&              \
...
#if ((defined __arm__ || defined __aarch64__) && __GNUC__ >= 4 &&              \
     __GNUC_MINOR__ >= 1 && __GNUC_PATCHLEVEL__ >= 0)
static INLINE atom_t abstraction_cas(
    volatile atom_t *destination, atom_t exchange, atom_t compare) {
  atom_t rv;
