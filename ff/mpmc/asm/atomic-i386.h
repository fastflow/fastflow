 ...
 * other cases.
 */
static __inline__ unsigned long atomic_sub_and_test(
    unsigned long i, atomic_t *v) {
  unsigned char c;
 ...
 */
static __inline__ unsigned long atomic_add_negative(
    unsigned long i, atomic_t *v) {
  unsigned char c;
 ...
 */
static __inline__ unsigned long atomic_add_return(
    unsigned long i, atomic_t *v) {
  int __i;
#ifdef CONFIG_M386
  unsigned long flags;
  if (unlikely(boot_cpu_data.x86 == 3)) goto no_xadd;
#endif
...
static __inline__ unsigned long atomic_sub_return(
    unsigned long i, atomic_t *v) {
...
static __inline__ unsigned long cmpxchg(
 ...
 */
static inline unsigned long atomic_add_unless(
    atomic_t *v, unsigned long a, unsigned long u) {
  unsigned long c, old;
  c = atomic_read(v);
  for (;;) {
    if (unlikely(c == (u))) break;
    old = atomic_cmpxchg((v), c, c + (a));
    if (likely(old == c)) break;
    c = old;
  }
  return c != (u);
}
