...
struct ffApply {
  ...
  static inline auto apply(F &&f, T &&t, A &&... a) -> decltype(
      ffApply<N - 1>::apply(::std::forward<F>(f), ::std::forward<T>(t),
...
inline auto ffapply(F &&f, T &&t) -> decltype(
    ffApply<::std::tuple_size<typename ::std::decay<T>::type>::value>::apply(
        ::std::forward<F>(f), ::std::forward<T>(t))) {
  return ffApply<::std::tuple_size<typename ::std::decay<T>::type>::value>::
...
private:
  typedef std::priority_queue<hash_task_t *, std::vector<hash_task_t *>,
      compare_t>
      priority_queue_t;
...
protected:
  ...
  inline void setDep(hash_task_t *t, unsigned long dep) {
    if (t->unblock_numb == t->unblock_act_numb) {
      ...
      t->unblock_task_ids = (unsigned long *)TASK_REALLOC(
          t->unblock_task_ids, t->unblock_act_numb * sizeof(unsigned long));
    }
    t->unblock_task_ids[t->unblock_numb] = dep;
    t->unblock_numb++;
  }

  inline hash_task_t *createTask(
      unsigned long id, task_status_t status, base_f_t *wtask) {
    ...
    t->is_dummy = false;
    t->unblock_task_ids =
        (unsigned long *)TASK_MALLOC(UNBLOCK_SIZE * sizeof(unsigned long));
    t->unblock_act_numb = UNBLOCK_SIZE;
    t->num_out = 0;
    return t;
  }
  ...
  inline hash_task_t *insertTask(
    ...
    for (auto p : msg->P) {
      ...
      if (dir == INPUT) {
        ...
        unsigned long t_id =
            (unsigned long)icl_hash_find(address_set, (void *)d);
        hash_task_t *t = NULL;
        ...
        if (t != NULL) {
          if (t->unblock_numb > 0) {
            ...
            for (long ii = 0; ii < t->unblock_numb; ii++) {
              hash_task_t *t2 = (hash_task_t *)icl_hash_find(
                  task_set, &t->unblock_task_ids[ii]);
              if (t2 != NULL && t2 != act_task && t2->status != DONE) {
                if (t2->unblock_numb == t2->unblock_act_numb) {
                  t2->unblock_act_numb += UNBLOCK_SIZE;
                  t2->unblock_task_ids =
                      (unsigned long *)TASK_REALLOC(t2->unblock_task_ids,
                          t2->unblock_act_numb * sizeof(unsigned long));
                }
          ...
          t->num_out--;
          if (t->status == DONE && t->num_out == 0) task_hash_delete(t);
        }
  ...
  inline void handleTask(hash_task_t *t, int workerid) {
    for (long i = 0; i < t->unblock_numb; i++) {
      hash_task_t *tmp =
          (hash_task_t *)icl_hash_find(task_set, &t->unblock_task_ids[i]);
      assert(tmp);
      tmp->remaining_dep--;
...
public:
  TaskFScheduler(ff_loadbalancer *lb, const int maxnw)
      : lb(lb), ffalloc(NULL), runningworkers(0), address_set(NULL),
        task_set(NULL), ready_queues(maxnw),
        nscheduled(maxnw) /* ,taskscheduled(maxnw) */ {
#if !defined(DONT_USE_FFALLOC)
    ...
    LOWER_TH = (std::max)(
        1024, TASK_PER_WORKER * maxnw); //FIX: check for deadlock problems !!!
  ...
  virtual int svc_init() {
    ...
    task_set =
        icl_hash_create(UPPER_TH * 8, ulong_hash_function, ulong_key_compare);
    address_set =
        icl_hash_create(0x01 << 12, address_hash_function, address_key_compare);
