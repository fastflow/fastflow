...
public:
  ...
  baseOCLTask()
      : inPtr(NULL), outPtr(NULL), reduceVar(NULL), size_in(0), size_out(0),
        iter(0), tuple_in(std::make_tuple(true, false, false)),
  ...
  void setInPtr(Tin *_inPtr, size_t sizeIn,
    ...
    tuple_in = std::make_tuple(copy == CopyFlags::COPY,
        reuse == ReuseFlags::REUSE, release == ReleaseFlags::RELEASE);
  }
  ...
  void setEnvPtr(const ptrT *_envPtr, size_t size,
    ...
    copyEnv.push_back(std::make_tuple(sizeof(ptrT), copy == CopyFlags::COPY,
        reuse == ReuseFlags::REUSE, release == ReleaseFlags::RELEASE));
  }
  ...
  void setEnvPtr(const ptrT *_envPtr, size_t size, const MemoryFlags &flags) {
    ...
    copyEnv.push_back(std::make_tuple(sizeof(ptrT),
        flags.copy == CopyFlags::COPY, flags.reuse == ReuseFlags::REUSE,
        flags.release == ReleaseFlags::RELEASE));
  }
  ...
  bool iterCondition_aux() { return iterCondition(*reduceVar, iter); }
...
public:
  ...
  ff_oclAccelerator(ff_oclallocator *alloc, const size_t width_,
      const Tout &identityVal, const bool from_source = false)
      : from_source(from_source), my_own_allocator(false), allocator(alloc),
        halo_half(width_), identityVal(identityVal), events_h2d(16),
        deviceId(NULL) {
    ...
    lenOutput = offset1_out = halo_out_left = halo_out_right =
        lenOutput_global = 0;
  ...
  int init(cl_device_id dId, reduceMode m, const std::string &kernel_code,
      const std::string &kernel_name1, const std::string &kernel_name2,
      const bool save_binary, const bool reuse_binary) {
#ifdef FF_OPENCL_LOG
    ...
    //set OCL objects
    deviceId = dId;
    const oclParameter *param =
        clEnvironment::instance()->getParameter(deviceId);
    assert(param);
    context = param->context;
    cmd_queue = param->commandQueue;
    //build OCL kernels
    cl_int status = buildKernels(kernel_code, kernel_name1, kernel_name2,
  ...
  void releaseEnv(size_t idx, const void *envPtr) {
    if (allocator->releaseBuffer(envPtr, context, envBuffer[idx].first) !=
        CL_SUCCESS)
  ...
  void setSizingHeuristics() {
    ...
    status = clGetDeviceInfo(deviceId, CL_DEVICE_MAX_WORK_GROUP_SIZE,
        sizeof(max_device_wgsize), &max_device_wgsize, NULL);
    checkResult(status, "clGetDeviceInfo (map)");
    if (kernel_map) { //map kernel
      //get kernel-dependent max wg size
      size_t max_kernel_wgsize;
      status = clGetKernelWorkGroupInfo(kernel_map, deviceId,
          CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &max_kernel_wgsize, 0);
    ...
    if (kernel_reduce) { //reduce kernel
      //get kernel-dependent max wg size
      size_t max_kernel_wgsize;
      status = clGetKernelWorkGroupInfo(kernel_reduce, deviceId,
          CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &max_kernel_wgsize, 0);
      checkResult(status, "GetKernelWorkGroupInfo (reduce)");
      wgsize_reduce_max =
          std::min<size_t>(max_device_wgsize, max_kernel_wgsize);
      //get size of the atomic scheduling unit (analogous to CUDA wrap size)
      //typical values are 16 or 32
      size_t wg_multiple;
      status = clGetKernelWorkGroupInfo(kernel_reduce, deviceId,
      ...
      wgsize_reduce_static =
          std::min<size_t>(wgsize_reduce_static, wgsize_reduce_max);
    }
#ifdef FF_OPENCL_LOG
  ...
  void adjustInputBufferOffset(const Tin *newPtr, const Tin *oldPtr,
      std::pair<size_t, size_t> &P, size_t len_global) {
    offset1_in = P.first;
    lenInput = P.second;
    lenInput_global = len_global;
    halo_in_left = (std::min)(halo_half, offset1_in);
    halo_in_right =
        (std::min)(halo_half, lenInput_global - lenInput - offset1_in);
  ...
  void relocateInputBuffer(
      const Tin *inPtr, const bool reuseIn, const Tout *reducePtr) {
    cl_int status;

    if (reuseIn) {
      inputBuffer = allocator->createBufferUnique(
          inPtr, context, CL_MEM_READ_WRITE, sizeInput_padded, &status);
      ...
      inputBuffer = allocator->createBuffer(
          inPtr, context, CL_MEM_READ_WRITE, sizeInput_padded, &status);
    ...
    if (kernel_map) {
      ...
      if (lenInput < lenOutput) {
        ...
        nthreads_map =
            wgsize_map * ((lenOutput + wgsize_map - 1) / wgsize_map); //round up
      } else {
        wgsize_map = std::min<size_t>(lenInput, wgsize_map_static);
        nthreads_map =
            wgsize_map * ((lenInput + wgsize_map - 1) / wgsize_map); //round up
      }
#ifdef FF_OPENCL_LOG
      std::cerr << "[virtual accelerator @" << this << "]\n";
  ...
  void adjustOutputBufferOffset(const Tout *newPtr, const Tout *oldPtr,
      std::pair<size_t, size_t> &P, size_t len_global) {
    offset1_out = P.first;
    lenOutput = P.second;
    ...
    halo_out_right =
        (std::min)(halo_half, lenOutput_global - lenOutput - offset1_out);
    sizeOutput = lenOutput * sizeof(Tout);
    sizeOutput_padded =
        sizeOutput + (halo_out_left + halo_out_right) * sizeof(Tout);
  ...
  void relocateOutputBuffer(const Tout *outPtr, const Tout *reducePtr) {
    ...
    outputBuffer = allocator->createBuffer(
        outPtr, context, CL_MEM_READ_WRITE, sizeOutput_padded, &status);
  ...
  void relocateEnvBuffer(const void *envptr, const bool reuseEnv,
      const size_t idx, const size_t envbytesize) {
    cl_int status = CL_SUCCESS;

    if (idx >= envBuffer.size()) {
      cl_mem envb;
      if (reuseEnv)
        envb = allocator->createBufferUnique(
            envptr, context, CL_MEM_READ_WRITE, envbytesize, &status);
      else
        envb = allocator->createBuffer(
            envptr, context, CL_MEM_READ_WRITE, envbytesize, &status);
  ...
  virtual void setMapKernelArgs(const size_t envSize) {
    ...
    cl_int status =
        clSetKernelArg(kernel_map, idx++, sizeof(cl_mem), &inputBuffer);
    checkResult(status, "setKernelArg input");
  ...
  size_t asyncH2Denv(const size_t idx, char *p) {
    if (nevents_h2d >= events_h2d.size()) events_h2d.reserve(nevents_h2d);
    cl_int status =
        clEnqueueWriteBuffer(cmd_queue, envBuffer[idx].first, CL_FALSE, 0,
  ...
  size_t asyncH2Dborders(Tout *p) {
    if (halo_out_left) {
      cl_int status = clEnqueueWriteBuffer(cmd_queue, outputBuffer, CL_FALSE, 0,
          halo_out_left * sizeof(Tout), p + offset1_out - halo_out_left, 0,
          NULL, &events_h2d[nevents_h2d++]);
    ...
    if (halo_out_right) {
      cl_int status = clEnqueueWriteBuffer(cmd_queue, outputBuffer, CL_FALSE,
          (halo_out_left + lenOutput) * sizeof(Tout),
          halo_out_right * sizeof(Tout), // NOTE: in a loop Tin == Tout !!
  ...
  size_t asyncD2Houtput(Tout *p) {
    cl_int status = clEnqueueReadBuffer(cmd_queue, outputBuffer, CL_FALSE,
        halo_out_left * sizeof(Tout), sizeOutput, p + offset1_out, 0, NULL,
        &event_d2h);
  ...
  size_t asyncD2Hborders(Tout *p) {
    cl_int status = clEnqueueReadBuffer(cmd_queue, outputBuffer, CL_FALSE,
        halo_out_left * sizeof(Tout), halo_half * sizeof(Tout), p + offset1_out,
        0, NULL, &events_h2d[0]);
    checkResult(status, "copying border1 back from device");
    ++nevents_h2d;
    status = clEnqueueReadBuffer(cmd_queue, outputBuffer, CL_FALSE,
        (halo_out_left + lenOutput - halo_half) * sizeof(Tout),
        halo_half * sizeof(Tout), p + offset1_out + lenOutput - halo_half, 0,
        NULL, &event_d2h);
  ...
  void initReduce() {
    ...
    status |=
        clSetKernelArg(kernel_reduce, idx++, sizeof(cl_mem), &reduceBuffer);
    status |=
        clSetKernelArg(kernel_reduce, idx++, sizeof(cl_uint), (void *)&len);
    status |= clSetKernelArg(kernel_reduce, idx++, wg_red_mem, NULL);
    status |= clSetKernelArg(
        kernel_reduce, idx++, sizeof(Tout), (void *)&identityVal);
  ...
  void asyncExecReduceKernel1() {
    ...
    cl_int status = clEnqueueNDRangeKernel(cmd_queue, kernel_reduce, 1, NULL,
        ...
        (nevents_map == 0) ? NULL : &event_map, &event_reduce1);
  ...
  void asyncExecReduceKernel2() {
    ...
    status |=
        clSetKernelArg(kernel_reduce, idx++, sizeof(cl_mem), &reduceBuffer);
    status |= clSetKernelArg(
        kernel_reduce, idx++, sizeof(cl_uint), (void *)&nwg_reduce);
    status |= clSetKernelArg(kernel_reduce, idx++, wg_red_mem, NULL);
    status |= clSetKernelArg(
        kernel_reduce, idx++, sizeof(Tout), (void *)&identityVal);
    ...
    status = clEnqueueNDRangeKernel(cmd_queue, kernel_reduce, 1, NULL,
        &wgsize_reduce, &wgsize_reduce, 1, &event_reduce1, &event_reduce2);
  ...
  /**
	 ...
	 */
  void setHaloHalf(const size_t h) { halo_half = h; }

private:
  int buildProgram(cl_device_id dId) {
    cl_int status = clBuildProgram(
        program, 1, &dId, /*"-cl-fast-relaxed-math"*/ NULL, NULL, NULL);
    ...
    if (status != CL_SUCCESS) {
      ...
      clGetProgramBuildInfo(
          program, dId, CL_PROGRAM_BUILD_LOG, len * sizeof(char), buffer, NULL);
      printf("LOG: %s\n\n", buffer);
...
#endif
    program =
        clCreateProgramWithSource(context, 1, &code, &sourceSize, &status);
    if (!program) {
  ...
  int createProgram(const std::string &filepath, cl_device_id dId,
      const bool save_binary, const bool reuse_binary) {
    ...
    if (binary) {
      program = clCreateProgramWithBinary(context, 1, &dId, &bufsize,
          reinterpret_cast<const unsigned char **>(&bufptr), &binaryStatus,
          &status);
    }
    if (status != CL_SUCCESS) { // maybe is not a binary file
      program =
          clCreateProgramWithSource(context, 1, &bufptr, &bufsize, &status);
      if (!program) {
        checkResult(status, "creating program with source");
        return -1;
      }
      ...
      if (save_binary) { // TODO: the logical deviceId has to be attached to the file name !
        ...
        status = clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES,
            sizeof(size_t) * 1, &programBinarySize, NULL);
        ...
        status = clGetProgramInfo(
            program, CL_PROGRAM_BINARIES, sizeof(char *) * 1, &binbufptr, NULL);
  ...
  cl_int buildKernels(const std::string &kernel_code,
    ...
    if (!from_source) { //compile kernel on device
      ...
      if (createProgram(kernel_code, deviceId, save_binary, reuse_binary) < 0)
        return -1;
    }
  ...
  void resetReduce(
      size_t lenReduceInput, size_t elem_size, const void *reducePtr) {
    ...
    reduceBuffer = allocator->createBuffer(
        reducePtr, context, CL_MEM_READ_WRITE, global_red_mem, &status);
...
protected:
  ...
  size_t
      sizeInput_padded; //byte-size of the input-portion plus left and right borders
  ...
  size_t lenOutput, offset1_out, halo_out_left, halo_out_right,
      lenOutput_global;
...
template <typename T, typename TOCL = T,
    typename accelerator_t = ff_oclAccelerator<T, TOCL>>
...
public:
  ...
  ff_stencilReduceLoopOCL_1D(
      const std::string &mapf,                      //OpenCL elemental function
      const std::string &reducef = std::string(""), //OpenCL combinator function
      const Tout &identityVal = Tout(), ff_oclallocator *allocator = nullptr,
      const size_t NACCELERATORS = 1, const int width = 1)
      : oneshot(false), saveBinary(false), reuseBinary(false),
        accelerators(NACCELERATORS), acc_in(NACCELERATORS),
        acc_out(NACCELERATORS), stencil_width_half(width), offset_dev(0),
        old_inPtr(NULL), old_outPtr(NULL), oldBytesizeIn(0), oldSizeOut(0),
        oldSizeReduce(0) {
  ...
  ff_stencilReduceLoopOCL_1D(const T &task, const std::string &mapf,
  ...
  void pickCPU() { ff_oclNode_t<T>::setDeviceType(CL_DEVICE_TYPE_CPU); }
  ...
  unsigned int getIter() { return Task.getIter(); }
      ...
      if (devices.size() == 0) { // the user didn't set any specific device
        ...
        case CL_DEVICE_TYPE_GPU: {
          ...
          std::vector<ssize_t> logdev =
              clEnvironment::instance()->coAllocateGPUDeviceRR(
                  accelerators.size(), (offset_dev == 0) ? -1 : offset_dev);
          if (logdev.size() == 0) {
            ...
            for (size_t i = 0; i < logdev.size(); ++i)
              devices.push_back(
                  clEnvironment::instance()->getDevice(logdev[i]));
            for (size_t i = 0; i < devices.size(); ++i)
              if (accelerators[i]->init(devices[i], getReduceMode(),
                      kernel_code, kernel_name1, kernel_name2, saveBinary,
                      reuseBinary) < 0)
                return -1;
            break;
          }
        ...
        case CL_DEVICE_TYPE_CPU: {
          if (accelerators.size() > 1) {
            error("Multiple (>1) virtual accelerators on CPU are requested. "
                  "Not yet implemented.\n");
            return -1;
          } else {
            ...
            devices.clear();
            devices.push_back(
                clEnvironment::instance()->getDevice( //convert to OpenCL Id
                    clEnvironment::instance()
                        ->getCPUDevice())); //retrieve logical device
        ...
        if (devices.size() > accelerators.size()) {
          error("stencilReduceOCL::nodeInit: Too many devices requested, "
        ...
        for (size_t i = 0; i < devices.size(); ++i)
          accelerators[i]->init(devices[i], getReduceMode(), kernel_code,
              kernel_name1, kernel_name2, saveBinary, reuseBinary);
      }
    }
  ...
  T *svc(T *task) {
    ...
    if ((void *)inPtr != (void *)outPtr) {
      ...
      if (oldSizeOut != Task.getBytesizeOut()) {
        ...
        for (size_t i = 0; i < accelerators.size(); ++i) {
          accelerators[i]->adjustOutputBufferOffset(outPtr,
              (memorychange ? old_outPtr : NULL), acc_out[i],
              Task.getSizeOut());
        }
      ...
      if (memorychange) {
        for (size_t i = 0; i < accelerators.size(); ++i) {
          accelerators[i]->relocateInputBuffer(
              inPtr, Task.getReuseIn(), reducePtr);
        }
    ...
    for (size_t i = 0; i < accelerators.size(); ++i)
      for (size_t k = 0; k < envSize; ++k) {
        ...
        accelerators[i]->relocateEnvBuffer(
            envptr, Task.getReuseEnv(k), k, Task.getBytesizeEnv(k));
      }
    ...
    if (isPureReduce()) {
      ...
      if (outPtr &&
          Task.getCopyOut()) { // do we have to copy back the output result ?
        for (size_t i = 0; i < accelerators.size(); ++i) {
#if defined(FF_REPARA)
...
protected:
  virtual void adjustInputBufferOffset(const bool memorychange) {
    for (size_t i = 0; i < accelerators.size(); ++i)
      accelerators[i]->adjustInputBufferOffset(Task.getInPtr(),
          (memorychange ? old_inPtr : NULL), acc_in[i], Task.getSizeIn());
  }
  ...
  void setsourcecode(const std::string &source, const std::string &kernel1,
  ...
  void compute_accmem(
      const size_t len, std::vector<std::pair<size_t, size_t>> &acc) {
    size_t start = 0,
           step = (len + accelerators.size() - 1) / accelerators.size();
    size_t i = 0;
...
public:
  ff_mapOCL_1D(std::string mapf, ff_oclallocator *alloc = nullptr,
      ...
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            mapf, "", 0, alloc, NACCELERATORS, 0) {}
  ...
  ff_mapOCL_1D(const std::string &kernels_source, const std::string &mapf_name,
      ...
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            kernels_source, mapf_name, "", 0, alloc, NACCELERATORS, 0) {}

  ff_mapOCL_1D(const T &task, std::string mapf,
      ff_oclallocator *alloc = nullptr, const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            task, mapf, "", 0, alloc, NACCELERATORS, 0) {}
  ff_mapOCL_1D(const T &task, const std::string &kernels_source,
      const std::string &mapf_name, ff_oclallocator *alloc = nullptr,
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            task, kernels_source, mapf_name, "", 0, alloc, NACCELERATORS, 0) {}
...
public:
  ...
  ff_reduceOCL_1D(std::string reducef, const Tout &identityVal = Tout(),
      ff_oclallocator *alloc = nullptr, const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            "", reducef, identityVal, alloc, NACCELERATORS, 0) {}
  ff_reduceOCL_1D(const std::string &kernels_source,
      const std::string &reducef_name, const Tout &identityVal = Tout(),
      ff_oclallocator *alloc = nullptr, const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(kernels_source, "", reducef_name,
            identityVal, alloc, NACCELERATORS, 0) {}

  ff_reduceOCL_1D(const T &task, std::string reducef,
      const Tout identityVal = Tout(), ff_oclallocator *alloc = nullptr,
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            task, "", reducef, identityVal, alloc, NACCELERATORS, 0) {}
  ff_reduceOCL_1D(const T &task, const std::string &kernels_source,
      const std::string &reducef_name, const Tout identityVal = Tout(),
      ff_oclallocator *alloc = nullptr, const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(task, kernels_source, "",
            reducef_name, identityVal, alloc, NACCELERATORS, 0) {}
...
public:
  ...
  ff_mapReduceOCL_1D(std::string mapf, std::string reducef,
      const Tout &identityVal = Tout(), ff_oclallocator *alloc = nullptr,
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            mapf, reducef, identityVal, alloc, NACCELERATORS, 0) {}

  ff_mapReduceOCL_1D(const std::string &kernels_code,
      const std::string &mapf_name, const std::string &reducef_name,
      const Tout &identityVal = Tout(), ff_oclallocator *alloc = nullptr,
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(kernels_code, mapf_name,
            reducef_name, identityVal, alloc, NACCELERATORS, 0) {}
  ...
  ff_mapReduceOCL_1D(const T &task, std::string mapf, std::string reducef,
      ...
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(
            task, mapf, reducef, identityVal, alloc, NACCELERATORS, 0) {}
  ff_mapReduceOCL_1D(const T &task, const std::string &kernels_code,
      const std::string &mapf_name, const std::string &reducef_name,
      const Tout &identityVal = Tout(), ff_oclallocator *alloc = nullptr,
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_1D<T, TOCL>(task, kernels_code, mapf_name,
            reducef_name, identityVal, alloc, NACCELERATORS, 0) {}
};
...
public:
  ...
  ff_oclAccelerator_2D(ff_oclallocator *alloc, const size_t halo_width_,
      const Tout &identityVal, const bool from_source = false)
      : ff_oclAccelerator<T, TOCL>(
            alloc, halo_width_, identityVal, from_source) {
  ...
  void setMapKernelArgs(const size_t envSize) {
    ...
    cl_int status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_mem), &this->inputBuffer);
    checkResult(status, "setKernelArg input");
    status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_mem), &this->outputBuffer);
    checkResult(status, "setKernelArg output");

    //set iteration-invariant MAP kernel args
    status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_uint), (void *)&heightInput_global);
    checkResult(status, "setKernelArg global input height");
    status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_uint), (void *)&widthInput_global);
    checkResult(status, "setKernelArg global input width");

    status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_uint), (void *)&this->lenOutput);
    checkResult(status, "setKernelArg local input length");
    status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_uint), (void *)&this->offset1_in);
    checkResult(status, "setKernelArg offset");
    status = clSetKernelArg(
        this->kernel_map, idx++, sizeof(cl_uint), (void *)&this->halo_out_left);
    checkResult(status, "setKernelArg halo");

    for (size_t k = 0; k < envSize; ++k) {
      status = clSetKernelArg(
          this->kernel_map, idx++, sizeof(cl_mem), &(this->envBuffer[k].first));
      checkResult(status, "setKernelArg env");
    }
  }
  ...
  /**
	 ...
	 */
  void setHeight(size_t h) { heightInput_global = h; }

  void setWidth(size_t w) { widthInput_global = w; }
 ...
 */
template <typename T, typename TOCL = T>
class ff_stencilReduceLoopOCL_2D : public ff_stencilReduceLoopOCL_1D<T, TOCL,
                                       ff_oclAccelerator_2D<T, TOCL>> {
private:
  typedef typename TOCL::Tin Tin;
  typedef typename TOCL::Tout Tout;
...
public:
  // build the program from the mapf and reducef functions
  ff_stencilReduceLoopOCL_2D(
      const std::string &mapf,                      //OpenCL elemental function
      const std::string &reducef = std::string(""), //OpenCL combinator function
      const Tout &identityVal = Tout(), ff_oclallocator *allocator = nullptr,
      const size_t NACCELERATORS = 1, const int stencil_width_half_ = 1,
      ...
      : //init srl 1D with no halo (1D halo depends on 2D width)
        ...
        stencil_width_half(stencil_width_half_),
        stencil_height_half(stencil_height_half_) {}
  ...
  ff_stencilReduceLoopOCL_2D(
      ...
      const int stencil_height_half_ = 1)
      : //init srl 1D with no halo (1D halo depends on 2D width)
        base_srl_t(kernels_source, mapf_name, reducef_name, identityVal,
            allocator, NACCELERATORS, 0),
        stencil_width_half(stencil_width_half_),
        stencil_height_half(stencil_height_half_) {}

  // the task is provided in the constructor -- one shot computation
  ff_stencilReduceLoopOCL_2D(const T &task, const std::string &mapf,
      const std::string &reducef = std::string(""),
      const Tout &identityVal = Tout(), ff_oclallocator *allocator = nullptr,
      const size_t NACCELERATORS = 1, const int stencil_width_half_ = 1,
      const int stencil_height_half_ = 1)
      : //init srl 1D with no halo (1D halo depends on 2D width)
        base_srl_t(
            task, mapf, reducef, identityVal, allocator, NACCELERATORS, 0),
        stencil_width_half(stencil_width_half_),
        stencil_height_half(stencil_height_half_) {}
  ...
  ff_stencilReduceLoopOCL_2D(const T &task,
      ...
      const int stencil_height_half_ = 1)
      : //init srl 1D with no halo (1D halo depends on 2D width)
        base_srl_t(task, kernels_source, mapf_name, reducef_name, identityVal,
            allocator, NACCELERATORS, 0),
        stencil_width_half(stencil_width_half_),
        stencil_height_half(stencil_height_half_) {}
...
public:
  ...
  ff_mapOCL_2D(const std::string &kernels_source, const std::string &mapf_name,
      ff_oclallocator *alloc = nullptr, const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_2D<T, TOCL>(
            kernels_source, mapf_name, "", 0, alloc, NACCELERATORS) {}

  ff_mapOCL_2D(const T &task, std::string mapf,
      ff_oclallocator *alloc = nullptr, const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_2D<T, TOCL>(
            task, mapf, "", 0, alloc, NACCELERATORS) {}
  ff_mapOCL_2D(const T &task, const std::string &kernels_source,
      const std::string &mapf_name, ff_oclallocator *alloc = nullptr,
      const size_t NACCELERATORS = 1)
      : ff_stencilReduceLoopOCL_2D<T, TOCL>(
            task, kernels_source, mapf_name, "", 0, alloc, NACCELERATORS) {}
