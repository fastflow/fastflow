...
public:
  ...
  stencilTask()
      : InTask(NULL), OutTask(NULL), Xsize(0), Ysize(0), Zsize(0), Xstart(0),
        Xstop(0), Xstep(0), Ystart(0), Ystop(0), Ystep(0), Zstart(0), Zstop(0),
        Zstep(0), Youtsize(0) {}
  stencilTask(T *t, size_t Xsize, size_t Xstart, size_t Xstop, size_t Xstep)
      : InTask(t), Xsize(Xsize), Ysize(1), Zsize(1), Xstart(Xstart),
        Xstop(Xstop), Xstep(Xstep), Ystart(0), Ystop(0), Ystep(0), Zstart(0),
        Zstop(0), Zstep(0), Youtsize(0) {}
  stencilTask(T *t, size_t Xsize, size_t Xstart, size_t Xstop, size_t Xstep,
      size_t Ysize, size_t Ystart, size_t Ystop, size_t Ystep)
      : InTask(t), Xsize(Xsize), Ysize(Ysize), Zsize(1), Xstart(Xstart),
        Xstop(Xstop), Xstep(Xstep), Ystart(Ystart), Ystop(Ystop), Ystep(Ystep),
        Zstart(0), Zstop(1), Zstep(0), Youtsize(0) {}
  stencilTask(T *t, size_t Xsize, size_t Xstart, size_t Xstop, size_t Xstep,
      size_t Ysize, size_t Ystart, size_t Ystop, size_t Ystep, size_t Zsize,
      size_t Zstart, size_t Zstop, size_t Zstep)
      : InTask(t), Xsize(Xsize), Ysize(Ysize), Zsize(Zsize), Xstart(Xstart),
        Xstop(Xstop), Xstep(Xstep), Ystart(Ystart), Ystop(Ystop), Ystep(Ystep),
        Zstart(Zstart), Zstop(Zstop), Zstep(Zstep), Youtsize(0) {}
...
private:
  ...
  typedef std::function<void(
      parloop_t &loopInit, T *M, const size_t Xsize, const size_t Ysize)>
      init2_F_t;
  typedef std::function<T(
      long i, long j, T *in, const size_t X, const size_t Y)>
      compute_F_t;
  typedef std::function<T(
      long i, long j, T *in, const size_t X, const size_t Y, T &reduceVar)>
      reduce_F_t;
  typedef std::function<void(parloop_t &loopCompute, T *in, T *out,
      ...
      reduce2_F_t;
  typedef std::function<void(
      T *inout, const size_t X, const size_t Y, T &reduceVar)>
      prepost_F_t;
...
public:
  stencil2D(T *Min, T *Mout, const size_t Xsize, const size_t Ysize,
      const size_t Youtsize, int nw, int Yradius = 1, int Xradius = 1,
      bool ghostcells = false,
      const size_t chunksize = DEFAULT_STENCIL_CHUNKSIZE)
      : oneShot(true), ghosts(ghostcells), nw(nw), Yradius(Yradius),
        Xradius(Xradius), chunkSize(chunksize), extraInitInParam(NULL),
        extraInitOutParam(NULL), initInF1(NULL), initOutF1(NULL),
        initInF2(NULL), initOutF2(NULL), beforeFor(NULL), computeF(NULL),
        computeFReduce1(NULL), computeFReduce2(NULL), afterFor(NULL),
        reduceOp(reduceOpDefault), iterCondition(NULL), identityValue((T)0),
        reduceVar((T)0), iter(0), maxIter(1) {
  ...
  stencil2D(int nw, int Yradius = 1, int Xradius = 1, bool ghostcells = false,
      const size_t chunksize = DEFAULT_STENCIL_CHUNKSIZE)
      : oneShot(false), ghosts(ghostcells), nw(nw), Yradius(Yradius),
        Xradius(Xradius), chunkSize(chunksize), extraInitInParam(NULL),
        extraInitOutParam(NULL), initInF1(NULL), initOutF1(NULL),
        initInF2(NULL), initOutF2(NULL), beforeFor(NULL), computeF(NULL),
        computeFReduce1(NULL), computeFReduce2(NULL), afterFor(NULL),
        reduceOp(reduceOpDefault), iterCondition(NULL), identityValue((T)0),
        reduceVar((T)0), iter(0), maxIter(1), ploop(nw, true) {}

  ~stencil2D() {}

  void initInFunc(init_F_t F, void *extra) {
    if (!oneShot) {
      error("stencil2D: initInFunc: the provided init function will not be "
            "called in this configuration\n");
    }
    initInF1 = F;
    extraInitInParam = extra;
  }
  ...
  void initOutFunc(init_F_t F, void *extra) {
    if (!oneShot) {
      error("stencil2D: initOutFunc: the provided init function will not be "
            "called in this configuration\n");
    }
    initOutF1 = F;
    extraInitOutParam = extra;
  }
  ...
  void computeFunc(reduce_F_t F, size_t xstart = 0, size_t xstop = 0,
      size_t xstep = 1, size_t ystart = 0, size_t ystop = 0, size_t ystep = 1,
  ...
  void computeFuncAll(reduce2_F_t F, size_t xstart = 0, size_t xstop = 0,
      size_t xstep = 1, size_t ystart = 0, size_t ystop = 0, size_t ystep = 1,
      size_t zstart = 0, size_t zstop = 0, size_t zstep = 1) {
    Task.setX(xstart, xstop ? xstop : Task.X_size(), xstep);
    Task.setY(ystart, ystop ? ystop : Task.Y_size(), ystep);
  ...
  void reduceFunc(iterCond_F_t I, size_t maxI, reduceOp_F_t R, T iV) {
    iterCondition = I;
    maxIter = maxI;
    reduceOp = R;
    identityValue = iV;
...

    {
      ...
      if (computeFReduce1) {
        do {
          ...
          ploop.parallel_reduce(
              rVar, identityValue, Xstart, Xstop, Xstep, chunkSize,
              [&](const long i, T &rVar) {
                for (long j = Ystart; j < Ystop; j += Ystep) {
                  Mout[i * Xsize + j] =
                      computeFReduce1(i, j, Min, Xsize, Ysize, rVar);
                }
              },
              reduceOp, nw);
        ...
        do {
          ...
          if (beforeFor) beforeFor(Min, Xsize, Ysize, rVar);
          computeFReduce2(ploop, Min, Mout, Xsize, Xstart, Xstop, Ysize, Ystart,
              Ystop, rVar);
          if (afterFor) afterFor(Mout, Xsize, Ysize, rVar);
