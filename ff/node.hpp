...
static void *FF_EOS_NOFREEZE =
    (void *)(ULLONG_MAX - 1); /// not automatically propagated
static void *FF_EOSW =
    (void *)(ULLONG_MAX - 2); /// propagated only by farm's stages
static void *FF_GO_ON =
    (void *)(ULLONG_MAX - 3); /// not automatically propagated
static void *FF_GO_OUT =
    (void *)(ULLONG_MAX - 4); /// not automatically propagated
  ...
  void thread_routine() {
    ...
    do {
      ...
      if (disable_cancelability()) {
        error(
            "ff_thread, thread_routine, could not change thread cancelability");
        return;
      }
...
public:
  ...
  virtual int spawn(int cpuId = -1) {
    ...
    int CPUId = -1;
    if (default_mapping) init_thread_affinity(attr, cpuId);
    if (CPUId == -2) return -2;
    ...
    int r = 0;
    if ((r = pthread_create(&th_handle, attr, proxy_thread_routine, this)) !=
        0) {
      errno = r;
  ...
  virtual void thaw(bool _freeze = false, ssize_t = -1) {
    ...
    if (_freeze)
      freezing = 2; // next time freeze again the thread
    else
      freezing = 0;
    //assert(thawed==false);
    frozen = false;
...
private:
  ...
  ssize_t CPUId;
  ssize_t neos =
      1; ///< n. of EOS the node expects to receive before terminating
  bool myoutbuffer;
  bool myinbuffer;
  bool skip1pop;
  ...
  ff_thread
      *thread; /// A \p thWorker object, which extends the \p ff_thread class
...
protected:
  virtual void set_id(ssize_t id) { myid = id; }
  ...
  virtual inline bool Push(void *ptr, unsigned long retry = ((unsigned long)-1),
  ...
  virtual inline bool Pop(void **ptr, unsigned long retry = ((unsigned long)-1),
      unsigned long ticks = (TICKS2WAIT)) {
    if (blocking_in) {
      if (!in_active) {
        *ptr = NULL;
  ...
  virtual inline void set_output_blocking(
    ...
    assert(canoverwrite || (p_cons_m == nullptr && p_cons_c == nullptr) ||
  ...
  /** 
     ...
     */
  virtual int create_input_buffer(
      int nentries, bool fixedsize = FF_FIXED_SIZE) {
    if (in) return -1;
  ...
  virtual int create_input_buffer_mp(
      int nentries, bool fixedsize = FF_FIXED_SIZE, int neos = 1) {
  ...
  /** 
     ...
     *  \return 0 if successful, -1 otherwise
     */
  virtual int create_output_buffer(
      int nentries, bool fixedsize = FF_FIXED_SIZE) {
    if (out) return -1;
...
public:
  ...
  /**
     ...
     *
     */
  inline size_t getOSThreadId() const {
    if (thread) return thread->getOSThreadId();
    return 0;
  }
  ...
  virtual void ffStats(std::ostream &out) {
    out << "ID: " << get_my_id() << "  work-time (ms): " << wttime << "\n"
        << "  n. tasks      : " << taskcnt << "\n"
        << "  svc ticks     : " << tickstot << " (min= " << ticksmin
        << " max= " << ticksmax << ")\n"
  ...
  struct rpr_measure_t {
    ...
    size_t
        problemSize; // computed if the rpr::task_size attribute is defined otherwise is 0
    size_t bytesIn, bytesOut;
    size_t vmSize, vmPeak;
    double energy;
  };

  using RPR_devices_measure =
      std::vector<std::pair<int, std::vector<rpr_measure_t>>>;
  ...
  /**
     ...
     */
  virtual RPR_measures_vector rpr_get_measures() {
...
protected:
  ...
  /** 
     ...
     */
  static inline bool ff_send_out_comp(
...
protected:
  ff_node()
      : in(0), out(0), myid(-1), CPUId(-1), myoutbuffer(false),
        myinbuffer(false), skip1pop(false), in_active(true),
        my_own_thread(true), thread(NULL), callback(NULL), barrier(NULL) {
    time_setzero(tstart);
    time_setzero(tstop);
  ...
  virtual inline void input_active(const bool onoff) {
    if (in_active != onoff) in_active = onoff;
  }
  ...
  virtual void registerAllGatherCallback(
...
private:
  ...
  public:
    thWorker(ff_node *const filter, const ssize_t input_neos = 1)
        : ff_thread(filter->barrier, filter->default_mapping), filter(filter),
          input_neos(input_neos) {}
    ...
    inline void *svc(void *) {
      ...
      do {
        ...
        filter->ticksmin = (std::min)(
            filter->ticksmin, diff); // (std::min) for win portability)
      ...
      if (filter->default_mapping) {
        ...
        if (ff_mapThreadToCpu(
                (cpuId < 0) ? (cpuId = threadMapper::instance()->getCoreId(tid))
                            : cpuId) != 0)
          error("Cannot map thread %d to CPU %d, mask is %u,  size is %u,  "
                "going on...\n",
              tid,
  ...
  inline void setThread(ff_thread *const th) {
    my_own_thread = false;
    thread = th;
  }
...
struct ff_buffernode : ff_node {
  ff_buffernode() {}
  ff_buffernode(int nentries, bool fixedsize = FF_FIXED_SIZE, int id = -1,
      int multi_producer_eos = -1) {
  ...
  void set(int nentries, bool fixedsize = FF_FIXED_SIZE, int id = -1,
      int multi_producer_eos = -1) {
    set_id(id);
    if (multi_producer_eos < 0) {
      ...
      if (create_input_buffer_mp(nentries, fixedsize, multi_producer_eos) < 0) {
        error(
            "FATAL ERROR: ff_buffernode::set: create_input_buffer_mp fails!\n");
        abort();
      }
      set_output_buffer(ff_node::get_in_buffer());
    }
  ...
  int init_blocking_stuff() {
    ...
    if (!ff_node::init_output_blocking(m, c)) {
      error("buffernode, FATAL ERROR, init input blocking mode for "
            "accelerator\n");
      return -1;
    }
  ...
  bool ff_send_out(void *ptr, unsigned long retry = ((unsigned long)-1),
      unsigned long ticks = (ff_node::TICKS2WAIT)) {
    return ff_node::ff_send_out(ptr, retry, ticks);
  }
  bool gather_task(void **task, unsigned long retry = ((unsigned long)-1),
      unsigned long ticks = (ff_node::TICKS2WAIT)) {
  ...
  template <typename T>
  bool gather_task(T *&task, unsigned long retry = ((unsigned long)-1),
      unsigned long ticks = (ff_node::TICKS2WAIT)) {
