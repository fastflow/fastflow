...
static inline ssize_t ff_numCores() {
  ...
  if (sysctlbyname("hw.logicalcpu", &nn, &len, NULL, 0) == -1) perror("sysctl");
  n = nn;
#elif defined(_WIN32)
  SYSTEM_INFO sysinfo;
  ...
  do {
    ...
    for (size_t i = 0; i < CHAR_BIT * sizeof(cpu_set_t); ++i) {
      if (CPU_ISSET(i, &cpuset)) {
        std::string str = "cat /sys/devices/system/cpu/cpu" +
                          std::to_string(i) + "/topology/thread_siblings_list";
        char buf[64];
        FILE *f;
...
static inline ssize_t ff_getMyCore() {
  ...
  thread_policy_get(mach_thread_self(), THREAD_AFFINITY_POLICY,
      (integer_t *)&mypolicy, &thread_info_count, &get_default);
...
static inline ssize_t ff_mapThreadToCpu(int cpu_id, int priority_level = 0) {
  ...
  CPU_ZERO(&mask);
  CPU_SET(cpu_id, &mask);
  if (sched_setaffinity(ff_gettid(), sizeof(mask), &mask) != 0) return EINVAL;
...
static inline size_t cache_line_size() {
  ...
  if (sysctlbyname(
          "hw.cachelinesize", &line_size, &sizeof_line_size, NULL, 0) != 0) {
