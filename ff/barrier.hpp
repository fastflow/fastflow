...
public:
  Barrier(const size_t = MAX_NUM_THREADS) : _barrier(0) {}
  ~Barrier() {
    if (_barrier > 0) pthread_barrier_destroy(&bar);
  }
  ...
  inline void doBarrier(size_t) { pthread_barrier_wait(&bar); }
...
public:
  spinBarrier(const size_t _maxNThreads = MAX_NUM_THREADS)
      : maxNThreads(_maxNThreads), _barrier(0) {
