...
INLINE void *aligned_malloc(size_t sz) {
  void *mem;
  if (posix_memalign(&mem, CACHE_LINE_SIZE, sz)) return 0;
  return mem;
}
...
public:
  ...
  char *enqueue_prepare(size_t sz) {
    ...
    size_t msg_size =
        ((uintptr_t)(sz + sizeof(void *) - 1) & ~(sizeof(void *) - 1)) +
        sizeof(void *);
  ...
  char *dequeue_prepare() {
    ...
    if (((uintptr_t)next & 1) == 0) {
      ...
      atomic_addr_store_release(
          (void *volatile *)&head_pos_, (char *)((uintptr_t)next & ~1));
...
private:
  ...
  bucket_t *alloc_bucket(size_t sz) {
    ...
    if (bucket == 0) throw std::bad_alloc();
    bucket->next = 0;
    bucket->size = sz;
    bucket_count_ += 1;
    return bucket;
  ...
  char *enqueue_prepare_slow(size_t sz) {
    ...
    bucket_t *bucket = 0;
    char *head_pos =
        (char *)atomic_addr_load_acquire((void *volatile *)&head_pos_);
    while (head_pos < last_bucket_->data ||
           head_pos >= last_bucket_->data + last_bucket_->size) {
      bucket = last_bucket_;
    ...
    if (bucket == 0) bucket = alloc_bucket(bucket_size);
    *(void *volatile *)bucket->data = (void *)1;
    atomic_addr_store_release(
        (void *volatile *)tail_pos_, (void *)((uintptr_t)bucket->data | 1));
    tail_pos_ = bucket->data;
