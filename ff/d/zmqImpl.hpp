...
#define BROADCAST_DESC(name, n, trasp, P)                                      \
  BROADCAST::descriptor(name, n, trasp, P)
#define ALLGATHER commPattern<zmqAllGather>
#define ALLGATHER_DESC(name, n, trasp, P)                                      \
  ALLGATHER::descriptor(name, n, trasp, P)
...
#define ONETOMANY_DESC(name, n, trasp, P)                                      \
  ONETOMANY::descriptor(name, n, trasp, P)
...
#define MANYTOONE_DESC(name, n, trasp, P)                                      \
  MANYTOONE::descriptor(name, n, trasp, P)
...
struct descriptor1_N {
  ...
  inline int init(const std::string &addr, const int nodeId = -1) {
    ...
    if (!P) { // if ROUTER: routing of messages to a specific connection
      ...
      identity.str("");
      identity << name << ":"
               << ((nodeId != -1) ? nodeId : transport->getProcId());
      // set durable identity. it must be set before connecting the socket
      socket->setsockopt(
          ZMQ_IDENTITY, identity.str().c_str(), identity.str().length() + 1);
  ...
  inline bool send(const msg_t &msg, int flags = 0) {
    for (int i = 0; i < peers; ++i) {
      ...
      zmq::message_t to(const_cast<char *>(transportIDs[i].c_str()),
          transportIDs[i].length() + 1, 0, 0);
      try {
        if (!zmqHdrSet)
  ...
  inline bool send(const msg_t &msg, const int dest, int flags) {
    assert(dest < peers);
    zmq::message_t to(const_cast<char *>(transportIDs[dest].c_str()),
        transportIDs[dest].length() + 1, 0, 0);
    try {
      if (!zmqHdrSet)
        if (!socket->send(to, ZMQ_SNDMORE)) return false;
  ...
  /*
     ...
     */
  inline bool recvhdr(msg_t &msg) { return recv(msg); }
  ...
  /*
     ...
     */
  inline bool sendReq(const msg_t &msg) { return sendreq(msg); }
  /*
...
public:
  ...
  typedef descriptor1_N
      descriptor; ///< descriptor used in Unicast, Bcast, Scatter,
                  ///< On-Demand, OneToMany
  ...
  inline bool putmore(const tosend_t &msg) {
    return desc->sendmore(msg, 0, ZMQ_SNDMORE);
  }
  ...
  /*
     ...
     * \return TODO
     */
  inline bool putmore(const msg_t &msg, const int) {
    return desc->send(msg, 0, ZMQ_SNDMORE);
  }
...
public:
  ...
  /*
     ...
     */
  inline bool putmore(const tosend_t &msg) {
    return desc->sendmore(msg, ZMQ_SNDMORE);
  }
...
public:
  ...
  /*
     ...
     * \return TODO
     */
  inline bool put(const msg_t &msg, const int) { return put(msg); }
  ...
  /*
     ...
     * \return TODO
     */
  inline bool putmore(const tosend_t &msg, const int) { return putmore(msg); }
  ...
  /*
     ...
     * \return TODO
     */
  inline bool gethdr(msg_t &msg, int &peer) { return desc->recvhdr(msg, peer); }
  ...
  /*
     ...
     * \return TODO
     */
  inline bool get(msg_t &msg, int &peer) { return desc->recv(msg, peer); }
...
public:
  ...
  /*
     ...
     * \return TODO
     */
  inline bool put(const msg_t &msg, const int) { return desc->send(msg, 0); }
...
public:
  ...
  /*
     ...
     * \return TODO
     */
  inline bool putmore(const tosend_t &msg) { return -1; }
  ...
  /*
     ...
     */
  inline bool put(const msg_t &msg) { return -1; }
  ...
  /*
     ...
     * \return A negative value is returned.
     */
  inline bool putmore(const msg_t &msg) { return -1; }
...
public:
  ...
  /*
     ...
     */
  zmqOnDemand()
      : desc(NULL), active(false), to(-1), requestsToWait(0),
        requestSent(false) {}
  ...
  /*
     ...
     *
     */
  zmqOnDemand(descriptor *D)
      : desc(D), active(false), to(-1), requestsToWait(0), requestSent(false) {}
  ...
  inline bool put(const tosend_t &msg) {
    ...
    if (to < 0)
      if (!desc->recvReq(to)) return false;
  ...
  inline bool put(const tosend_t &msg, const int dest, int flag = 0) {
    if (dest <
        0) { // sends the same message to all peers, usefull for termination
      for (int i = 0; i < desc->getPeers(); ++i) {
        to = i;
        tosend_t _msg;
        _msg.copy(const_cast<msg_t &>(msg));
        if (!desc->send(_msg, to, flag)) {
          to = -1;
          return false;
        }
        ++requestsToWait;
