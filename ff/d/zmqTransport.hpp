...
public:
  ...
  /*
     ...
     */
  inline zmqTransportMsg_t(
      const void *data, size_t size, msg_callback_t cb = 0, void *arg = 0)
  ...
  /*
     ...
     */
  inline void copy(zmq::message_t &msg) { zmq::message_t::copy(&msg); }
  ...
  /*
     ...
     */
  inline void *getData() { return zmq::message_t::data(); }
  ...
  /*
     ...
     */
  inline size_t size() { return zmq::message_t::size(); }
};
...
public:
  ...
  /*
     ...
     */
  zmqTransport(const int procId) : procId(procId), context(NULL){};
  ...
  int deleteEndPoint(endpoint_t *s) {
    if (s) {
      std::deque<endpoint_t *>::iterator it =
          std::find(Socks.begin(), Socks.end(), s);
      if (it != Socks.end()) {
        Socks.erase(it);
