...
protected:
  clEnvironment() : platforms(NULL), numPlatforms(0), lastAssigned(0) {
    ...
    for (unsigned int i = 0; i < numPlatforms; ++i) {
      ...
      clGetDeviceIDs(
          platforms[i], CL_DEVICE_TYPE_ALL, numDevices, deviceIds, NULL);
      ...
      for (size_t j = 0; j < numDevices; j++) {
        ...
        clGetDeviceInfo(
            deviceIds[j], CL_DEVICE_AVAILABLE, sizeof(cl_bool), &(b), NULL);
        context = clCreateContext(NULL, 1, &deviceIds[j], NULL, NULL, &status);
        clGetDeviceInfo(
            deviceIds[j], CL_DEVICE_TYPE, sizeof(cl_device_type), &(dt), NULL);
    ...
    for (std::vector<cl_device_id>::iterator iter = clDevices.begin();
         iter < clDevices.end(); ++iter) {
      ...
      oclParams->commandQueue =
          clCreateCommandQueue(oclParams->context, dId, prop, &status);
...
public:
  ...
  /**
     ...
     */
  std::vector<ssize_t> coAllocateGPUDeviceRR(size_t n = 1,
      ssize_t preferred_dev = -1, bool exclusive = false,
    ...
    size_t dev = (preferred_dev >= 0) ? (preferred_dev % clDevices.size())
                                      : lastAssigned;
    ...
    while (true) {
      ...
      for (size_t i = 0; i < clDevices.size(); i++) {
        clGetDeviceInfo(clDevices[dev], CL_DEVICE_TYPE, sizeof(cl_device_type),
            &(dt), NULL);
        if ((!clDeviceInUse[dev] |
                !exclusive)                 //dev is free or not exclusive mode
      ...
      if (!count) { // commit
        ...
        for (size_t i = 0; i < ret.size(); ++i) clDeviceInUse[ret[i]] = true;
        lastAssigned = dev;
        break;
      }
  ...
  ssize_t getCPUDevice(bool exclusive = false) {
    ...
    for (size_t i = 0; i < clDevices.size(); i++) {
      clGetDeviceInfo(
          clDevices[i], CL_DEVICE_TYPE, sizeof(cl_device_type), &(dt), NULL);
    ...
    pthread_mutex_unlock(&instanceMutex);
    if (ret == -1)
      std::cerr << "CPU not available or in exclusive use: aborting\n";
    return ret;
  }

  void releaseDevice(ssize_t id) { std::cerr << "Not yet implemented\n"; }
  ...
  std::vector<ssize_t> getAllGPUDevices() {
    ...
    for (size_t i = 0; i < clDevices.size(); i++) {
      clGetDeviceInfo(
          clDevices[i], CL_DEVICE_TYPE, sizeof(cl_device_type), &(dt), NULL);
      if ((dt)&CL_DEVICE_TYPE_GPU) ret.push_back(i);
    }
    return ret;
  }
