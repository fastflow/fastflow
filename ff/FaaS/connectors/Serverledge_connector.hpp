...
namespace connector {
    ...
    public:
        ...
        std::unique_ptr<ff::faasBuffer> invokeFaasFunction(std::shared_ptr<ff::faasBuffer> payload, std::shared_ptr<stats_entry>& stats) override {
            if (!payload) {
            ...
            char* payloadBuff = payload->getBuffer();
            ...
            auto result = simdutf::binary_to_base64(payloadBuff, payloadSize, base64_translation_buffer.data());
            if (result != base64_translation_buffer.size()) {
                std::cerr << "[Serverledge_connector] Function invocation error: Base64 encoding failed. Expected size: " << base64_translation_buffer.size() << ", got: " << result << " for function: " << *functionName << std::endl;
                return nullptr;
            }
            size_t total_size = 0;
            ...
            if(stats_collection) {
                init_stats(stats); 
                stats->Msg_dim_sent = base64_translation_buffer.size() + json_payload_starting_stats_size + json_payload_ending_size;  
            ...
            total_size = base64_translation_buffer.size() + json_payload_starting_size + json_payload_ending_size;
            
            std::shared_lock<std::shared_mutex> lock(registrationMutex, std::defer_lock);
            lock.lock(); 
            ...
            while (attempt < maxRetries) {
                ...
                try{
                    ...
                    PRINT_DBG(std::string("[Serverledge_connector] Function ") + *functionName + std::string(" invoked with message: ") + std::string(json_payload_starting) + std::string(base64_translation_buffer.data()) + std::string(json_payload_ending) + std::string("\n"));
                    ...
                    HTTPres = cli->Post(EntryPoint, headers,
                            ...
                            [&](size_t offset, size_t length, httplib::DataSink &sink) {
                                ...
                                } else if (offset < json_payload_starting_size + base64_translation_buffer.size()) {
                                    size_t relative = offset - json_payload_starting_size;
                                    size_t len = std::min(length, base64_translation_buffer.size() - relative);
                                    sink.write(base64_translation_buffer.data() + relative, len);
                                } else if (offset < total_size) {
                                    size_t relative = offset - json_payload_starting_size - base64_translation_buffer.size();
    ...
    private:
        ...
        std::vector<char> base64_translation_buffer;
