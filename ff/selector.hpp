...
protected:
  static bool ff_send_out_selector(
  ...
  void add2selectorall(const IN_t &task) { setTask(task); }
  void add2selectorall(IN_t &task) { setTask(task); }
...
public:
  ...
  void nodeEnd() {
    for (size_t i = 0; i < devices.size(); ++i) devices[i]->nodeEnd();
  }
