...
protected:
  ...
  int svc_init() {
    ...
    case CL_DEVICE_TYPE_ALL: {
      ...
      if ((GPUdevId != -1) &&
          (oclId < clEnvironment::instance()->getNumGPU())) {
      ...
      if (CPUdevId != -1) {
        printf("%d: Allocated a CPU device as either no GPU device is "
               "available or no GPU slot is available (cpuId=%ld)\n",
            oclId, CPUdevId);
    ...
    default:
      std::cerr << "Unknown/not supported device type\n";
      return -1;
    }
    return 0;
  }
...
struct ff_oclNode_t : ff_oclNode {
  ...
  ff_oclNode_t()
      : GO_ON((OUT *)FF_GO_ON), EOS((OUT *)FF_EOS), GO_OUT((OUT *)FF_GO_OUT),
