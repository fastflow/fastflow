...
struct Arg_t {
  Arg_t()
      : ptr(NULL), size(0), byvalue(false), copy(false), reuse(false),
        release(false) {}
  Arg_t(void *ptr, size_t size)
      : ptr(ptr), size(size), byvalue(true), copy(true), reuse(false),
        release(false) {}
  Arg_t(void *ptr, size_t size, bool copy, bool reuse, bool release)
      : ptr(ptr), size(size), byvalue(false), copy(copy), reuse(reuse),
        release(release) {}
...
public:
  ...
  void setInPtr(const ptrT *inPtr, size_t size,
    ...
    internal::Arg_t arg(const_cast<ptrT *>(inPtr), size * sizeof(ptrT),
        copy == CopyFlags::COPY, reuse == ReuseFlags::REUSE,
  ...
  void setOutPtr(const ptrT *_outPtr, size_t size,
    ...
    internal::Arg_t arg(const_cast<ptrT *>(_outPtr), size * sizeof(ptrT),
        copyback == CopyFlags::COPY, reuse == ReuseFlags::REUSE,
        release == ReleaseFlags::RELEASE);
...
protected:
  ...
  const std::vector<internal::Arg_t> &getOutputArgs() const {
    return tpcOutput;
  }
...
public:
  ...
  ff_tpcNode_t(ff_tpcallocator *alloc = nullptr)
      : tpcId(-1), deviceId(-1), my_own_allocator(false), oneshot(false),
        oneshotTask(nullptr), allocator(alloc) {
  ...
  ff_tpcNode_t(const IN_t &task, ff_tpcallocator *alloc = nullptr)
      : tpcId(-1), deviceId(-1), my_own_allocator(false), oneshot(true),
        oneshotTask(&task), allocator(alloc) {
  ...
  const TPC_t *getTask() const { return &Task; }
...
protected:
  int nodeInit() {
    if (tpcId < 0) {
      ...
      if (deviceId ==
          (tpc_dev_id_t)-1) { // the user didn't set any specific device
  ...
  void *svc(void *task) {
    ...
    tpc_res_t res;
    tpc_job_id_t j_id = tpc_device_acquire_job_id(
        dev_ctx, f_id, TPC_DEVICE_ACQUIRE_JOB_ID_BLOCKING);
    ...
    for (size_t i = 0; i < inV.size(); ++i) {
      ...
      if (!inV[i].byvalue) {
        ...
        if (inHandles[i].size < inV[i].size) {
          ...
          if (inHandles[i].handle) { // not first time
            ...
            allocator->releaseBuffer(
                inHandles[i].ptr, dev_ctx, inHandles[i].handle);
          }
          if (inV[i].reuse) {
            handle = allocator->createBufferUnique(
                inV[i].ptr, dev_ctx, TPC_DEVICE_ALLOC_FLAGS_NONE, inV[i].size);
          } else {
            handle = allocator->createBuffer(
                inV[i].ptr, dev_ctx, TPC_DEVICE_ALLOC_FLAGS_NONE, inV[i].size);
          }

          if (!handle) {
            error("ff_tpcNode::svc unable to allocate memory on the TPC device "
                  "(IN)");
            inHandles[i].size = 0, inHandles[i].ptr = nullptr,
            inHandles[i].handle = 0;
            return (oneshot ? NULL : GO_ON);
          }
          inHandles[i].size = inV[i].size, inHandles[i].ptr = inV[i].ptr,
          inHandles[i].handle = handle;
        }

        if (inV[i].copy) {
          res = tpc_device_copy_to(dev_ctx, inV[i].ptr, handle, inV[i].size,
              TPC_DEVICE_COPY_BLOCKING);
          if (res != TPC_SUCCESS) {
        ...
        res = tpc_device_job_set_arg(
            dev_ctx, j_id, arg_idx, sizeof(handle), &handle);

      } else { // argument passed by-value
        res = tpc_device_job_set_arg(
            dev_ctx, j_id, arg_idx, inV[i].size, inV[i].ptr);
      }
    ...
    for (size_t i = 0; i < outV.size(); ++i) {
      ...
      it = std::find_if(inHandles.begin(), inHandles.end(),
          [ptr](const handle_t &arg) { return arg.ptr == ptr; });
      ...
      if (it == inHandles.end()) {     // not found
        ...
        if (outHandles[i].size < outV[i].size) {
          ...
          if (outHandles[i].handle) { // not first time
            assert(!outV[i].reuse);
            allocator->releaseBuffer(
                outHandles[i].ptr, dev_ctx, outHandles[i].handle);
          }

          if (outV[i].reuse) {
            handle = allocator->createBufferUnique(
                ptr, dev_ctx, TPC_DEVICE_ALLOC_FLAGS_NONE, outV[i].size);
          } else {
            handle = allocator->createBuffer(
                ptr, dev_ctx, TPC_DEVICE_ALLOC_FLAGS_NONE, outV[i].size);
          }
          ...
          if (!handle) {
            error("ff_tpcNode::svc unable to allocate memory on the TPC device "
                  "(OUT)");
            outHandles[i].size = 0, outHandles[i].ptr = nullptr,
            outHandles[i].handle = 0;
            return (oneshot ? NULL : GO_ON);
          }
          outHandles[i].size = outV[i].size, outHandles[i].ptr = ptr,
          outHandles[i].handle = handle;
        }
      } else
        handle = (*it).handle; // in/out parameter
    ...
    for (size_t i = 0; i < outV.size(); ++i) {
      ...
      if (outV[i].copy) {
        res = tpc_device_copy_from(dev_ctx, handle, outV[i].ptr, outV[i].size,
            TPC_DEVICE_COPY_BLOCKING);
        if (res != TPC_SUCCESS) {
    ...
    if (ret_val && ret_val_size > 0) {
      ...
      if (res != TPC_SUCCESS) {
        error("ff_tpcNode::svc error getting back the return value of the "
              "kernel function\n");
        return (oneshot ? NULL : GO_ON);
      }
    }
    ...
    OUT_t *task_out;
    if (task)
      task_out = Task.releaseTask((IN_t *)task);
    else
      task_out = Task.releaseTask(const_cast<IN_t *>(oneshotTask));
