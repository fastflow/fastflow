#ifndef FF_DRECEIVEROD_H
#define FF_DRECEIVEROD_H
...
class ff_dreceiverOD: public ff_monode_t<message_t> { 
private:
...
public:
    ff_dreceiverOD(const int dGroup_id, ff_endpoint acceptAddr, size_t input_channels, std::map<int, int> routingTable = {std::make_pair(0,0)}, bool onDemand = false, int coreid=-1)
		: onDemand(onDemand), input_channels(input_channels), acceptAddr(acceptAddr), routingTable(routingTable),
    ...
    */
    message_t *svc(message_t* task) {
        ...
        while(neos < input_channels){
            ...
            int fixed_last = this->last_receive_fd + 1;
