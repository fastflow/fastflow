...
protected:
  ...
  public:
    GD(void (*F)(T *const), T *const args)
        : active(false), F(F), args(args), ntasks(0),
          maxMsgs(DEFAULT_OUTSTANDING_TASKS), TASKS(maxMsgs) {}
  ...
  public:
    Scheduler(ff_loadbalancer *lb, const int maxnw,
        ...
        : TaskFScheduler<task_f_t, compare_t>(lb, maxnw), task_numb(0),
    ...
    task_f_t *svc(task_f_t *task) {
      if (!task) {
        if (!gd_ended &&
            (task_numb - task_completed) < (unsigned long)baseSched::LOWER_TH)
          ff_node::input_active(
              true); // start receiveing from input channel again
        else if (schedRelaxF)
          schedRelaxF(++bk_count);
      ...
      if (baseSched::fromInput()) {
        ...
        if (msg->wtask == nullptr) {
          ...
          return ((task_numb != task_completed) ? baseSched::GO_ON
                                                : baseSched::EOS);
        }
        ++task_numb;
        insertTask(msg);
  ...
  struct ff_mdf_f_t : public base_f_t {
    ff_mdf_f_t(const F_t F, Param &... a) : F(F) {
...
public:
  ...
  ff_mdf(void (*F)(T1 *const), T1 *const args,
      size_t outstandingTasks = DEFAULT_OUTSTANDING_TASKS,
      int maxnw = ff_realNumCores(), void (*schedRelaxF)(unsigned long) = NULL)
      : ff_pipeline(false, outstandingTasks),
        farmworkers(maxnw) { //NOTE: pipe has fixed size queue by default
    GD<T1> *_gd = new GD<T1>(F, args);
    _gd->setMaxTasks(outstandingTasks +
                     16); // NOTE: TASKS must be greater than pipe's queue!
    ...
    farm->add_workers(w);
    farm->add_emitter(
        sched = new Scheduler<compare_t>(farm->getlb(), maxnw, schedRelaxF));
    farm->wrap_around();
  ...
  inline void AddTask(std::vector<param_info> &P, const F_t F, Param... args) {
    ff_mdf_f_t<F_t, Param...> *wtask =
        new ff_mdf_f_t<F_t, Param...>(F, args...);
  ...
  void setNumWorkers(ssize_t nw) {
    if (nw > ff_numCores()) // TODO: use the mapper to get the number of cores
      error(
          "ff_mdf: setNumWorkers: too much workers, setting num worker to %d\n",
          ff_numCores());
