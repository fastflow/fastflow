...
class ff_comb : public ff_minode {
  ...
  static bool devnull(void *, unsigned long, unsigned long, void *) {
    return true;
  }
...
public:
  ...
  ff_comb(ff_node *n1, ff_node *n2, bool first_cleanup = false,
  ...
  int wait() {
    if (comp_nodes[0]->isMultiInput()) return ff_minode::wait();
  ...
  /**
     ...
     */
  bool done() const {
    if (comp_nodes[0]->isMultiInput()) return ff_minode::done();
    return ff_node::done();
  }
  ...
  ff_node *getFirst() const {
    if (comp_nodes[0]->isComp()) return ((ff_comb *)comp_nodes[0])->getFirst();
    return comp_nodes[0];
  }
  // returns the last node on the right-hand side
  ff_node *getLast() const {
    if (comp_nodes[1]->isComp()) return ((ff_comb *)comp_nodes[1])->getLast();
    return comp_nodes[1];
  }
...
protected:
  ...
  inline bool check(T1 *n1, T2 *n2) {
    if (n1->isFarm() || n1->isAll2All() || n1->isPipe() || n2->isFarm() ||
        n2->isAll2All() || n2->isPipe()) {
      error("COMBINE, input nodes cannot be farm, all-2-all or pipeline "
            "building-blocks\n");
      return false;
    }
    return true;
  ...
  bool put(void *ptr) { return ff_node::put(ptr); }
  ...
  void connectCallback() {
    if (comp_nodes[0]->isComp()) ((ff_comb *)comp_nodes[0])->connectCallback();
    if (comp_nodes[1]->isComp()) ((ff_comb *)comp_nodes[1])->connectCallback();
  ...
  int prepare() {
    ...
    } else if (n2->get_out_buffer() == nullptr)
      n2->registerCallback(devnull, nullptr); // devnull callback
  ...
  void set_multiinput() {
    ...
    if (comp_nodes[0]->isComp()) return comp_nodes[0]->set_multiinput();
    comp_multi_input = true;
  }
  void set_multioutput() {
    if (comp_nodes[1]->isComp()) return comp_nodes[1]->set_multioutput();
    comp_multi_output = true;
  }

  void set_neos(ssize_t n) { getFirst()->set_neos(n); }
      ...
      for (size_t k = 0; k < n; ++k) {
        ...
        if (t == FF_EOS) {
          ...
          if (comp_nodes[1]->isMultiOutput())
            ...
            if (comp_multi_output)
              ret = FF_EOS;
            else
              comp_nodes[1]->ff_send_out(
                  FF_EOS); // if no output is present it calls devnull
          }
          break;
        }
        r2 = comp_nodes[1]->svc(t);
        if (r2 == FF_GO_ON || r2 == FF_GO_OUT || r2 == FF_EOS_NOFREEZE)
          continue;
        if (r2 == FF_EOS) {
          ret = FF_GO_OUT;
          if (comp_nodes[1]->isMultiOutput())
        ...
        comp_nodes[1]->ff_send_out(
            r2); // if no output is present it calls devnull
      }
      localdata1.clear();
    }
    return ret;
  ...
  int set_output(ff_node *n) { return comp_nodes[1]->set_output(n); }
  ...
  void eosnotify(ssize_t id = -1) {
    ...
    if (comp_nodes[0]->isMultiInput() || comp_multi_input) {
      ...
      if ((neos >= n) &&
          (ret !=
              FF_GO_OUT) /*this means that svc_comp_node1 has already called eosnotify */
      )
        comp_nodes[1]->eosnotify(id);
      return;
    }
    if (ret != FF_GO_OUT) comp_nodes[1]->eosnotify(id);
  }
  ...
  void propagateEOS(void *task = FF_EOS) {
    ...
    if (comp_nodes[1]->isMultiOutput() || comp_multi_output)
      comp_nodes[1]->propagateEOS(task);
    else
  ...
  void get_out_nodes(svector<ff_node *> &w) {
    ...
    if (len == w.size() && !comp_nodes[1]->isComp()) w.push_back(comp_nodes[1]);
  }
  void get_in_nodes(svector<ff_node *> &w) {
    size_t len = w.size();
    comp_nodes[0]->get_in_nodes(w);
    if (len == w.size() && !comp_nodes[0]->isComp()) w.push_back(comp_nodes[0]);
  }
  ...
  bool init_input_blocking(
    ...
    if (n->isMultiInput()) {
      ...
      for (size_t i = 0; i < w.size(); ++i) w[i]->set_output_blocking(m, c);
      return true;
    }
  ...
  inline bool ff_send_out_to(void *task, int,
 ...
 */
static inline const ff_pipeline combine_nodes_in_pipeline(ff_node &node1,
    ff_node &node2, bool cleanup1 = false, bool cleanup2 = false) {
  if (node1.isAll2All() || node2.isAll2All()) {
    error("combine_nodes_in_pipeline, cannot be used if one of the nodes is "
          "A2A\n");
    return ff_pipeline();
  }
  if (node1.isOFarm()) {
    ...
    error("combine_nodes_in_pipeline, FEATURE NOT YET SUPPORTED (node1 ordered "
          "farm and node1 standard or combine node\n");
    abort(); // TODO
    return ff_pipeline();
  }
  if (!node1.isFarm() && !node2.isFarm()) { // two sequential nodes
    ...
    if (!e)
      farm->add_emitter(&node1);
    else {
      ff_comb *p =
          new ff_comb(&node1, e, cleanup1, farm->isset_cleanup_emitter());
    ...
    if (!c)
      farm->add_collector(&node2, cleanup2);
    else {
      ff_comb *p =
          new ff_comb(c, &node2, farm->isset_cleanup_collector(), cleanup2);
  ...
  ff_pipeline pipe;
  if (c) {
    ff_comb *p = new ff_comb(
        c, e, farm1->isset_cleanup_collector(), farm2->isset_cleanup_emitter());
...
static inline const ff_farm combine_farms_a2a(ff_farm &farm1, ff_farm &farm2) {
  ...
  if (farm2.getEmitter() != nullptr) {
    error("ff_comb, second farm has a non-default emitter, use: "
          "combine_farm(farm1, emitter2, farm2)\n");
    return newfarm;
  }
...
static inline const ff_farm combine_farms_a2a(
    ff_farm &farm1, const E_t &node, ff_farm &farm2) {
  ff_farm newfarm;
  ...
  for (size_t i = 0; i < W1.size(); ++i) {
    if (!node.isMultiOutput()) {
      ...
      assert(mo);
      auto pc = new ff_comb(W1[i], mo, farm1.isset_cleanup_workers(), true);
      assert(pc);
      Wtmp[i] = pc;
    } else {
      ...
      //auto pc = new decltype(c)(c);
      auto e = new E_t(node);
      auto pc = new ff_comb(W1[i], e, farm1.isset_cleanup_workers(), true);
      assert(pc);
      Wtmp[i] = pc;
    }
  }
...
static inline const ff_farm combine_farms_nf(ff_farm &farm1, ff_farm &farm2) {
  ...
  if (farm1.getNWorkers() != farm2.getNWorkers()) {
    error("combine_farms_nf, cannot combine farms with different number of "
          "workers\n");
    return newfarm;
  }
  if (farm1.isOFarm() ^ farm2.isOFarm()) {
    error("combine_farms_nf, if one of the two farms is ordered both must be "
          "ordered\n");
    return newfarm;
  }
  ...
  if (farm1.isOFarm() && farm2.isOFarm()) {
    if (farm1.ondemand_buffer() != farm2.ondemand_buffer()) {
      error("combine_farms_nf, cannot combine ordered farms with different "
            "ondemand buffer\n");
      return newfarm;
    }
  ...
  if (w1[0]->isMultiOutput() ||
      w2[0]->isMultiInput()) { // NOTE: we suppose homogeneous workers
    error("combine_farms_nf, cannot combine farms whose workers are either "
          "multi-output or multi-input nodes\n");
    return newfarm;
  }
  if (w1[0]->isPipe() ||
      w2[0]->isPipe()) { // NOTE: we suppose homogeneous workers
    error("combine_farms_nf, cannot combine farms whose workers are pipeline "
          "nodes\n");
    return newfarm;
  }
  std::vector<ff_node *> W1(w1.size());
  ...
  for (size_t i = 0; i < W1.size(); ++i) {
    ...
    //auto pc = new decltype(c)(c);
    auto pc = new ff_comb(W1[i], W2[i], farm1.isset_cleanup_workers(),
 ...
 *
 */
static inline const ff_pipeline combine_ofarm_farm(
    ff_farm &farm1, ff_farm &farm2) {
  ff_pipeline newpipe;
  if (!farm1.isOFarm()) {
  ...
  if (farm2.isOFarm() && farm1.getNWorkers() == farm2.getNWorkers()) {
    error("combine_ofarm_farm, two ordered farms with the same cardinality, "
          "the function cambine_farms_nf must be called explicitly\n");
  ...
  const size_t memsize =
      farm1.getNWorkers() *
          (2 * newfarm1.ondemand_buffer() + ff_farm::DEF_IN_OUT_DIFF + 3) +
      DEF_OFARM_ONDEMAND_MEMORY;
  newfarm1.ordered_resize_memory(memsize);
  _lb->init(newfarm1.ordered_get_memory(), memsize);
  newfarm1.setlb(_lb, true);
  OrderedCollectorWrapper *cw =
      new OrderedCollectorWrapper(DEF_OFARM_ONDEMAND_MEMORY);
  assert(cw);
  ...
  if (!collector1 && !emitter2) {
    ...
    if (!collector1) {
      ff_comb *comb =
          new ff_comb(cw, emitter2, true, farm2.isset_cleanup_emitter());
      if (farm2.isset_cleanup_emitter()) farm2.cleanup_emitter(false);
      ...
      if (!emitter2) {
        ff_comb *comb =
            new ff_comb(cw, collector1, true, farm1.isset_cleanup_collector());
        if (farm1.isset_cleanup_collector()) farm1.cleanup_collector(false);

        farm2.change_emitter(comb, true);
      } else {
        ff_comb *comb0 = new ff_comb(collector1, emitter2,
            farm1.isset_cleanup_collector(), farm2.isset_cleanup_emitter());
        if (farm1.isset_cleanup_collector()) farm1.cleanup_collector(false);
        if (farm2.isset_cleanup_emitter()) farm2.cleanup_emitter(false);

        ff_comb *comb = new ff_comb(cw, comb0, true, true);
        farm2.change_emitter(comb, true);
      }
    }
  }
...
static inline const ff_pipeline combine_farms(ff_farm &farm1, const C_t *node1,
    ff_farm &farm2, const E_t *node2, bool mergeCE) {
  ff_pipeline newpipe;
  ...
  if (mergeCE) { // we have to merge nodes!!!
    ...
    if (farm1.isOFarm() || farm2.isOFarm()) {
      if (node1 != nullptr || node2 != nullptr) { // TODO
        error("combine_farms, FEATURE NOT YET SUPPORTED, if at least one of "
              "the two farms is an ordered farm then node1 and node2 must be "
              "nullptr\n");
        return newpipe;
      }
      if (farm1.getNWorkers() == farm2.getNWorkers()) {
        ...
        // since this kind of transformation may violates the ordering semantics,
        // the user must call it explicitly
        error("combine_farms, at least one of the two farms is ordered and "
              "they have the same cardinality, the function cambine_farms_nf "
  ...
  if (farm1.isOFarm() || farm2.isOFarm()) {
    error("combine_farms, A2A cannot be introduced if one of the two farms is "
          "an ordered farms\n");
    return newpipe;
  }
  ...
  if (node2 == nullptr && node1 != nullptr) { // case3.1
    ...
    for (size_t i = 0; i < W1.size(); ++i) {
      if (!node1->isMultiOutput()) {
        ...
        assert(c);
        auto pc = new ff_comb(W1[i], c, farm1.isset_cleanup_workers(), true);
        assert(pc);
        Wtmp1[i] = pc;
      }
    }
    if (farm1.isset_cleanup_workers()) farm1.cleanup_workers(false);
    a2a->add_firstset(
        Wtmp1, farm2.ondemand_buffer(), true); // cleanup set to true
    a2a->add_secondset(W2, farm2.isset_cleanup_workers());
    if (farm2.isset_cleanup_workers()) farm2.cleanup_workers(false);
  ...
  for (size_t i = 0; i < W2.size(); ++i) {
    if (!node2->isMultiInput()) {
      ...
      assert(mi);
      auto pc = new ff_comb(mi, W2[i], true, farm2.isset_cleanup_workers());
      assert(pc);
      Wtmp2[i] = pc;
    } else {
      ...
      assert(e);
      auto pc = new ff_comb(e, W2[i], true, farm2.isset_cleanup_workers());
      assert(pc);
      Wtmp2[i] = pc;
    }
  }
