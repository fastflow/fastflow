...
protected:
  ...
  struct ff_task_f_t : public base_f_t {
    ff_task_f_t(const F_t F, Param &... a) : F(F) {
...
public:
  ...
  ff_taskf(int maxnw = ff_realNumCores(),
      ...
      : ff_farm(true, (std::max)(maxTasks, (size_t)(MAX_NUM_THREADS * 8)),
            (std::max)(maxTasks, (size_t)(MAX_NUM_THREADS * 8)), true, maxnw,
            true),
        farmworkers(maxnw), ntasks(0),
        outstandingTasks((std::max)(maxTasks, (size_t)(MAX_NUM_THREADS * 8))),
        taskscounter(0) {
    ...
    if (ff_farm::prepare() < 0) error("ff_taskf: running farm (1)\n");
  ...
  inline task_f_t *AddTask(const F_t F, Param... args) {
    ...
    ff_task_f_t<F_t, Param...> *wtask =
        new ff_task_f_t<F_t, Param...>(F, args...);
