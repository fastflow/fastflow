...
// (icpc -E -dM -std=c++11 -x c++ /dev/null | grep GXX_EX)
    ...
    (defined(HAS_CXX11_AUTO) && defined(HAS_CXX11_LAMBDA))
...
protected:
  int prepare() {
    if (!_node::prepared) {
      ...
      pfr.resetskipwarmup();
      auto r = -1;
      if (pfr.run_then_freeze() != -1) r = pfr.wait_freezing();
      if (r < 0) {
...
public:
  ...
  ff_Map(size_t maxp = -1, bool spinWait = false, bool spinBarrier = false)
      : pfr(maxp, false, true,
            spinBarrier), // skip loop warmup and disable spinwait
        spinWait(spinWait) {
  ...
  inline void parallel_reduce(reduceT &var, const reduceT &identity, long first,
      long last, const Function &partialreduce_body,
      const FReduction &finalreduce_body, const long nw = FF_AUTO) {
    pfr.parallel_reduce(
        var, identity, first, last, partialreduce_body, finalreduce_body, nw);
  }
  template <typename Function, typename FReduction>
  inline void parallel_reduce(reduceT &var, const reduceT &identity, long first,
      long last, long step, const Function &body, const FReduction &finalreduce,
      const long nw = FF_AUTO) {
    pfr.parallel_reduce(
        var, identity, first, last, step, body, finalreduce, nw);
  }
  template <typename Function, typename FReduction>
  inline void parallel_reduce(reduceT &var, const reduceT &identity, long first,
    ...
    pfr.parallel_reduce(
        var, identity, first, last, step, grain, body, finalreduce, nw);
  }

  template <typename Function, typename FReduction>
  inline void parallel_reduce_thid(reduceT &var, const reduceT &identity,
      long first, long last, long step, long grain, const Function &body,
      const FReduction &finalreduce, const long nw = FF_AUTO) {
    pfr.parallel_reduce_thid(
        var, identity, first, last, step, grain, body, finalreduce, nw);
  }
  template <typename Function, typename FReduction>
