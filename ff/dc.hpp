...
template <typename IN_t, typename OUT_t = IN_t, typename OperandType = IN_t,
    typename ResultType = OUT_t, typename compare_t = CompareTask_Par>
class ff_DC : public ff_node_t<IN_t, OUT_t> {
  using divide_f_t =
      std::function<void(const OperandType &, std::vector<OperandType> &)>;
  using combine_f_t =
      std::function<void(std::vector<ResultType> &, ResultType &)>;
...
protected:
  ...
  virtual OUT_t *releaseTask(ResultType *in) { return (OUT_t *)in; }
  ...
  struct ff_dac_f_t : public base_f_t {
    ff_dac_f_t(const F_t F, Param &... a) : F(F) {
  ...
  /**
     ...
     */
  static void CombineFunction(void * /*w*/,
      const std::function<void(std::vector<ResultType> &, ResultType &)>
          &combine_fn,
      std::vector<ResultType> *ress, ResultType *res,
      std::vector<OperandType> *subops) {
    combine_fn(*ress, *res);
  ...
  /**
     ...
     */
  static void DACFunction(void *w,
      const std::function<void(const OperandType &, std::vector<OperandType> &)>
          &_divide_fn,
      const std::function<void(std::vector<ResultType> &, ResultType &)>
          &_combine_fn,
    ...
    if (!_condition_fn(*op)) //this is not the base case
    {
      ...
      std::vector<ResultType> *ress =
          new std::vector<ResultType>(branch_factor);
      ...
      for (int i = 0; i < branch_factor - 1; i++) {
        ...
        params.push_back(r);
        ((DACWorker *)w)
            ->AddTaskWorker(params,
                ff_DC<IN_t, OUT_t, OperandType, ResultType,
                    compare_t>::DACFunction,
                w, _divide_fn, _combine_fn, _seq_fn, _condition_fn, &(*ops)[i],
                &(*ress)[i]);
      }

      DACFunction(w, _divide_fn, _combine_fn, _seq_fn, _condition_fn,
          &(*ops)[branch_factor - 1], &(*ress)[branch_factor - 1]);
  ...
  struct DACWorker : ff_node_t<hash_task_t, generic_task_t> {
    ...
    inline generic_task_t *svc(hash_task_t *task) {
      ...
      generic_task_t *gt =
          (generic_task_t *)TASK_MALLOC(sizeof(generic_task_t));
    ...
    template <typename F_t, typename... Param>
    inline void AddTaskWorker(
        std::vector<param_info> &P, const F_t F, Param... args) {

      ff_dac_f_t<F_t, Param...> *wtask =
          new ff_dac_f_t<F_t, Param...>(F, args...);
      generic_task_t *gt =
          (generic_task_t *)TASK_MALLOC(sizeof(generic_task_t));
      new (gt)
          task_f_t(); // calling the constructor of the internal data structure
      gt->is_new_task = true;
      gt->tf.P = P;
      gt->tf.wtask = wtask;
      this->ff_send_out(gt);
  ...
  protected:
    inline hash_task_t *insertTask(
        task_f_t *const msg, hash_task_t *waittask = nullptr) {
      unsigned long act_id = baseSched::task_id++;
      hash_task_t *act_task =
          baseSched::createTask(act_id, NOT_READY, msg->wtask);
      ...
      for (auto p : msg->P) {
        ...
        if (dir == INPUT) {
          ...
          unsigned long t_id =
              (unsigned long)icl_hash_find(baseSched::address_set, (void *)d);
          hash_task_t *t = NULL;
          ...
          if (t != NULL) {
            if (t->unblock_numb == t->unblock_act_numb) {
              ...
              t->unblock_task_ids =
                  (unsigned long *)TASK_REALLOC(t->unblock_task_ids,
                      t->unblock_act_numb * sizeof(unsigned long));
            }
          ...
          if (t != NULL) { //someone write that data
            ...
            if (t->unblock_numb > 0) {
              ...
              for (long ii = 0; ii < t->unblock_numb; ii++) {
                hash_task_t *t2 = (hash_task_t *)icl_hash_find(
                    baseSched::task_set, &t->unblock_task_ids[ii]);
                ...
                if (t2 != NULL && t2 != act_task && t2->status != DONE &&
                  ...
                  if (act_task->unblock_numb == act_task->unblock_act_numb) {
                    ...
                    act_task->unblock_task_ids = (unsigned long *)TASK_REALLOC(
                        act_task->unblock_task_ids,
                        act_task->unblock_act_numb * sizeof(unsigned long));
                  }
              ...
              for (size_t i = 0; i < todelete.size(); ++i) {
                t->unblock_task_ids[todelete[i]] =
                    t->unblock_task_ids[t->unblock_numb];
                t->unblock_numb--;
              }
          ...
          icl_hash_insert(
              baseSched::address_set, (void *)d, (void *)(act_task->id));
        }
      }
  ...
  public:
    Scheduler(ff_loadbalancer *lb, const int maxnw,
        void (*schedRelaxF)(unsigned long),
        const std::function<void(
            const OperandType &, std::vector<OperandType> &)> &divide_fn,
        const std::function<void(std::vector<ResultType> &, ResultType &)>
            &combine_fn,
        const std::function<void(const OperandType &, ResultType &)> &seq_fn,
        const std::function<bool(const OperandType &)> &cond_fn,
        const OperandType *op, ResultType *res)
        : TaskFScheduler<generic_task_t, compare_t>(lb, maxnw), task_numb(0),
          task_completed(0), bk_count(0), schedRelaxF(schedRelaxF),
          _divide_fn(divide_fn), _combine_fn(combine_fn), _seq_fn(seq_fn),
          _condition_fn(cond_fn), _op(op), _res(res) {}
    virtual ~Scheduler() {}
        ...
        if (!_condition_fn(*_op)) {
          ...
          for (int i = 0; i < branch_factor; i++) {
            ...
            this->AddTaskScheduler(params,
                ff_DC<IN_t, OUT_t, OperandType, ResultType,
                    compare_t>::DACFunction,
                (void *)0, _divide_fn, _combine_fn, _seq_fn, _condition_fn,
                &(*ops)[i], &(*ress)[i]);
          }
          ...
          this->AddTaskScheduler(
              params, CombineFunction, (void *)0, _combine_fn, ress, _res, ops);
  ...
  private:
    ...
    inline void schedule() { schedule_task(0); }
  };
...
public:
  ...
  ff_DC(const divide_f_t &divide_fn, const combine_f_t &combine_fn,
      const seq_f_t &seq_fn, const cond_f_t &cond_fn, const OperandType &op,
      ResultType &res, int numw,
      size_t /*outstandingTasks*/ = DEFAULT_OUTSTANDING_TASKS,
      int maxnw = ff_numCores(), void (*schedRelaxF)(unsigned long) = NULL)
      : _divide_fn(divide_fn), _combine_fn(combine_fn), _seq_fn(seq_fn),
        _condition_fn(cond_fn) {
    ...
    farm->add_emitter(
        sched = new Scheduler(farm->getlb(), numw, schedRelaxF, _divide_fn,
  ...
  ff_DC(
      ...
      const std::function<void(std::vector<ResultType> &, ResultType &)>
          &combine_fn,
      const std::function<void(const OperandType &, ResultType &)> &seq_fn,
      const std::function<bool(const OperandType &)> &cond_fn, int numw,
      size_t outstandingTasks = DEFAULT_OUTSTANDING_TASKS,
      int maxnw = ff_numCores(), void (*schedRelaxF)(unsigned long) = NULL)
      : _divide_fn(divide_fn), _combine_fn(combine_fn), _seq_fn(seq_fn),
        _condition_fn(cond_fn) {
    ...
    farm->add_emitter(
        sched = new Scheduler(farm->getlb(), numw, schedRelaxF, _divide_fn,
            _combine_fn, _seq_fn, _condition_fn, nullptr, nullptr));
    farm->wrap_around();
  }
  ...
  void setNumWorkers(ssize_t nw) {
    if (nw > ff_numCores())
      error(
          "ff_DC: setNumWorkers: too much workers, setting num worker to %d\n",
          ff_numCores());
