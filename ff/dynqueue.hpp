...
public:
  ...
  dynqueue(int cachesize = DEFAULT_CACHE_SIZE, bool fillcache = false)
      : cache(cachesize) {
...
private:
  ...
  long padding1[longxCacheLine -
                ((sizeof(Node *) + sizeof(unsigned long)) / sizeof(long))];
  Node *volatile tail;
  volatile unsigned long pread;
  long padding2[longxCacheLine -
                ((sizeof(Node *) + sizeof(unsigned long)) / sizeof(long))];
...
public:
  ...
  dynqueue(int cachesize = DEFAULT_CACHE_SIZE, bool fillcache = false)
    ...
    cache = (void **)getAlignedMemory(
        longxCacheLine * sizeof(long), cachesize * sizeof(void *));
    if (!cache) {
  ...
  inline bool push(void *const data) {
    ...
    if (!cachepop(&p.n2)) p.n = (Node *)::malloc(sizeof(Node));
