...
#define FF_PARFOR_END(name)                                                    \
  ...
  {                                                                            \
    if (name.getnw() > 1) {                                                    \
      ...
      F_##name(name.startIdx(), name.stopIdx(), 0, 0);                         \
  }
...
/**
     ...
     *  op      : reduce operation (+ * ....) 
     */
#define FF_PARFORREDUCE_BEGIN(                                                 \
    name, var, identity, idx, begin, end, step, chunk, nw)                     \
  ff_forall_farm<forallreduce_W<decltype(var)>> name(nw, false, true);         \
  name.setloop(begin, end, step, chunk, nw);                                   \
  auto idtt_##name = identity;                                                 \
  auto F_##name = [&](const long start, const long stop,                       \
                      const int _ff_thread_id, decltype(var) &var) {                                      \
        FF_IGNORE_UNUSED(_ff_thread_id);                                          \
        PRAGMA_IVDEP;                                                             \
        for(long idx=start;idx<stop;idx+=step)

#define FF_PARFORREDUCE_BEGIN_IDX(                                             \
    name, var, identity, idx, begin, end, step, chunk, nw, onoff)              \
  ff_forall_farm<forallreduce_W<decltype(var)>> name(nw, false, true);         \
  name.setloop(begin, end, step, chunk, nw);                                   \
  name.disableScheduler(onoff);                                                \
  auto idtt_##name = identity;                                                 \
...
#define FF_PARFOR_ASSIGN(name, nw)                                             \
  name = new ff_forall_farm<forallreduce_W<int>>(nw)
...
#define FF_PARFORREDUCE_DECL(name, type)                                       \
  ff_forall_farm<forallreduce_W<type>> *name
#define FF_PARFORREDUCE_ASSIGN(name, type, nw)                                 \
  name = new ff_forall_farm<forallreduce_W<type>>(nw)
...
#define FF_PARFORREDUCE_START_IDX(                                             \
    name, var, identity, idx, begin, end, step, chunk, nw)                     \
  name->setloop(begin, end, step, chunk, nw);                                  \
  auto idtt_##name = identity;                                                 \
  auto F_##name = [&](const long ff_start_idx, const long ff_stop_idx,         \
...
#define FF_PARFORREDUCE_START_STATIC(                                          \
    name, var, identity, idx, begin, end, step, chunk, nw)                     \
  assert(chunk <= 0);                                                          \
  name->setloop(begin, end, step, chunk, nw);                                  \
  auto idtt_##name = identity;                                                 \
...
struct forall_task_t {
  ...
  forall_task_t(const forall_task_t &t) : end(t.end) {
    start.store(t.start.load(
        std::memory_order_relaxed)); // MA: consistency of store to be checked
  }
...
struct dataPair {
  ...
  dataPair(const dataPair &d) : task(d.task) {
    ntask.store(d.ntask.load(
        std::memory_order_relaxed)); // MA: consistency of store to be checked
  }
  dataPair &operator=(const dataPair &d) {
    ntask = d.ntask.load(std::memory_order_relaxed), task = d.task;
    return *this;
  }
};
...
protected:
  ...
  virtual inline size_t init_data(ssize_t start, ssize_t stop) {
    ...
    eossent.resize(_nw);
    taskv.resize(
        8 * _nw); // 8 is the maximum n. of jumps, see the heuristic below
    ...
    if (totalnumtasks) {
      ...
      // try to keep the n. of tasks per worker as smaller as possible
      if (ntxw > 1)
        data[_nw - 1].ntask += totalnumtasks;
      else {
        --tt, _chunk *= 2;
      }
  ...
  virtual inline size_t init_data_static(long start, long stop) {
    ...
    long chunk = -_chunk;
    _chunk =
        stop; // needed because sendTask has to send the range (begin, stop(
    const long numtasks = std::lrint(std::ceil((stop - start) / (double)_step));
...
public:
  forall_Scheduler(ff_loadbalancer *lb, long start, long stop, long step,
      long chunk, size_t nw)
      : lb(lb), _start(start), _stop(stop), _step(step), _chunk(chunk),
        totaltasks(0), _nw(nw), jump(0), skip1(false), workersspinwait(false),
    ...
    if (_chunk <= 0)
      totaltasks = init_data_static(start, stop);
    else
      totaltasks = init_data(start, stop);
    assert(totaltasks >= 1);
  }
  forall_Scheduler(ff_loadbalancer *lb, size_t nw)
      : lb(lb), _start(0), _stop(0), _step(1), _chunk(1), totaltasks(0),
        _nw(nw), jump(0), skip1(false), workersspinwait(false),
        static_scheduling(false) {
#ifdef FF_PARFOR_PASSIVE_NOSTEALING
    _nextIteration = 0;
#endif
  ...
  inline bool canUseNoStealing() {
    return !globalSchedRunning && !static_scheduling && _step == 1 &&
           _chunk == 1;
  }
#endif
  ...
  more:
    ...
    if (!skipmore && !skip1 && totaltasks >= 4 * _nw) {
      skip1 = true;
      goto more;
    }
  ...
  inline bool nextTaskConcurrentNoStealing(forall_task_t *task, const int wid) {
    ...
    error("To use nextTaskConcurrentNoStealing you need to define macro "
          "FF_PARFOR_PASSIVE_NOSTEALING\n");
    return false;
#endif
  }
  ...
  L1:
    ...
    if (maxid.load(std::memory_order_acquire) < 0)
      _maxid = (long)(std::max_element(data.begin(), data.end(), data_cmp) -
                      data.begin());
    else
      _maxid = maxid;
    ...
    if (maxid.load(std::memory_order_acquire) < 0)
      _maxid =
          (std::max_element(data.begin(), data.end(), data_cmp) - data.begin());
    else
      _maxid = maxid;
  L2:
    ntask = data[_maxid].ntask.load(std::memory_order_relaxed);
  ...
  inline bool nextTask(forall_task_t *task, const int wid) {
    ...
    L2:
      maxid = (long)(std::max_element(data.begin(), data.end(), data_cmp) -
                     data.begin());
  ...
  inline void *svc(void *t) {
    ...
    if (nextTask((forall_task_t *)t, (int)wid))
      lb->ff_send_out_to(t, int(wid));
    else {
      if (!eossent[wid]) {
        lb->ff_send_out_to((workersspinwait ? EOS_NOFREEZE : GO_OUT), int(wid));
        eossent[wid] = true;
...
public:
  forallreduce_W(forall_Scheduler *const sched, ffBarrier *const loopbar, F_t F)
      : sched(sched), loopbar(loopbar), schedRunning(true), spinwait(false),
        aggressive(true), F(F) {}
...
public:
  ...
  typedef std::function<void(
      const long, const long, const int, ff_buffernode &)>
      F_t;

public:
  forallpipereduce_W(
      forall_Scheduler *const sched, ffBarrier *const loopbar, F_t F)
...
public:
  ff_forall_farm(ssize_t maxnw, const bool spinwait = false,
      const bool skipwarmup = false, const bool spinbarrier = false)
      : ff_farm(false, 8 * DEF_MAX_NUM_WORKERS, 8 * DEF_MAX_NUM_WORKERS, true,
            DEF_MAX_NUM_WORKERS, true), // cleanup at exit !
        loopbar(
            (spinwait && spinbarrier)
                ? (ffBarrier *)(new spinBarrier(maxnw <= 0
                                                    ? DEF_MAX_NUM_WORKERS + 1
                                                    : (size_t)(maxnw + 1)))
                : (ffBarrier *)(new Barrier(maxnw <= 0 ? DEF_MAX_NUM_WORKERS + 1
                                                       : (size_t)(maxnw + 1)))),
    ...
    if (ff_farm::prepare() < 0) error("running base forall farm(2)\n");
    ...
    if (!skipwarmup) {
      auto r = -1;
      getlb()->freeze();
      if (getlb()->run() != -1) r = getlb()->wait_freezing();
  ...
  //
  inline bool startScheduler(
      const size_t nwtostart, const size_t numtasks) const {
  ...
  inline int run_and_wait_end() {
    ...
    if (schedRunning) {
      ...
      if (getlb()->runlb() != -1) {
        if (getlb()->runWorkers(nwtostart) != -1) r = getlb()->wait();
      }
    } else {
      ((forall_Scheduler *)getEmitter())->sendTask(true);
      if (getlb()->runWorkers(nwtostart) != -1) r = getlb()->waitWorkers();
    }
    return r;
  }
  ...
  inline int wait() {
    if (spinwait) {
      const svector<ff_node *> &nodes = getWorkers();
      for (size_t i = 0; i < nodes.size(); ++i) getlb()->ff_send_out_to(EOS, i);
    }
    return ff_farm::wait();
  }
  ...
  inline void setF(F_t _F, const Tres_t &idtt = Tres_t()) { //(Tres)0) {
    ...
    schedRunning = (!removeSched &&
                    startScheduler(
                        nw, ((forall_Scheduler *)getEmitter())->getnumtasks()));
  ...
  inline void setloop(long begin, long end, long step, long chunk, long nw) {
    if (nw > (ssize_t)getNWorkers()) {
      error("The number of threads specified is greater than the number set in "
            "the ParallelFor* constructor, it will be downsized\n");
      nw = getNWorkers();
    }
    assert(nw <= (ssize_t)getNWorkers());
    forall_Scheduler *sched = (forall_Scheduler *)getEmitter();
    sched->setloop(
        begin, end, step, chunk, (nw <= 0) ? getNWorkers() : (size_t)nw);
  }
  // return the number of workers running or supposed to run
  inline size_t getnw() {
    return ((const forall_Scheduler *)getEmitter())->running();
  }
  ...
  inline long startIdx() {
    return ((const forall_Scheduler *)getEmitter())->startIdx();
  }
  inline long stopIdx() {
    return ((const forall_Scheduler *)getEmitter())->stopIdx();
  }
  inline long stepIdx() {
    return ((const forall_Scheduler *)getEmitter())->stepIdx();
  }

  void resetskipwarmup() {
    assert(skipwarmup);
    skipwarmup = false;
  }
