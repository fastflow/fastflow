...
public:
  BufferPool(
      int cachesize, const bool fillcache = false, unsigned long size = -1)
...
public:
  ...
  inline bool available() { return buf_w->available(); }
  ...
  inline bool flush() { return (mcnt ? multipush() : true); }
  ...
  inline unsigned long readMiss() { return pool.readPoolMiss(); }

  inline unsigned long readHit() { return pool.readPoolHit(); }
#endif
