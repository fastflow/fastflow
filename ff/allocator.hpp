...
struct xThreadData {
  ...
  ~xThreadData() {
    if (leak) {
      leak->~uSWSR_Ptr_Buffer();
      freeAlignedMemory(leak);
    } // free(leak)
  }

  uSWSR_Ptr_Buffer *leak; //
  const pthread_t key;    // used to identify a thread (threadID)
  long padding[longxCacheLine -
               ((sizeof(const pthread_t) + sizeof(uSWSR_Ptr_Buffer *)) /
                   sizeof(long))]; //
};
...
private:
  ...
  inline Seg_ctl *getsegctl(Buf_ctl *buf) { return *((Seg_ctl **)buf); }
  ...
  inline int newslab() {
    /*
             ...
             */
    void *ptr = alloc->newsegment(
        size * nslabs + sizeof(Seg_ctl) + nslabs * BUFFER_OVERHEAD);
  ...
  inline void freeslab(void *ptr, bool seglist_rem = true) {
    alloc->freesegment(
        ptr, size * nslabs + sizeof(Seg_ctl) + nslabs * BUFFER_OVERHEAD);
...
public:
  ...
  SlabCache(ff_allocator *const mainalloc, const int delayedReclaim,
      ...
      : size(sz), nslabs(ns), fb(0), fb_capacity(0), fb_size(0), buffptr(0),
        availbuffers(0), alloc(alloc), mainalloc(mainalloc),
        delayedReclaim(delayedReclaim), lastqueue(0) {}
  ...
  inline xThreadData *register4free(const bool allocator = false) {
    ...
    if (entry < 0) {                // if threadID not found
      ...
      if (fb_size == fb_capacity) {
        xThreadData **fb_new = (xThreadData **)::realloc(
            fb, (fb_capacity + MIN_FB_CAPACITY) * sizeof(xThreadData *));
        if (!fb_new) {
          spin_unlock(lock);
          return NULL;
        }
  ...
  inline void *getitem() {
    ...
    avail:
      ...
      DBG(if ((getsegctl((Buf_ctl *)((char *)item - sizeof(Buf_ctl))))
                  ->allocator == NULL) abort());
      return item;
    }
  ...
  inline bool putitem(Buf_ctl *buf) {
    ...
    if (entry < 0)
      xtd = register4free(); // if not present, register it
    else
      xtd = fb[entry]; // else, point to its position
    DBG(if (!xtd) abort());
...
protected:
  ...
  inline size_t allocatedsize() {
    ...
    for (; b != e; ++b) {
      if ((*b) && (*b)->getnslabs()) s += (*b)->allocatedsize();
    }
    return s;
  }
...
public:
  ...
  int init(const int _nslabs[N_SLABBUFFER] = 0, bool prealloc = true) {
    ...
    if (_nslabs)
      for (int i = 0; i < N_SLABBUFFER; ++i) nslabs.push_back(_nslabs[i]);
    else
    ...
    for (int i = 0; i < N_SLABBUFFER; ++i) {
      ...
      if (!s) return -1;
      new (s) SlabCache(this, delayedReclaim, alloc, buffersize[i], nslabs[i]);
  ...
  inline void deregisterAllocator(bool reclaimMemory = true) {
    svector<SlabCache *>::iterator b(slabcache.begin()), e(slabcache.end());
    for (; b != e; ++b) {
      if ((*b)->getnslabs()) (*b)->deregisterAllocator(reclaimMemory);
    }
  }
  ...
  inline int posix_memalign(void **memptr, size_t alignment, size_t size) {
    ...
    if (realsize > MAX_SLABBUFFER_SIZE ||
      ...
      ((Buf_ctl *)ptr)->ptr = 0;
      void *ptraligned =
          (void *)(((long)((char *)ptr + sizeof(Buf_ctl)) + alignment) &
                   ~(alignment - 1));
    ...
    DBG(assert(backptr));
    void *ptraligned =
        (void *)(((long)((char *)buf) + alignment) & ~(alignment - 1));
  ...
  ALLSTATS(
      void printstats(std::ostream &out) { all_stats::instance()->print(out); })
...
public:
  ...
  inline void *malloc(size_t size) { return ff_allocator::malloc(size); }
...
private:
  inline void nomoremalloc() { nomorealloc.store(1); }
...
protected:
  ...
  inline FFAxThreadData *newAllocator(
      bool prealloc, int _nslabs[N_SLABBUFFER], size_t max_size) {
    FFAxThreadData *ffaxtd = (FFAxThreadData *)::malloc(
        sizeof(FFAxThreadData) + sizeof(ffa_wrapper));
    ...
    if (A_size == A_capacity) {
      FFAxThreadData **A_new = (FFAxThreadData **)::realloc(
          A, (A_capacity + MIN_A_CAPACITY) * sizeof(FFAxThreadData *));
...
public:
  ...
  FFAllocator(int delayedReclaim = 0)
      : A(0), A_capacity(0), A_size(0), delayedReclaim(delayedReclaim) {
    init_unlocked(lock);

    if (pthread_key_create(
            &A_key, (delayedReclaim ? NULL : FFAkeyDestructorHandler)) != 0) {
  ...
  inline ffa_wrapper *newAllocator(size_t max_size = 0,
      int _nslabs[N_SLABBUFFER] = 0, bool prealloc = true) {
  ...
  inline void deleteAllocator(ffa_wrapper *f, bool reclaimMemory = true) {
    ...
    for (unsigned int i = 0; i < A_size; ++i) {
      if (A[i]->f == f) {
        ...
        for (unsigned int j = i + 1; j < A_size; ++i, ++j) A[i] = A[j];
        --A_size;
...
static inline void ff_free(void *ptr) { FFAllocator::instance()->free(ptr); }
static inline void *ff_realloc(void *ptr, size_t newsize) {
  return FFAllocator::instance()->realloc(ptr, newsize);
}
static inline int ff_posix_memalign(
