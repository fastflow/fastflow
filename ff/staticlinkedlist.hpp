...
public:
  ...
  staticlinkedlist(
      int cachesize = DEFAULT_CACHE_SIZE, bool /*fillcache*/ = false) {
    // avoid unused field warning for padding
    if (longxCacheLine > 1) padding1[0] = padding2[0];
    // end
    ...
    if (CAST_TO_UL(cache_mem) % CPU_cachesize) {
      min_cache = (Node *)(((CAST_TO_UL(cache_mem) / CPU_cachesize) + 1) *
                           CPU_cachesize);
    } else {
  ...
  ~staticlinkedlist() { free(cache_mem); }
