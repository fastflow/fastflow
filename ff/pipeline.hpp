...
protected:
  inline int prepare() {
    ...
    for (int i = 1; i < nstages; ++i) {
      ...
      const bool curr_single_multiinput =
          (!isa2a_curr && nodes_list[i]->isMultiInput());
      const bool curr_multi_standard = [&]() {
        if (!isa2a_curr) return false;
        const svector<ff_node *> &w1 = isa2a_getfirstset(get_node(i));
        assert(w1.size() > 0);
        if (w1[0]->isMultiInput())
          return false; // NOTE: we suppose homogeneous first set
        return true;
      }();
      const bool curr_multi_multiinput = [&]() {
        ...
        assert(w1.size() > 0);
        if (w1[0]->isMultiInput())
          return true; // NOTE: we suppose homogeneous first set
        return false;
      }();
      const bool isa2a_prev = get_node_last(i - 1)->isAll2All();
      const bool isfarm_prev = get_node_last(i - 1)->isFarm();
      const bool prev_isfarm_nocollector =
          (isfarm_prev && !get_node_last(i - 1)->isOFarm() &&
              !(isfarm_withcollector(get_node_last(i - 1))));
      const bool prev_isfarm_withcollector =
          isfarm_withcollector(get_node_last(i - 1));

      const bool prev_single_standard =
          (!get_node_last(i - 1)->isMultiOutput());
      const bool prev_single_multioutput =
          ((get_node_last(i - 1)->isMultiOutput() && !isa2a_prev &&
               !isfarm_prev) ||
              (prev_isfarm_withcollector &&
                  get_node_last(i - 1)->isMultiOutput()));
      const bool prev_multi_standard = [&]() {
        if (prev_isfarm_nocollector) {
          ...
          if (!w1[0]->isMultiOutput())
            return true; // NOTE: we suppose homogeneous workers
        }
        if (isa2a_prev) {
      ...
      if (curr_single_standard) {
        ...
        if (nodes_list[i]->create_input_buffer(in_buffer_entries, fixedsize) <
            0)
          return -1;
        if (prev_single_standard) {
          if (nodes_list[i - 1]->set_output_buffer(
                  nodes_list[i]->get_in_buffer()) < 0)
            return -1;
        } else {
      ...
      if (curr_single_multiinput) {
        if (prev_single_standard) {
          if (nodes_list[i - 1]->create_output_buffer(
                  in_buffer_entries, fixedsize) < 0)
            return -1;
          ...
          if (prev_multi_standard) {
            if (nodes_list[i - 1]->create_output_buffer(
                    out_buffer_entries, fixedsize) < 0)
              return -1;
      ...
      if (curr_multi_standard) {
        ...
        if (prev_single_standard) {
          if (W1.size() > 1) {
            error("PIPE, cannot connect stage %d with stage %d because of "
                  "different cardinality\n",
                i - 1, i);
            return -1;
          }
          if (a2a->create_input_buffer(in_buffer_entries, fixedsize) < 0)
            return -1;
          svector<ff_node *> w(1);
          nodes_list[i]->get_in_nodes(w);
          assert(w.size() == 1);
          if (nodes_list[i - 1]->set_output_buffer(w[0]->get_in_buffer()) < 0)
            return -1;
        } else {
          if (prev_single_multioutput) {
            if (a2a->create_input_buffer(in_buffer_entries, fixedsize) < 0)
            ...
            if (w1.size() != W1.size()) {
              error("PIPE, cannot connect stage %d with stage %d, because of "
                    "different input/output cardinality (**)\n",
                  i - 1, i);
              return -1;
            }
            if (a2a->create_input_buffer(in_buffer_entries, fixedsize) < 0)
            ...
            for (size_t i = 0; i < w1.size(); ++i) {
              if (prev_multi_multioutput) {
                ...
                if (w1[i]->set_output_buffer(W1[i]->get_in_buffer()) < 0)
                  return -1;
              }
            }
    ...
    if (has_input_channel) {
      ...
      if (!init_output_blocking(m, c)) {
        error(
            "FARM, add_collector, init input blocking mode for accelerator\n");
      }
...
public:
  ...
  explicit ff_pipeline(bool input_ch = false,
      ...
      : has_input_channel(input_ch), node_cleanup(false), fixedsize(fixedsize),
        in_buffer_entries(in_buffer_entries),
        out_buffer_entries(out_buffer_entries) {}

  ff_pipeline(const ff_pipeline &p) : ff_node(p) {
    if (p.prepared) {
      error("ff_pipeline, copy constructor, the input pipeline is already "
            "prepared\n");
      return;
    }
  ...
  void remove_stage(int pos, bool remove_from_cleanuplist = false) {
    if (prepared) {
      error("PIPE, remove_stage, stage %d cannot be removed because the PIPE "
    ...
    if (remove_from_cleanuplist) {
      ...
      int pos2 = -1;
      for (size_t i = 0; i < internalSupportNodes.size(); ++i)
        if (internalSupportNodes[i] == node) {
          pos2 = i;
          break;
        }
      if (pos2 >= 0)
        internalSupportNodes.erase(internalSupportNodes.begin() + pos2);
    }
    nodes_list.erase(it + pos);
  }
  void insert_stage(int pos, ff_node *node, bool cleanup = false) {
    if (prepared) {
      error("PIPE, insert_stage, stage %d cannot be added because the PIPE has "
  ...
  const svector<std::pair<ff_node *, bool>> get_and_remove_nodes() {
    ...
    for (int i = 0; i < nstages; ++i) {
      if (nodes_list[i]->isPipe()) {
        ...
        const svector<std::pair<ff_node *, bool>> &W =
            p->get_and_remove_nodes();
        newvector += W;
        if (node_cleanup) {
          bool found = false;
          for (size_t i = 0; i < internalSupportNodes.size(); ++i)
            if (internalSupportNodes[i] == p) {
              found = true;
              break;
            }
  ...
  int wrap_around() {
    ...
    bool last_isfarm_nocollector =
        get_lastnode()->isFarm() && !isfarm_withcollector(get_lastnode());
    bool last_isfarm_withcollector =
        get_lastnode()->isFarm() && !last_isfarm_nocollector;

    bool first_single_standard = (!nodes_list[0]->isMultiInput());
    // the farm is considered single_multiinput
    bool first_single_multiinput =
        (nodes_list[0]->isMultiInput() && !isa2a_first);
    ...
    bool last_single_multioutput =
        ((nodes_list[last]->isMultiOutput() && !isa2a_last && !isfarm_last) ||
    ...
    bool last_multi_standard = [&]() {
      ...
      if (isa2a_last) {
        ...
        assert(w1.size() > 0);
        if (!w1[0]->isMultiOutput())
          return true; // NOTE: we suppose homogeneous second set
      }
      return false;
    }();
    ...
    if (first_single_standard) {
      ...
      if (last_single_standard) {
        ...
        if (last_single_multioutput) {
          ff_node *t =
              new ff_buffernode(last, get_in_buffer(), get_in_buffer());
          internalSupportNodes.push_back(t);
          assert(t);
    ...
    if (first_single_multiinput) {
      ...
      if (last_single_standard) {
        if (nodes_list[last]->create_output_buffer(out_buffer_entries, false) <
            0)
          return -1;
        ...
        if (last_single_multioutput) {
          ...
          if (last_multi_standard) {
            if (nodes_list[last]->create_output_buffer(
                    out_buffer_entries, false) < 0)
              return -1;
            svector<ff_node *> w(MAX_NUM_THREADS);
    ...
    if (first_multi_standard) { // all-to-all
      ...
      if (last_multi_multioutput) {
        error(
            "PIPE, wrap_around, cannot connect last stage with first stage\n");
        return -1;
      }
      nodes_list[0]->skipfirstpop(true);
    }
...
protected:
  ...
  /* The pipeline has not been flattened and its first stage is a multi-input node used as 
     * a standard node. 
     */
  inline bool put(void *ptr) { return nodes_list[0]->put(ptr); }
  ...
  virtual inline pthread_mutex_t &get_cons_m() {
    return nodes_list[0]->get_cons_m();
  }
  virtual inline pthread_cond_t &get_cons_c() {
    return nodes_list[0]->get_cons_c();
  }
  ...
  inline int set_input(ff_node *node) { return nodes_list[0]->set_input(node); }
...
private:
  ...
  struct valid_stage_types<A &&, B &&, Bs &&...>
      : std::integral_constant<bool,
            std::is_same<typename A::out_type, typename B::in_type>{} &&
                valid_stage_types<B, Bs...>{}> {};

  template <class A, class B, class... Bs>
  struct valid_stage_types<std::unique_ptr<A> &&, std::unique_ptr<B> &&,
      Bs &&...>
      : std::integral_constant<bool,
            std::is_same<typename A::out_type, typename B::in_type>{} &&
                valid_stage_types<std::unique_ptr<B>, Bs...>{}> {};

  template <class A, class B, class... Bs>
  struct valid_stage_types<std::unique_ptr<A> &&, B &&, Bs &&...>
      : std::integral_constant<bool,
            std::is_same<typename A::out_type, typename B::in_type>{} &&
                valid_stage_types<B, Bs...>{}> {};
  template <class A, class B, class... Bs>
  struct valid_stage_types<A &&, std::unique_ptr<B> &&, Bs &&...>
      : std::integral_constant<bool,
            std::is_same<typename A::out_type, typename B::in_type>{} &&
                valid_stage_types<std::unique_ptr<B>, Bs...>{}> {};
...
public:
  ...
  ff_Pipe(
      STAGES &&... stages) { // forwarding reference (aka universal reference)
#if !defined(__CUDACC__) && !defined(WIN32) && !defined(__ICC)
    static_assert(valid_stage_types<STAGES...>{},
        "Input & output types of the pipe's stages don't match");
#endif
    this->add2pipeall(
        stages...); //this->add2pipeall(std::forward<STAGES>(stages)...);
  }
  /**
