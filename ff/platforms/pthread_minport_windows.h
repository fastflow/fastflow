...
INLINE int pthread_cond_init(
    pthread_cond_t RESTRICT *cv, const pthread_condattr_t RESTRICT *attr) {
  cv->waiters_count_ = 0;
...
INLINE int pthread_cond_wait(
    pthread_cond_t RESTRICT *cv, pthread_mutex_t RESTRICT *external_mutex) {
  ...
  for (;;) {
    ...
    int wait_done =
        cv->release_count_ > 0 && cv->wait_generation_count_ != my_generation;
    LeaveCriticalSection(&cv->waiters_count_lock_);
