...
static const char *_getopt_initialize_a(
...
int _getopt_internal_r_a(int argc, char *const *argv, const char *optstring,
  ...
  }
  {
    ...
    if (temp[0] == 'W' && temp[1] == ';') {
      ...
      int indfound = 0;
      int option_index;
      if (longopts == NULL) goto no_longs;
      if (*d->__nextchar != '\0') {
...
static const wchar_t *_getopt_initialize_w(
...
int _getopt_internal_r_w(int argc, wchar_t *const *argv,
    const wchar_t *optstring, const struct option_w *longopts, int *longind,
  ...
  if (d->optind == 0 || !d->__initialized) {
    ...
    optstring++;
  if (optstring[0] == L':') print_errors = 0;
  ...
  if (longopts != NULL &&
      (argv[d->optind][1] == L'-' ||
          (long_only && (argv[d->optind][2] ||
                            !wcschr(optstring, argv[d->optind][1]))))) {
    wchar_t *nameend;
    unsigned int namelen;
...
int getopt_w(
    int argc, wchar_t *const *argv, const wchar_t *optstring) _GETOPT_THROW {
...
int _getopt_long_only_r_w(int argc, wchar_t *const *argv,
    const wchar_t *options, const struct option_w *long_options, int *opt_index,
    struct _getopt_data_w *d) {
  return _getopt_internal_r_w(
      argc, argv, options, long_options, opt_index, 1, d, 0);
}
