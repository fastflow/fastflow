...
struct ordered_gt : ff_gatherer {
  struct PairCmp {
    bool operator()(
        const ordering_pair_t *lhs, const ordering_pair_t *rhs) const {
  ...
  inline ssize_t gather_task(void **task) {
    ...
    if (*task < FF_TAG_MIN) {
      ...
      Q.push(in);
      if (Q.size() > MemSize) {
        error("FATAL ERROR: OFARM, ondemand, not enough memory, increase "
              "MemoryElements\n");
      }
      *task = FF_GO_ON;
    }
  ...
  int all_gather(void *task, void **V) {
    ssize_t sender =
        ff_gatherer::get_channel_id(); // set current sender of element task
  ...
  size_t cnt;
  std::priority_queue<ordering_pair_t *, std::vector<ordering_pair_t *>,
      PairCmp>
      Q;
  size_t MemSize;
};
