...
struct fftree {
  ...
  fftree(ff_node *ffnode_, fftype nodetype_) : nodetype(nodetype_) {
  ...
  fftree(ff_thread *ffnode_, fftype nodetype_) : nodetype(nodetype_) {
    do_comp = (nodetype == WORKER || nodetype == OCL_WORKER);
  ...
  ~fftree() {
    for (size_t i = 0; i < children.size(); ++i)
      if (children[i].first && !children[i].second) delete children[i].first;
    if (isroot) {
      treeLock.lock();
      //pthread_mutex_lock(&treeLock);
#if defined(DEBUG_FFTREE)
  ...
  bool ispattern() const { return nodetype == FARM || nodetype == PIPE; }
  ...
  size_t threadcount(threadcount_t *tc) const {
    ...
    for (size_t i = 0; i < children.size(); ++i)
      if (children[i].first) cnt += children[i].first->threadcount(tc);
    return cnt;
  }
};
...
static inline void print_fftrees(std::ostream &os) {
  size_t t = 0, total = 0;
  for (std::set<fftree *>::const_iterator it = roots.begin(); it != roots.end();
       ++it) {
    threadcount_t tc;
    os << "> ff tree @" << *it << "\n";
    (*it)->print(os);
