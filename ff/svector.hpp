...
public:
  ...
  svector(size_t chunk = SVECTOR_CHUNK)
      : first(NULL), len(0), cap(0), chunk(chunk) {
    reserve(chunk);
  }
  ...
  svector(const_iterator i1, const_iterator i2)
      : first(0), len(0), cap(0), chunk(SVECTOR_CHUNK) {
    first = (vector_type *)::malloc((i2 - i1) * sizeof(vector_type));
    while (i1 != i2) push_back(*(i1++));
  }
  ...
  inline void reserve(size_t newcapacity) {
    ...
    if (first == NULL)
      ...
      first =
          (vector_type *)::realloc(first, sizeof(vector_type) * newcapacity);
    cap = newcapacity;
  }
  ...
  inline void resize(size_t newsize) {
    ...
    reserve(newsize);
    while (len < newsize) new (first + len++) vector_type();
  }
  ...
  /**
     * front
     */
  inline vector_type &front() { return first[0]; }

  /**
     * front
     */
  inline const vector_type &front() const { return first[0]; }
  ...
  inline iterator erase(iterator where) {
    ...
    for (iterator i3 = i1++; i1 != i2; ++i3, ++i1) *i3 = *i1;
  ...
  inline iterator insert(iterator where, const vector_type &elem) {
    ...
    for (iterator i3 = i2 + 1; i2 >= i1; --i2, --i3) *i3 = *i2;
    size_t pos = (i1 - begin());
