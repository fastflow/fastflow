...
public:
  ...
  inline void out_nodes(std::deque<gnode_t *> &out) const { out = outNodes; }
...
protected:
  ...
  public:
    ...
    Emitter(const std::vector<ff_node *> &W, const long &foundflag,
  ...
  inline void resetqueues() {
    for (auto n : W) n->reset();
  }
...
public:
  ff_graphsearch(const int nw = ff_numCores(), const bool all = false)
      : found(0), start(NULL), all(all) {
  ...
  inline bool search(
      T *const st, T *const search, T *&result, const int nw = -1) {
    if (nw > (int)W.size()) {
  ...
  inline bool search(T *const st, T *const search, std::deque<T *> &result,
      const int nw = -1) {
    if (nw > (int)W.size()) {
      error("ff_graphsearch:search: nw too big, using nw=%d\n", W.size());
    }
