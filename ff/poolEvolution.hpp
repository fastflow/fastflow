...
public:
  typedef void (*selection_t)(
      ParallelForReduce<T> &, std::vector<T> &, std::vector<T> &, env_t &);
  typedef const T &(*evolution_t)(T &, const env_t &, const int);
  typedef void (*filtering_t)(
      ParallelForReduce<T> &, std::vector<T> &, std::vector<T> &, env_t &);
...
public:
  ...
  poolEvolution(size_t maxp, // maximum parallelism degree
      ...
      const env_t &E = env_t(),
      bool spinWait = true) // NOTE: spinWait does not enable spinBarrier !
      : maxp(maxp), pE(maxp), env(E), input(&pop), selection(sel),
        evolution(evol), filter(fil), termination(term),
  ...
  poolEvolution(size_t maxp, // maximum parallelism degree
      ...
      : maxp(maxp), pE(maxp), env(E), input(NULL), selection(sel),
        evolution(evol), filter(fil), termination(term),
        loopevol(maxp, spinWait) {
    loopevol.disableScheduler(true);
  }
  ...
  void setParEvolution(size_t pardegree) {
    if (pardegree > maxp)
      error("setParEvolution: pardegree too high, it should be less than or "
            "equal to %ld\n",
          maxp);
    else
      pE = pardegree;
...
protected:
  void *svc(void *task) {
    ...
    while (!termination(*input, env)) {
      ...
      loopevol.parallel_for_thid(0, buffer.size(), 1, PARFOR_STATIC(0), E, pE);
