...
static std::map<std::string, Kernel_Target_t> String2Target = {{"GPU:0", GPU0},
    {"GPU:1", GPU1}, {"GPU:2", GPU2}, {"FPGA:0", FPGA0}, {"FPGA:1", FPGA1},
    {"FPGA:2", FPGA2}, {"DSP:0", DSP0}, {"DSP:1", DSP1}, {"DSP:2", DSP2}};
...
static int Setup(const std::string &program_name) {
  ...
  if (scopeservice == nullptr) {
    std::cerr << "SCOPESERVICE env variable not set, using default value ("
              << DEFAULT_SCOPESERVICE << ")\n";
  ...
  if (rmeasureservice == nullptr) {
    std::cerr << "RMEASURESERVICE  env variable not set, using default value ("
              << DEFAULT_RMEASURESERVICE << ")\n";
    setenv("RMEASURESERVICE", DEFAULT_RMEASURESERVICE, 1);
  }
  char *fifoFile = getenv("RMEASURE_FIFO");
  if (fifoFile == nullptr) {
    std::cerr << "RMEASURE_FIFO  env variable not set, using default value ("
              << DEFAULT_FIFO_FILE << ")\n";
 ...
 */
static void Cleanup() { DPE(rpr::cleanup()); }
/**
 ...
 */
static inline const std::string schedule_kernel(
    int kernel_id, size_t problem_size = 0) {
#if defined(DPE_PROTOCOL)
 ...
 */
static inline const std::string schedule_pipeline(
    int pipeline_id, size_t problem_size = 0) {
#if defined(DPE_PROTOCOL)
...
/* ------------------  Utility functions ---------------------- */
static inline void print_Measures(
    const ff_node::RPR_measures_vector &V, const std::string &str) {
  ...
  for (size_t i = 0; i < entries; ++i) {
    ...
    for (size_t m = 0; m < V[i].size(); ++m) {
      for (size_t j = 0; j < V[i][m].size(); ++j) {
        std::cout << "  "
                  << "device=" << V[i][m][j].first << "\n";
        for (size_t k = 0; k < V[i][m][j].second.size(); ++k) {
          std::cout << "  " << V[i][m][j].second[k].time_before << ","
...
static inline void printMemoryFlags(
    const memoryflagsVector &mf, const std::string &str) {
  std::cout << str << "\n";
  for (size_t i = 0; i < mf.size(); ++i) {
    std::cout << "param: " << i << "\n";
    std::cout << "    copy   :  "
              << (mf[i].copy == CopyFlags::COPY ? "COPY" : "DONTCOPY") << "\n";
    std::cout << "    reuse  :  "
              << (mf[i].reuse == ReuseFlags::REUSE ? "REUSE" : "DONTREUSE")
              << "\n";
    std::cout << "    release:  "
              << (mf[i].release == ReleaseFlags::RELEASE ? "RELEASE"
                                                         : "DONTRELEASE")
              << "\n";
  }
  std::cout << "\n";
...
static double getSourceResults(const Measurement &measurement) {
  ...
  Measurement::KernelSourceMap::const_iterator kernelSourceIt =
      measurement.kernelSourceMap().begin();
  Measurement::SourceContainer::const_iterator resultsIt =
      kernelSourceIt->second.begin();
...
static inline void EnergyMeasure_End(
    const std::string &kernel, double &energy) {
  sendFifoMsg("E;", 0);
  energy = 0.0;
#if defined(SCOPE)
  PicoScopeMeasurement *measurement =
      PicoScopeMethod::getInstance()->stopMeasurement();
#else
  RaplMeasurement *measurement = RaplMethod::getInstance()->stopMeasurement();
#endif
  if (measurement) energy = getSourceResults(*measurement);
}
 ...
 *
 */
template <typename T, typename CONDF_t, typename INCF_t,
...
public:
  /**
     ...
     */
  streamGen_Kernel(int pipeline_id, CONDF_t condF, INCF_t incF,
      INITF_t initF = default_init_F<T>, const size_t init_value = 0)
      : pipeline_id(pipeline_id), init_value(init_value), condF(condF),
        incF(incF), initF(initF) {}
...
template <typename T>
static size_t zeroProblemSize(const T &) {
  return 0;
}
...
public:
  ...
  Kernel(int kernel_id)
      : kernel_id(kernel_id), Task(nullptr),
...
#if defined(ENERGY_MEASUREMENT)
    if (measureEnergy)
      EnergyMeasure_End("kernel", measure.energy);
    else
      measure.energy = 0.0;
#endif
...
public:
  ...
  RPR_measures_vector rpr_get_measures() {
    ...
    (tmp[0][CPU].second)[0].time_before =
        getusec(ff_Pipe<>::startTime()); // stores pipeline starting time
...
public:
  explicit Farm_Kernel(const int farm_id,
      ...
      : ff_Farm<>(std::move(W), std::move(E), std::move(C), false),
        farm_id(farm_id) {
    //sets bounded queues
...
public:
  explicit OFarm_Kernel(const int farm_id,
      ...
      std::unique_ptr<ff_node> E = std::unique_ptr<ff_node>(nullptr),
      std::unique_ptr<ff_node> C = std::unique_ptr<ff_node>(nullptr))
      : ff_OFarm<>(std::move(W), false), EmitterF(std::move(E)),
        CollectorF(std::move(C)), farm_id(farm_id) {
    //sets bounded queues
